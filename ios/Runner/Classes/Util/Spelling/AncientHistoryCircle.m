...
@implementation NameSquareMeasureUnit
...
@interface AncientHistoryCircle ()
//: - (SpellingUnit *)calcSpellingOfString: (NSString *)source;
- (NameSquareMeasureUnit *)confirm: (NSString *)source;
//: @end
@end


//: @implementation NTESSpellingCenter
@implementation AncientHistoryCircle
//: + (NTESSpellingCenter *)sharedCenter
+ (AncientHistoryCircle *)heritage
{
    //: static NTESSpellingCenter *instance = nil;
    static AncientHistoryCircle *instance = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSpellingCenter alloc]init];
        instance = [[AncientHistoryCircle alloc]init];
    //: });
    });
    //: return instance;
    return instance;
...
- (NameSquareMeasureUnit *)pageView:(NSString *)source
{
    //: if ([source length] == 0)
    if ([source length] == 0)
    {
    ...
    NameSquareMeasureUnit *spellingUnit = nil;
    //: @synchronized(self)
    @synchronized(self)
    {
        //: SpellingUnit *unit = [_spellingCache objectForKey:source];
        NameSquareMeasureUnit *unit = [_spellingCache objectForKey:source];
        //: if (!unit)
        if (!unit)
        {
            //: unit = [self calcSpellingOfString:source];
            unit = [self confirm:source];
...
- (NSString *)destabilisation:(NSString *)input
{
    //: SpellingUnit *unit = [self spellingForString:input];
    NameSquareMeasureUnit *unit = [self pageView:input];
...
- (NameSquareMeasureUnit *)confirm:(NSString *)source
{
    ...
    for (NSInteger i = 0; i < [source length]; i++)
    {
        ...
        NSString *pinyin = [[InfoViewTitle sureAtInstance] redDoing:word];
    ...
    NameSquareMeasureUnit *unit = [[NameSquareMeasureUnit alloc]init];
