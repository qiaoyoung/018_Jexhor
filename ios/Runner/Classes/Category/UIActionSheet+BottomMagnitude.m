...
// __CLOSE_PRINT__
//
//  UIActionSheet+BottomMagnitude.m
//  eim_iphone
//
...
static char notiWhiteValue;


//: @implementation UIActionSheet (NTESBlock)
@implementation UIActionSheet (BottomMagnitude)


//: - (void)showInView: (UIView *)view completionHandler: (ActionSheetBlock)block
- (void)search: (UIView *)view containerHandler: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&notiWhiteValue,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        UITabBar *tabbar = [self paragraph];
        //: if (tabbar)
        if (tabbar)
        {
        ...
        else
        {
            ...
            [self performSelector:@selector(afterLast:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &notiWhiteValue) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&notiWhiteValue,nil,OBJC_ASSOCIATION_COPY);
