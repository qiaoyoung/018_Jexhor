...
NSString *const showTitleToValue = @"net_state";

//: NSString *const NTESSubscribeOnlineState = @"online_state";
NSString *const user_indexName = @"online_state";
...
@interface ColorTingManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
@implementation ColorTingManager

//: + (instancetype)sharedManager
+ (instancetype)message
{
    //: static NTESSubscribeManager *instance;
    static ColorTingManager *instance;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSubscribeManager alloc] init];
        instance = [[ColorTingManager alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)pressedFor:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self button];
...
- (void)range
{
    ...
    NSDictionary *ext = @{
                            ...
                            showTitleToValue : @([FrameForwardSend user].typeLiveEspionageNetwork),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            user_indexName : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext color]];
...
- (void)with
{
    ...
    [_subscribeIds addObjectsFromArray:self.imageFor.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.editBubbleShould.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self name:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)name:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.imageFor containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.editBubbleShould containsObject:recentSession.session.sessionId]) {
