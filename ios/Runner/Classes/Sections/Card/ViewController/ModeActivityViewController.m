...
@interface ModeActivityViewController ()

//: @property (nonatomic,strong) NSString *userId;
@property (nonatomic,strong) NSString *userId;

//: @property (nonatomic,strong) NTESColorButton *chatButton;
@property (nonatomic,strong) TotalTitleButton *chatButton;

//: @end
@end

//: @implementation NTESRobotCardViewController
@implementation ModeActivityViewController

//: - (instancetype)initWithUserId:(NSString *)userId
- (instancetype)initWithStreetwise:(NSString *)userId
{
...
- (void)viewDidLoad {
    ...
    self.chatButton = [[TotalTitleButton alloc] initWithFrame:CGRectZero];
...
//: - (void)chat:(id)sender
- (void)occurrenceChat:(id)sender
{
    //: NIMSession *session = [NIMSession session:self.userId type:NIMSessionTypeP2P];
    NIMSession *session = [NIMSession session:self.userId type:NIMSessionTypeP2P];
    //: NTESSessionViewController *vc = [[NTESSessionViewController alloc] initWithSession:session];
    DisplayAccountViewController *vc = [[DisplayAccountViewController alloc] initWithSession:session];
