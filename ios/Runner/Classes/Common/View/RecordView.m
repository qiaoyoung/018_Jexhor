...
@interface InningWithData : NSObject

@end

@implementation InningWithData

+ (Byte *)InningWithDataToCache:(Byte *)data {
    int sting = data[0];
    Byte sessionNeed = data[1];
    int appliedScience = data[2];
    for (int i = appliedScience; i < appliedScience + sting; i++) {
        int value = data[i] - sessionNeed;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[appliedScience + sting] = 0;
    return data + appliedScience;
}

+ (NSString *)StringFromInningWithData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self InningWithDataToCache:data]];
}

//: loading_%zd
+ (NSString *)appRoperData {
    /* static */ NSString *appRoperData = nil;
    if (!appRoperData) {
        Byte value[] = {11, 97, 9, 63, 124, 99, 153, 49, 166, 205, 208, 194, 197, 202, 207, 200, 192, 134, 219, 197, 7};
        appRoperData = [self StringFromInningWithData:value];
    }
    return appRoperData;
}
...
+ (RecordView *)blue:(UIView *)view status:(BOOL)animated
{
    ...
    RecordView *notice = [[RecordView alloc] init];
    ...
    [view addSubview:notice];
        ...
        for (NSInteger i = 0; i<30; i++) {
            ...
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:[InningWithData appRoperData],i]];
