...
#import "UINavigationBar+DoingButton.h"
...
@implementation UINavigationBar (DoingButton)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(sessionColor), @selector(possibilityDate));
    //: });
    });
}
...
- (void)possibilityDate{
    //: [self swizzling_layoutSubviews];
    [self possibilityDate];
