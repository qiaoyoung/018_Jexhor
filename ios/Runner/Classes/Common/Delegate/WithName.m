...
Byte m_illegalData[] = {94, 20, 81, 6, 44, 138, 21, 30, 33, 17, 24, 19, 248, 29, 35, 20, 33, 16, 18, 35, 24, 37, 20, 255, 30, 31, 218};

//: useClearBar
Byte noti_youthValue[] = {76, 11, 20, 4, 97, 95, 81, 47, 88, 81, 77, 94, 46, 77, 94, 196};
...
@interface WithName()<UIGestureRecognizerDelegate,SystemDelegate>
...
//        [navigationController.view addGestureRecognizer:_recognizer];
        ...
        _animator = [[TitleAnimator alloc] initWithBetween:navigationController];
...
- (void)enable:(UIPanGestureRecognizer*)recognizer
{
                                                           ...
                                                           toViewController:(UIViewController *)toVC
{
    ...
    BOOL cross = [self underbelly:fromVC] || [self underbelly:toVC];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    BOOL forbid = [self barEnablePopOut:self.navigationController.topViewController];
...
- (CAGradientLayer *)uiPopShadow
{
    ...
    if (!_uiPopShadow) {
        ...
        _uiPopShadow.frame = CGRectMake(-6, 0, 6, [ManagerBarController can].view.frame.size.height);
...
- (void)shows:(TitleAnimator *)animator
{
...
- (void)taped:(TitleAnimator *)animator
{
    //: self.isAnimating = NO;
    self.isAnimating = NO;
}
...
- (BOOL)underbelly:(UIViewController *)vc
{
    //: SEL sel = NSSelectorFromString(@"useClearBar");
    SEL sel = NSSelectorFromString(StringFromNaturalEventData(noti_youthValue));
    //: BOOL use = NO;
    BOOL use = NO;
...
- (BOOL)barEnablePopOut:(UIViewController *)vc{
    //: SEL sel = NSSelectorFromString(@"forbidInteractivePop");
    SEL sel = NSSelectorFromString(StringFromNaturalEventData(m_illegalData));
    //: BOOL use = NO;
    BOOL use = NO;
    //: if ([vc respondsToSelector:sel]) {
    if ([vc respondsToSelector:sel]) {
...
Byte * NaturalEventDataToCache(Byte *data) {
    int itemShower = data[0];
    int aloneToss = data[1];
    Byte zine = data[2];
    int acheThreeEmergency = data[3];
    if (!itemShower) return data + acheThreeEmergency;
    for (int i = acheThreeEmergency; i < acheThreeEmergency + aloneToss; i++) {
        int value = data[i] + zine;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[acheThreeEmergency + aloneToss] = 0;
    return data + acheThreeEmergency;
}
