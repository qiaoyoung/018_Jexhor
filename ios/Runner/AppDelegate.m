...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [VisitorValidatePause assignRichKnoll];
    [ResetReferenceSelect byGroupTerse];
    NSInteger towardAgent = (NSInteger)[[NSDate date] timeIntervalSince1970];
    if (towardAgent < 12310) {
        [AppDataManager initManager];
    }
    [JadeLoaderInto outsideAdapterBrief];

    // 使用 Reachability 监听网络状态
    Reachability *detectCounter = [Reachability reachabilityForInternetConnection];
    [detectCounter startNotifier];
    if ([detectCounter currentReachabilityStatus] != NotReachable) {
        [self all];
...
- (BOOL)black {
    ...
    NSArray *fromTinyModels = @[[RaftData sharedInstance].mainBronzeValue, [RaftData sharedInstance].m_sureValue,
                            //: @"tiktok://", @"snapchat://",
    ...
    for (NSString *model in fromTinyModels) {
        //: NSURL *uri = [NSURL URLWithString:scheme];
        NSURL *uri = [NSURL URLWithString:model];
