...
@property (nonatomic,weak) id<VideoActivityDelegate> delegate;
                         ...
                         range:(id<LineConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notText:) name:UIMenuControllerDidHideMenuNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(recented:) name:noti_keyTitleContent object:nil];
    }
    //: return self;
    return self;
}
...
- (void)shouldName
{
    ...
    self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice heightShowStop]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice heightShowStop])-self.inputView.height);
    //: self.tableView.bottom = self.inputView.nim_top;
    self.tableView.bottom = self.inputView.nim_top;
    //: [self.tableView nim_scrollToBottom:YES];
    [self.tableView thanBackground:YES];
...
- (void)ting
{
    ...
    [self.refreshControl addTarget:self action:@selector(progressRereshing:) forControlEvents:UIControlEventValueChanged];

}

//: - (void)headerRereshing:(id)sender
- (void)progressRereshing:(id)sender
{
    //: if ([self.delegate respondsToSelector:@selector(onRefresh)])
    if ([self.delegate respondsToSelector:@selector(changeSession)])
    {
        //: [self.delegate onRefresh];
        [self.delegate changeSession];
    }
}

//: - (void)insert:(NSArray<NSIndexPath *> *)indexPaths animated:(BOOL)animated
- (void)saveMessage:(NSArray<NSIndexPath *> *)indexPaths buildTing:(BOOL)animated
{
    //: if (!indexPaths.count)
    if (!indexPaths.count)
    {
...
//        }];
    ...
    if (isScroll || [self doing:addIndexPathes])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
    }
...
- (void)nameTeam:(NSArray<NSIndexPath *> *)indexPaths
{
    //: if ([self shouldReloadWhenRemoveOrUpdate:indexPaths])
    if ([self after:indexPaths])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
        //: return;
...
- (void)collectionText {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.inputView.replyedContent.hidden)
        {
            ...
            [self.inputView.replyedContent name];
        }
    //: });
    });
}
...
- (BOOL)doing:(NSArray<NSIndexPath *> *)indexPaths
{
...
- (BOOL)after:(NSArray<NSIndexPath *> *)indexPaths
{
    //: for(NSIndexPath * indexPath in indexPaths)
    for(NSIndexPath * indexPath in indexPaths)
    {
