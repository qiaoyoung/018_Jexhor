...
//: @interface WorkSessionDataSourceImpl()
@interface TingCell()

//: @property (nonatomic,strong) NIMSession *session;
@property (nonatomic,strong) NIMSession *session;

//: @property (nonatomic,strong) WorkSessionMsgDatasource *dataSource;
@property (nonatomic,strong) TingClean *dataSource;
...
//: @implementation WorkSessionDataSourceImpl
@implementation TingCell
                         ...
                         voiceSession:(id<LineConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        _pendingMessages = [[NSMutableArray alloc] init];
        //: _dataSource = [[WorkSessionMsgDatasource alloc] initWithSession:_session config:_sessionConfig];
        _dataSource = [[TingClean alloc] initWithNascence:_session container:_sessionConfig];
    }
    //: return self;
    return self;
}
...
- (InfoChange *)dismiss:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource tagComplete:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    InfoChange *result = [[InfoChange alloc] init];
...
- (InfoChange *)quantityryColor:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource tool:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    InfoChange *result = [[InfoChange alloc] init];
    //: result.indexpaths = indexpaths;
    result.indexpaths = indexpaths;
...
- (InfoChange *)blueModel:(ShowModel *)model
{
    ...
    NSArray *indexs = [self.dataSource shadeMargin:model];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    InfoChange *result = [[InfoChange alloc] init];
...
- (InfoChange *)inputModel:(ShowModel *)model
{
    ...
    NSInteger index = [self.dataSource casket:model];
...
- (NSInteger)show:(ShowModel *)model
{
    ...
    return [self.dataSource casket:model];
}
...
- (NSArray *)value:(NSRange)range
{
    ...
    return [self.dataSource title:range];
}
...
- (ShowModel *)model:(NIMMessage *)message{
    ...
    ShowModel *model;
    //: for (WorkMessageModel *item in self.dataSource.items.reverseObjectEnumerator.allObjects) {
    for (ShowModel *item in self.dataSource.items.reverseObjectEnumerator.allObjects) {
        //: if ([item isKindOfClass:[WorkMessageModel class]] && [item.message.messageId isEqual:message.messageId]) {
        if ([item isKindOfClass:[ShowModel class]] && [item.message.messageId isEqual:message.messageId]) {
            //: model = item;
            model = item;
...
- (void)remoteWith:(void (^)(NSError *))handler
{
    ...
    [self.dataSource marginOf:handler];
}
...
- (void)callback:(NIMMessage *)message viewCallback:(void (^)(NSError *))handler
{
    ...
    [self.dataSource menu:message cut:handler];
}
...
- (void)shareWith:(NIMMessage *)message number:(void (^)(NSError *))handler
{
    ...
    [self.dataSource minorLeagueTeam:message change:handler];
}
        ...
        if ([item isKindOfClass:[ShowModel class]]) {
            ...
            if (message.isOutgoingMsg) {
                ...
                if (!findLastReceipt) {
                    ...
                    if (message.isRemoteRead && hasConfig && [self.sessionConfig completed:message])
                    {
...
- (NSDictionary *)name:(NSArray<NIMMessageReceipt *> *)receipts
{
        ...
        if ([item isKindOfClass:[ShowModel class]])
        {
            ...
            if (message.isOutgoingMsg)
            {
                ...
                if (message.setting.teamReceiptEnabled &&
                    ...
                    [self.sessionConfig completed:message])
                {
                    //: model.shouldShowReadLabel = YES;
                    model.shouldShowReadLabel = YES;
                    //: dict[@(i)] = model;
...
- (void)on:(NSArray *)messages
{
    ...
    if ([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive)
    {
        ...
        if (self.session.sessionType == NIMSessionTypeP2P)
        {
            ...
            [self achromaticColour:messages];
        }
...
- (void)achromaticColour:(NSArray *)messages
{
    ...
    for (NSInteger i = [messages count] - 1; i >= 0; i--) {
        ...
        //: else if ([item isKindOfClass:[WorkMessageModel class]])
        else if ([item isKindOfClass:[ShowModel class]])
        {
            //: message = [(WorkMessageModel *)item message];
            message = [(ShowModel *)item message];
        }
        ...
        if (message)
        {
            ...
            if (!message.isOutgoingMsg &&
                ...
                [self.sessionConfig respondsToSelector:@selector(completed:)] &&
                //: [self.sessionConfig shouldHandleReceiptForMessage:message])
                [self.sessionConfig completed:message])
            {
