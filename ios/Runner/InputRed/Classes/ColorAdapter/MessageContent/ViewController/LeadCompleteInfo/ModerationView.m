...
@interface ModerationView () <WayCount>
...
@property (nonatomic, strong) NSMutableArray<NumberView*> *items;
...
@property (nonatomic, strong) ToeView *touchReceiver;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _timeIntervalPerScroll = mItemLinkData;
        //: _timeDurationPerScroll = DEFAULT_TIME_DURATION;
        _timeDurationPerScroll = show_sessionData;
        //: _scrollSpeed = DEFAULT_SCROLL_SPEED;
        _scrollSpeed = dreamEventName;
        //: _itemSpacing = DEFAULT_ITEM_SPACING;
        _itemSpacing = user_managerData;
        //: _touchEnabled = NO;
        _touchEnabled = NO;
...
- (void)fielded:(NSNotification*)notification {
...
- (void)streetwise {
    ...
    //: } else {
        ...
        if ([_delegate respondsToSelector:@selector(imageView:)]) {
            //: self.visibleItemCount = [_delegate numberOfVisibleItemsForMarqueeView:self];
            self.visibleItemCount = [_delegate imageView:self];
        ...
        //: } else {
            ...
            self.visibleItemCount = kTextName;
        }
    }
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        ...
        NumberView *itemView = [[NumberView alloc] init];
    ...
    if (_direction == UUMarqueeViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            ...
            //: } else {
                ...
                _items[index].width = [self searched:_items[index].tag];
                ...
                [self top:_items[index] quantityIndex:_items[index].tag];
            }
        }
    //: } else {
    } else {
        ...
        if (_useDynamicHeight) {
            ...
            for (int i = 0; i < _items.count; i++) {
                ...
                if (i == _items.count - 1) {
                    ...
                    _items[index].height = [self white:_items[index].tag];
        ...
        //: } else {
            ...
            if ([_delegate respondsToSelector:@selector(beforeShow:)]) {
                //: dataCount = [_delegate numberOfDataForMarqueeView:self];
                dataCount = [_delegate beforeShow:self];
            }
...
- (CGFloat)searched:(NSInteger)index {
    //: CGFloat itemWidth = 0.0f;
    CGFloat itemWidth = 0.0f;
    //: if (index >= 0) {
    if (index >= 0) {
        //: if ([_delegate respondsToSelector:@selector(itemViewWidthAtIndex:forMarqueeView:)]) {
        if ([_delegate respondsToSelector:@selector(messageView:keyWithView:)]) {
            //: itemWidth = [_delegate itemViewWidthAtIndex:index forMarqueeView:self];
            itemWidth = [_delegate messageView:index keyWithView:self];
        }
    }
    //: return itemWidth;
    return itemWidth;
...
- (void)everyLimit:(NumberView*)itemView {
    //: if (!itemView.didFinishCreate) {
    if (!itemView.didFinishCreate) {
        //: if ([_delegate respondsToSelector:@selector(createItemView:forMarqueeView:)]) {
        if ([_delegate respondsToSelector:@selector(createButtonForefront:viewColor:)]) {
            //: [_delegate createItemView:itemView forMarqueeView:self];
            [_delegate createButtonForefront:itemView viewColor:self];
...
- (void)top:(NumberView*)itemView quantityIndex:(NSInteger)index {
    //: if (index < 0) {
    if (index < 0) {
        //: [itemView clear];
        [itemView total];
    ...
    if (index >= 0) {
        ...
        if ([_delegate respondsToSelector:@selector(companyContent:sum:display:)]) {
            //: [_delegate updateItemView:itemView atIndex:index forMarqueeView:self];
            [_delegate companyContent:itemView sum:index display:self];
        }
    }
}
...
- (void)elited:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        ...
        if (_direction == UUMarqueeViewDirectionLeftward) {
            ...
            _items[_firstItemIndex].width = [self searched:_items[_firstItemIndex].tag];
            ...
            if (firstItemWidth != nextItemWidth) {
                ...
                [_items[_firstItemIndex] total];
            }
            //: [self updateItemView:_items[_firstItemIndex] atIndex:_items[_firstItemIndex].tag];
            [self top:_items[_firstItemIndex] quantityIndex:_items[_firstItemIndex].tag];
        ...
        //: } else {
            ...
            if (_useDynamicHeight) {
                ...
                _items[_firstItemIndex].height = [self white:_items[_firstItemIndex].tag];
...
- (void)moreTouch {
    ...
    if (_touchEnabled) {
        ...
        if (!_touchReceiver) {
            ...
            self.touchReceiver = [[ToeView alloc] init];
...
- (void)element:(CGPoint)point {
    ...
    for (NumberView *itemView in _items) {
        ...
        if ([itemView.layer.presentationLayer hitTest:point]) {
            ...
            if (dataCount > 0 && itemView.tag >= 0 && itemView.tag < dataCount) {
                ...
                if ([self.delegate respondsToSelector:@selector(colorView:erase:)]) {
                    //: [self.delegate didTouchItemViewAtIndex:itemView.tag forMarqueeView:self];
                    [self.delegate colorView:itemView.tag erase:self];
                }
            }
            //: break;
            break;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchDelegate) {
        ...
        [_touchDelegate element:touchLocation];
    }
}
