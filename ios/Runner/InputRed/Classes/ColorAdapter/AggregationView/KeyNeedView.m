...
- (NSInteger)bound:(NSInteger)value message:(NSInteger)min inward:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)viewLive:(NSInteger)currentPage
{
    ...
    NSInteger first = [self bound:currentPage - 1 message:0 inward:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self bound:currentPage + 1 message:0 inward:count - 1];
...
- (void)barnburner
{
    ...
    [self viewLive:_currentPage];
    //: [self raisePageIndexChangedDelegate];
    [self graduatedTableDelegate];
    //: [self setNeedsLayout];
    [self setNeedsLayout];
}
