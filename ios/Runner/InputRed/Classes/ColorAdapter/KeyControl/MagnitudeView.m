...
Byte user_againstContent[] = {66, 26, 12, 22, 6, 2, 71, 165, 127, 34, 38, 244, 100, 110, 101, 115, 95, 121, 116, 105, 118, 105, 116, 118, 97, 95, 101, 108, 105, 102, 111, 114, 112, 95, 114, 101, 115, 117, 237};

//: #8A8E98
Byte dream_unitData[] = {73, 7, 5, 241, 115, 56, 57, 69, 56, 65, 56, 35, 189};

//: #05D481
Byte app_lengthValue[] = {67, 7, 13, 233, 55, 140, 230, 18, 29, 133, 13, 251, 19, 49, 56, 52, 68, 53, 48, 35, 136};
...
@interface MagnitudeView ()<RecordNeedMessage,UIGestureRecognizerDelegate>
...
- (void)imaging:(UIPanGestureRecognizer *)gesture
{
...
- (void)initDisable {
    ...
    _panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(imaging:)];
    ...
    _line.backgroundColor = [UIColor ground:StringFromAmbitionData(dream_unitData)];
    ...
    _albumPickerView = [[RfView alloc] initWithFrame:CGRectMake(20, 30, [[UIScreen mainScreen] bounds].size.width-40, _minHeight-120)];
...
- (UIButton *)confirmButton
{
    ...
    if (!_confirmButton) {
        ...
        _confirmButton.backgroundColor = [UIColor ground:StringFromAmbitionData(app_lengthValue)];
...
- (void)tingValue {
    ...
    if ([self.delegate respondsToSelector:@selector(tipCamera)]) {
        //: [self.delegate CustomPickerDidSelectCamera];
        [self.delegate tipCamera];
    }
}
...
- (void)saveFrame {
    ...
    if ([self.delegate respondsToSelector:@selector(sited:)]) {
        //: [self.delegate CustomPickerDidSelectAssets:self.selectedPhoto];
        [self.delegate sited:self.selectedPhoto];
    }
}
...
Byte * AmbitionDataToCache(Byte *data) {
    int showerheadAnyway = data[0];
    int grateQuick = data[1];
    int textFederal = data[2];
    if (!showerheadAnyway) return data + textFederal;
    for (int i = 0; i < grateQuick / 2; i++) {
        int begin = textFederal + i;
        int end = textFederal + grateQuick - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[textFederal + grateQuick] = 0;
    return data + textFederal;
}

NSString *StringFromAmbitionData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)AmbitionDataToCache(data)];
}  
