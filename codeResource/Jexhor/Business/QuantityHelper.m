...
@interface AnywayData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation AnywayData

+ (instancetype)sharedInstance {
    static AnywayData *instance = nil;
...
- (Byte *)AnywayDataToCache:(Byte *)data {
    int uniteRoper = data[0];
    int promoteIts = data[1];
    for (int i = 0; i < uniteRoper / 2; i++) {
        int begin = promoteIts + i;
        int end = promoteIts + uniteRoper - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[promoteIts + uniteRoper] = 0;
    return data + promoteIts;
}

- (NSString *)StringFromAnywayData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self AnywayDataToCache:data]];
}  

//: colors
- (NSString *)notiEmotionSuspectValue {
    /* static */ NSString *notiEmotionSuspectValue = nil;
    if (!notiEmotionSuspectValue) {
        Byte value[] = {6, 5, 111, 15, 102, 115, 114, 111, 108, 111, 99, 125};
        notiEmotionSuspectValue = [self StringFromAnywayData:value];
    }
    return notiEmotionSuspectValue;
}

//: chromateAnimate
- (NSString *)show_injureTableData {
    /* static */ NSString *show_injureTableData = nil;
    if (!show_injureTableData) {
        Byte value[] = {15, 11, 82, 155, 86, 149, 69, 241, 125, 39, 193, 101, 116, 97, 109, 105, 110, 65, 101, 116, 97, 109, 111, 114, 104, 99, 62};
        show_injureTableData = [self StringFromAnywayData:value];
    }
    return show_injureTableData;
}
...
#import "QuantityHelper.h"

//: @implementation SNGradientHelper
@implementation QuantityHelper
...
+ (UIImage *)image:(UIColor *)startColor get:(UIColor *)endColor show:(SNGradientDirection)directionType {
    ...
    return [self notCorner:startColor stateOption:endColor setApartItem:directionType m:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice heightShowStop]))];
}
...
+ (UIImage *)gen:(UIColor *)centerColor state:(UIColor *)outColor {
    ...
    return [self item:centerColor someImage:outColor send:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice heightShowStop]))];
}
...
+ (UIImage *)item:(UIColor *)centerColor someImage:(UIColor *)outColor send:(CGSize)size {
    ...
    [self duringPath:gc indexEvent:path min:centerColor.CGColor dayPull:outColor.CGColor];
    //: CGPathRelease(path);
    CGPathRelease(path);
...
+ (void)duringPath:(CGContextRef)context
                      //: path:(CGPathRef)path
                      indexEvent:(CGPathRef)path
                //: startColor:(CGColorRef)startColor
                min:(CGColorRef)startColor
                  //: endColor:(CGColorRef)endColor {
                  dayPull:(CGColorRef)endColor {
...
+ (void)up:(UIView *)view {
    ...
    CAKeyframeAnimation *chromateAnimate = [self outside];

    //: [chromatoLayer addAnimation:chromateAnimate forKey:@"chromateAnimate"];
    [chromatoLayer addAnimation:chromateAnimate forKey:[[AnywayData sharedInstance] show_injureTableData]];
...
+ (CAKeyframeAnimation *)outside {
    //: CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:@"colors"];
    CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:[[AnywayData sharedInstance] notiEmotionSuspectValue]];
