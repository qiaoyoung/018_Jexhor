...
//: Chartlet
@property (nonatomic, copy) NSString *notiAnnuallyText;

//: default
@property (nonatomic, copy) NSString *kCrushRollingTitle;

//: content
@property (nonatomic, copy) NSString *mQuickPressData;
...
+ (instancetype)sharedInstance {
    static CharacterData *instance = nil;
...
- (Byte *)CharacterDataToCache:(Byte *)data {
    int onBroCircuit = data[0];
    Byte attack = data[1];
    int sessionEarth = data[2];
    for (int i = sessionEarth; i < sessionEarth + onBroCircuit; i++) {
        int value = data[i] - attack;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[sessionEarth + onBroCircuit] = 0;
    return data + sessionEarth;
}
...
//: default
- (NSString *)kCrushRollingTitle {
    if (!_kCrushRollingTitle) {
        Byte value[] = {7, 50, 9, 74, 168, 222, 80, 69, 186, 150, 151, 152, 147, 167, 158, 166, 250};
        _kCrushRollingTitle = [self StringFromCharacterData:value];
    }
    return _kCrushRollingTitle;
}

//: Chartlet
- (NSString *)notiAnnuallyText {
    if (!_notiAnnuallyText) {
        Byte value[] = {8, 42, 7, 13, 61, 61, 70, 109, 146, 139, 156, 158, 150, 143, 158, 186};
        _notiAnnuallyText = [self StringFromCharacterData:value];
    }
    return _notiAnnuallyText;
}

//: content
- (NSString *)mQuickPressData {
    if (!_mQuickPressData) {
        Byte value[] = {7, 28, 8, 194, 24, 159, 187, 6, 127, 139, 138, 144, 129, 138, 144, 227};
        _mQuickPressData = [self StringFromCharacterData:value];
    }
    return _mQuickPressData;
}
...
@implementation UIImage (MessageContent)

//: + (UIImage *)nim_fetchChartlet:(NSString *)imageName chartletId:(NSString *)chartletId{
+ (UIImage *)name:(NSString *)imageName color:(NSString *)chartletId{
    //: if ([chartletId isEqualToString:@"default"]) {
    if ([chartletId isEqualToString:[CharacterData sharedInstance].kCrushRollingTitle]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",[CharacterData sharedInstance].notiAnnuallyText,chartletId,[CharacterData sharedInstance].mQuickPressData];
    //先拿2倍图
...
- (UIImage *)forbid: (CGFloat)suggestPixels
{
...
- (UIImage *)high: (CGFloat)maxPixels
{
    ...
    return [self child:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)nim_scaleToSize:(CGSize)newSize
- (UIImage *)child:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
