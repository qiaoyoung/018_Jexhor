...
- (instancetype) initWithMessage:(NSArray *(^)(void))data{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    LigatureSection *tableSection = self.button[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LigatureSection *tableSection = self.button[indexPath.section];
    //: NIMCommonTableRow *tableRow = tableSection.rows[indexPath.row];
    ConstituentRow *tableRow = tableSection.rows[indexPath.row];
    //: NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : DefaultTableCell;
    NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : showPageText;
    ...
    if (![cell respondsToSelector:@selector(demonstrate:gray:)]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!tableRow.forbidSelect) {
        ...
        UIViewController *vc = tableView.styleController;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //: }else{
        ...
        LigatureSection *section = self.button[indexPath.section];
...
- (void)stochastic:(ConstituentRow *)rowData sum:(UITableViewCell *)cell{
