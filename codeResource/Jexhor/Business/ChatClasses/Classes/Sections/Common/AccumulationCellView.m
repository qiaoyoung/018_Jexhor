...
+ (instancetype)tagTip:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self magnitudeContext:rect content:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self ting:rect present:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self messageOf:badgeValue];
...
- (CGSize)userStr:(NSString *)badgeValue{
...
- (CGRect)messageOf:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self userStr:badgeValue];
...
- (void)magnitudeContext:(CGRect)rect content:(CGContextRef)context{
...
- (void)ting:(CGRect)rect present:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
