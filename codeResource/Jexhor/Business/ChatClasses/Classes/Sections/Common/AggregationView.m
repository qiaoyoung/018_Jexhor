...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self table:rect color:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self title:rect message:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self styleStr:badgeValue];
...
- (CGSize)read:(NSString *)badgeValue{
...
- (CGRect)styleStr:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self read:badgeValue];
...
- (void)table:(CGRect)rect color:(CGContextRef)context{
...
- (void)title:(CGRect)rect message:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
