...
- (NSInteger)icon:(NSInteger)value standard:(NSInteger)min max:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)loadPathPage:(NSInteger)currentPage
{
    ...
    NSInteger first = [self icon:currentPage - 1 standard:0 max:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self icon:currentPage + 1 standard:0 max:count - 1];
...
- (void)inputPost
{
    ...
    [self loadPathPage:_currentPage];
    //: [self raisePageIndexChangedDelegate];
    [self incentiveStockOption];
    //: [self setNeedsLayout];
    [self setNeedsLayout];
}
