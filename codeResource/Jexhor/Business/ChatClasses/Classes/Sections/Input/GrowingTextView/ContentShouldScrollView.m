...
- (CGSize)intrinsicContentSize
{
    ...
    return [self specs:self.want].size;
}
...
- (void)setMinNumberOfLines:(NSInteger)minNumberOfLines
{
    ...
    self.minHeight = [self center:minNumberOfLines];
...
- (void)setMaxNumberOfLines:(NSInteger)maxNumberOfLines
{
    ...
    self.maxHeight = [self center:maxNumberOfLines];
...
- (CGFloat)center:(NSInteger)line
{
    ...
    CGFloat height = self.want.height - (textViewMargin + self.textView.contentInset.top + self.textView.contentInset.bottom);
...
- (void)appStatus{
    ...
    CGSize actualTextViewSize = [self want];
    ...
    if (newScrollViewFrame.size.height <= self.maxHeight && _previousTextHeight == 0) {
        ...
        if(oldScrollViewFrame.size.height != newScrollViewFrame.size.height) {
            ...
            if ([self.textViewDelegate respondsToSelector:@selector(dimensions:)]) {
                //: [self.textViewDelegate willChangeHeight:newScrollViewFrame.size.height];
                [self.textViewDelegate dimensions:newScrollViewFrame.size.height];
            }
    ...
    if (oldScrollViewFrame.size.height != newScrollViewFrame.size.height && [self.textViewDelegate respondsToSelector:@selector(keys:)]) {
        //: [self.textViewDelegate didChangeHeight:newScrollViewFrame.size.height];
        [self.textViewDelegate keys:newScrollViewFrame.size.height];
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(text:table:)]) {
        //: return [self.textViewDelegate shouldChangeTextInRange:range replacementText:text];
        return [self.textViewDelegate text:range table:text];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(should:total:)]) {
        //: return [self.textViewDelegate shouldInteractWithURL:URL inRange:characterRange];
        return [self.textViewDelegate should:URL total:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithTextAttachment:(NSTextAttachment *)textAttachment inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(messageInfo:squeeze:)]) {
        //: return [self.textViewDelegate shouldInteractWithTextAttachment:textAttachment inRange:characterRange];
        return [self.textViewDelegate messageInfo:textAttachment squeeze:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(nearTouch:)]) {
        //: [self.textViewDelegate textViewDidBeginEditing:self];
        [self.textViewDelegate nearTouch:self];
    }
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(titled:)]) {
        //: [self.textViewDelegate textViewDidChangeSelection:self];
        [self.textViewDelegate titled:self];
    }
}
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(nameBegin:)]) {
        //: [self.textViewDelegate textViewDidEndEditing:self];
        [self.textViewDelegate nameBegin:self];
    }
}
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(wordsBack:)]) {
       //: return [self.textViewDelegate textViewShouldBeginEditing:self];
       return [self.textViewDelegate wordsBack:self];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(cellAcross:)]) {
        //: return [self.textViewDelegate textViewShouldEndEditing:self];
        return [self.textViewDelegate cellAcross:self];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(messageShould:)]) {
        //: [self.textViewDelegate textViewDidChange:self];
        [self.textViewDelegate messageShould:self];
    }
    //: [self fitToScrollView];
    [self appStatus];
}
