...
//: @interface WorkGrowingInternalTextView()
@interface RedScrollView()
...
//: @implementation WorkGrowingInternalTextView
@implementation RedScrollView
...
- (instancetype)initWithFrame:(CGRect)frame textContainer:(NSTextContainer *)textContainer
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(withValue:) name:UITextViewTextDidChangeNotification object:self];
    }
    //: return self;
    return self;
}
...
- (void)withValue:(NSNotification *)notification
{
    //: [self updatePlaceholder];
    [self accumulation];
}
