...
//: @interface PushSessionLayoutImpl()
@interface BoundaryOnly()
{
...
//: @property (nonatomic,strong) id<PushSessionConfig> sessionConfig;
@property (nonatomic,strong) id<CreateTop> sessionConfig;

//: @property (nonatomic,weak) id<NIMSessionLayoutDelegate> delegate;
@property (nonatomic,weak) id<TitleCover> delegate;

//: @end
@end

//: @implementation PushSessionLayoutImpl
@implementation BoundaryOnly
                         ...
                         assemblage:(id<CreateTop>)sessionConfig
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(veilOn:) name:UIMenuControllerDidHideMenuNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ofsing:) name:noti_toFieldFormat object:nil];
    }
    //: return self;
    return self;
}
...
- (void)reconcileDate
{
    ...
    self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice title]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice title])-self.inputView.height);
    //: self.tableView.bottom = self.inputView.nim_top;
    self.tableView.bottom = self.inputView.nim_top;
    //: [self.tableView nim_scrollToBottom:YES];
    [self.tableView record:YES];
...
- (void)ofsing:(NSNotification *)notification
{
...
- (void)need:(PurseModel *)model{
    ...
    if ([NSThread isMainThread]) {^{ [model victoryQuery:self.tableView.nim_width]; }();} else {dispatch_sync(dispatch_get_main_queue(), ^{ [model victoryQuery:self.tableView.nim_width]; });};
...
- (void)item
{
    ...
    [self.refreshControl addTarget:self action:@selector(intervalmateRereshing:) forControlEvents:UIControlEventValueChanged];

}

//: - (void)headerRereshing:(id)sender
- (void)intervalmateRereshing:(id)sender
{
    //: if ([self.delegate respondsToSelector:@selector(onRefresh)])
    if ([self.delegate respondsToSelector:@selector(cellProgress)])
    {
        //: [self.delegate onRefresh];
        [self.delegate cellProgress];
    }
}

//: - (void)insert:(NSArray<NSIndexPath *> *)indexPaths animated:(BOOL)animated
- (void)inputArrayRubricBy:(NSArray<NSIndexPath *> *)indexPaths title:(BOOL)animated
{
    //: if (!indexPaths.count)
    if (!indexPaths.count)
    {
...
//        }];
    ...
    if (isScroll || [self addProduce:addIndexPathes])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
    }
...
- (void)gpaRemove:(NSArray<NSIndexPath *> *)indexPaths
{
    //: if ([self shouldReloadWhenRemoveOrUpdate:indexPaths])
    if ([self lip:indexPaths])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
        //: return;
...
{
    //: PushMessageCell *cell = (PushMessageCell *)[self.tableView cellForRowAtIndexPath:indexPath];
    BroadcastTouchCellView *cell = (BroadcastTouchCellView *)[self.tableView cellForRowAtIndexPath:indexPath];
    //: if (cell) {
    if (cell) {
        //: @try {
        @try {
...
- (void)send {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.inputView.replyedContent.hidden)
        {
            ...
            [self.inputView.replyedContent off];
        }
    //: });
    });
}
...
- (BOOL)addProduce:(NSArray<NSIndexPath *> *)indexPaths
{
...
- (BOOL)lip:(NSArray<NSIndexPath *> *)indexPaths
{
    //: for(NSIndexPath * indexPath in indexPaths)
    for(NSIndexPath * indexPath in indexPaths)
    {
