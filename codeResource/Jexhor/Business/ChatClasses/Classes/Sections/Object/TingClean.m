                         ...
                         container:(id<LineConfig>)sessionConfig
{
    ...
    if (self = [self init]) {
        ...
        NSInteger limit = [MessageContent secretResolution].config.messageLimit;
        //: NSTimeInterval showTimestampInterval = [MyUserKit sharedKit].config.messageInterval;
        NSTimeInterval showTimestampInterval = [MessageContent secretResolution].config.messageInterval;
...
- (void)infoOutside:(void(^)(NSError *error)) handler
{
    ...
    [self input:^(NSError *error, NSArray *models) {
       //: if (handler)
       if (handler)
       {
           //: handler(error);
...
- (void)input:(void(^)(NSError *error, NSArray *))handler
{
    ...
    if ([self.dataProvider respondsToSelector:@selector(language:stepHandler:)])
    {
        //: __weak typeof(self) wself = self;
        __weak typeof(self) wself = self;
        //: [self.dataProvider pullDown:nil handler:^(NSError *error, NSArray<NIMMessage *> *messages) {
        [self.dataProvider language:nil stepHandler:^(NSError *error, NSArray<NIMMessage *> *messages) {
            ...
            if ([NSThread isMainThread]) {^{ NSArray *models = [self embolismUponMessages:messages]; [wself tagComplete:models]; if (handler) { handler(error, models); } }();} else {dispatch_async(dispatch_get_main_queue(), ^{ NSArray *models = [self embolismUponMessages:messages]; [wself tagComplete:models]; if (handler) { handler(error, models); } });};
...
//        [SVProgressHUD dismiss];
        ...
        if (messages.count == 0){
            ...
            [self language:nil stepHandler:^(NSError *error, NSArray<NIMMessage *> *messages) {
                //: NSArray *models = [self modelsWithMessages:messages];
                NSArray *models = [self embolismUponMessages:messages];
                //: [self appendMessageModels:models];
                [self tagComplete:models];
                //: if (handler) {
                if (handler) {
                    //: handler(nil,models);
                    handler(nil,models);
...
- (NSArray *)tagComplete:(NSArray *)models{
    //: if (!models.count) {
    if (!models.count) {
        //: return @[];
        return @[];
    ...
    for (ShowModel *model in models) {
        ...
        NSArray *result = [self size:model scale:self.items.count];
...
- (NSArray *)tool:(NSArray *)models{
    ...
    for (ShowModel *model in sortModels) {
        ...
        NSInteger i = [self isPress:model];
        //: NSArray *result = [self insertMessageModel:model index:i];
        NSArray *result = [self size:model scale:i];
...
- (void)historicPeriod:(void(^)(NSInteger index, NSArray *messages , NSError *error))handler
{
    ...
    if ([self.dataProvider respondsToSelector:@selector(language:stepHandler:)])
    {
        ...
        [self.dataProvider language:currentOldestMsg.message stepHandler:^(NSError *error, NSArray *messages) {
    ...
    else
    {
        ...
        if (messages.count == 0){
            ...
            [self language:currentOldestMsg.message stepHandler:^(NSError *error, NSArray<NIMMessage *> *messages) {
                //: index = [self insertMessages:messages];
                index = [self titleContent:messages];
                //: if (handler) {
                if (handler) {
...
- (void)media:(void (^)(NSInteger, NSArray *, NSError *))handler {
    ...
    option.limit = [MessageContent secretResolution].config.messageLimit;
    ...
    [[NIMSDK sharedSDK].conversationManager searchMessages:_currentSession
                                                    ...
                                                    result:^(NSError * _Nullable error, NSArray<NIMMessage *> * _Nullable messages) {
                                                        ...
                                                        index = [wself tagComplete:[self embolismUponMessages:messages]].count;
                                                        //: if (handler) {
                                                        if (handler) {
                                                            //: if ([NSThread isMainThread]) {^{ handler(index,messages,nil); }();} else {dispatch_async(dispatch_get_main_queue(), ^{ handler(index,messages,nil); });};
                                                            if ([NSThread isMainThread]) {^{ handler(index,messages,nil); }();} else {dispatch_async(dispatch_get_main_queue(), ^{ handler(index,messages,nil); });};
...
- (void)marginOf:(void (^)(NSError *))handler
{
    ...
    [NIMSDK.sharedSDK.chatExtendManager loadMessagePinsForSession:_currentSession completion:^(NSError * _Nonnull error, NSArray<NIMMessagePinItem *> * _Nullable items) {
        ...
        [items enumerateObjectsUsingBlock:^(NIMMessagePinItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            self.pinUsers[obj.messageId] = [MessageContent.secretResolution recent:pinUserID blue:option].showName;
        //: }];
        }];
        //: if (handler) {
        if (handler) {
...
- (NSArray<NSIndexPath *> *)title:(NSRange)range
{
...
- (void)ting:(ShowModel *)model
{
    ...
    if ([_sessionConfig respondsToSelector:@selector(tinkleLabel)]) {
        //: model.shouldShowPinContent = [_sessionConfig shouldShowPinContent];
        model.shouldShowPinContent = [_sessionConfig tinkleLabel];
    }
...
- (void)menu:(NIMMessage *)message cut:(void (^)(NSError *))handler
{
    ...
    NSString *pinUserName = [MessageContent.secretResolution recent:accountID blue:option].showName;
...
- (void)tingObject:(NIMMessage *)message{
    ...
    NSTimeInterval firstTimeInterval = [self lapState];
...
- (NSInteger)originMedia:(NSArray *)array modelOf:(ShowModel *)model
{
    ...
    return [self originMedia:half modelOf:model];
}
