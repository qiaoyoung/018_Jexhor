...
typedef struct {
    Byte countmit;
    Byte *possession;
    unsigned int countmitStorage;
	int findingFood;
	int bondSession;
	int federalLeast;
} StructSackData;

@interface SackData : NSObject

@end

@implementation SackData

+ (Byte *)SackDataToByte:(StructSackData *)data {
    for (int i = 0; i < data->countmitStorage; i++) {
        data->possession[i] ^= data->countmit;
    }
    data->possession[data->countmitStorage] = 0;
	if (data->countmitStorage >= 3) {
		data->findingFood = data->possession[0];
		data->bondSession = data->possession[1];
		data->federalLeast = data->possession[2];
	}
    return data->possession;
}

+ (NSString *)StringFromSackData:(StructSackData *)data {
    return [NSString stringWithUTF8String:(char *)[self SackDataToByte:data]];
}

//: tableButtonCell
+ (NSString *)dream_participantContent {
    /* static */ NSString *dream_participantContent = nil;
    if (!dream_participantContent) {
        StructSackData value = (StructSackData){250, (Byte []){142, 155, 152, 150, 159, 184, 143, 142, 142, 149, 148, 185, 159, 150, 150, 52}, 15, 94, 109, 27};
        dream_participantContent = [self StringFromSackData:&value];
    }
    return dream_participantContent;
}

//: TableSwitch%ld%ld
+ (NSString *)kInformalName {
    /* static */ NSString *kInformalName = nil;
    if (!kInformalName) {
        StructSackData value = (StructSackData){111, (Byte []){59, 14, 13, 3, 10, 60, 24, 6, 27, 12, 7, 74, 3, 11, 74, 3, 11, 168}, 17, 227, 55, 50};
        kInformalName = [self StringFromSackData:&value];
    }
    return kInformalName;
}
...
+ (NSString *)userShowData {
    ...
    if (!userShowData) {
        StructSackData value = (StructSackData){131, (Byte []){224, 236, 237, 247, 226, 224, 247, 220, 247, 226, 228, 220, 229, 241, 226, 228, 238, 230, 237, 247, 220, 224, 226, 237, 224, 230, 239, 151}, 27, 92, 174, 208};
        userShowData = [self StringFromSackData:&value];
    }
    return userShowData;
}

//: personCard_bg
+ (NSString *)dreamMessageBindTitle {
    /* static */ NSString *dreamMessageBindTitle = nil;
    if (!dreamMessageBindTitle) {
        StructSackData value = (StructSackData){237, (Byte []){157, 136, 159, 158, 130, 131, 174, 140, 159, 137, 178, 143, 138, 45}, 13, 174, 65, 239};
        dreamMessageBindTitle = [self StringFromSackData:&value];
    }
    return dreamMessageBindTitle;
}

//: tableMemberCell
+ (NSString *)notiUnitText {
    /* static */ NSString *notiUnitText = nil;
    if (!notiUnitText) {
        StructSackData value = (StructSackData){195, (Byte []){183, 162, 161, 175, 166, 142, 166, 174, 161, 166, 177, 128, 166, 175, 175, 27}, 15, 70, 74, 82};
        notiUnitText = [self StringFromSackData:&value];
    }
    return notiUnitText;
}

//: back_arrow_bl
+ (NSString *)notiExpectName {
    /* static */ NSString *notiExpectName = nil;
    if (!notiExpectName) {
        StructSackData value = (StructSackData){123, (Byte []){25, 26, 24, 16, 36, 26, 9, 9, 20, 12, 36, 25, 23, 136}, 13, 91, 19, 100};
        notiExpectName = [self StringFromSackData:&value];
    }
    return notiExpectName;
}

//: #2C3042
+ (NSString *)dream_economicsName {
    /* static */ NSString *dream_economicsName = nil;
    if (!dream_economicsName) {
        StructSackData value = (StructSackData){53, (Byte []){22, 7, 118, 6, 5, 1, 7, 74}, 7, 56, 204, 77};
        dream_economicsName = [self StringFromSackData:&value];
    }
    return dream_economicsName;
}

//: tableButtonCell%ld%ld
+ (NSString *)main_sterolName {
    /* static */ NSString *main_sterolName = nil;
    if (!main_sterolName) {
        StructSackData value = (StructSackData){105, (Byte []){29, 8, 11, 5, 12, 43, 28, 29, 29, 6, 7, 42, 12, 5, 5, 76, 5, 13, 76, 5, 13, 26}, 21, 121, 121, 90};
        main_sterolName = [self StringFromSackData:&value];
    }
    return main_sterolName;
}

//: #F6F7FA
+ (NSString *)showWhipName {
    /* static */ NSString *showWhipName = nil;
    if (!showWhipName) {
        StructSackData value = (StructSackData){149, (Byte []){182, 211, 163, 211, 162, 211, 212, 72}, 7, 10, 25, 233};
        showWhipName = [self StringFromSackData:&value];
    }
    return showWhipName;
}

//: builidCommonCell%ld%ld
+ (NSString *)notiAbuseText {
    /* static */ NSString *notiAbuseText = nil;
    if (!notiAbuseText) {
        StructSackData value = (StructSackData){12, (Byte []){110, 121, 101, 96, 101, 104, 79, 99, 97, 97, 99, 98, 79, 105, 96, 96, 41, 96, 104, 41, 96, 104, 205}, 22, 99, 92, 61};
        notiAbuseText = [self StringFromSackData:&value];
    }
    return notiAbuseText;
}
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor ground:[SackData showWhipName]];
    ...
    Bg.image = [UIImage imageNamed:[SackData dreamMessageBindTitle]];
    ...
    backButton.frame = CGRectMake(5, [UIDevice heightShowStop], 40, 40);
    //: [backButton setImage:[UIImage imageNamed:@"back_arrow_bl"] forState:(UIControlStateNormal)];
    [backButton setImage:[UIImage imageNamed:[SackData notiExpectName]] forState:(UIControlStateNormal)];
...
- (id<NTESCardBodyData>)temp:(NSIndexPath*)indexpath{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    id<NTESCardBodyData> bodyData = [self temp:indexPath];
...
- (UITableViewCell*)viewKey:(id<NTESCardBodyData>) bodyData progress:(NSIndexPath *)indexPath
{
...
- (UITableViewCell*)end:(id<NTESCardBodyData>) bodyData builidWithSunnaPath:(NSIndexPath *)indexPath{
//    tableButtonCell
...
- (UITableViewCell *)image:(id<NTESCardBodyData>)bodyData orderedSeries:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor = [UIColor ground:[SackData dream_economicsName]];
    //: cell.imageView.image = bodyData.img;
    cell.imageView.image = bodyData.img;
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(15, (44.0f + [UIDevice heightShowStop]), [[UIScreen mainScreen] bounds].size.width-30, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice heightShowStop])) style:UITableViewStyleGrouped];
