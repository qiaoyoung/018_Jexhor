 ...
 *************************简要说明************************
 ...
 [BubbleAlert configMainWindow:self.window];
 
 
 Should 使用方法
 
 [BubbleAlert alert].cofing.XXXXX.XXXXX.LeeShow();
 
 Model 使用方法
 
 [BubbleAlert actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface BubbleAlert: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull SelectedBaseConfig *)picture;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull TotalervalBaseConfig *)find;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull WindowView *)value;
...
+ (void)force:(NSString *)identifier completion:(BOOL)force press:(void (^ _Nullable)(void))completionBlock;
...
+ (void)withing:(void (^ _Nullable)(void))completionBlock;
...
/** 设置 添加自定义视图 -> 格式: .LeeAddCustomView(^(ConcealedRandom *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;
@property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;

/** 设置 添加一项 -> 格式: .LeeAddItem(^(SimplyLayer *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;
@property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;

/** 设置 添加动作 -> 格式: .LeeAddAction(^(TitleCenterInput *){ //code.. }) */
...
@interface MemoryModel(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([ColorSession windowLevel:1000] / [ColorSession viewController:xxxx]) */
...
@interface MemoryModel(Animation)
...
@interface MemoryModel(Callback)
...
@interface SimplyLayer: NSObject
...
@interface TitleCenterInput: NSObject
...
@interface ConcealedRandom: NSObject
...
ToolTouch resultCell(void);

//: CornerRadii CornerRadiiNull(void);
ToolTouch sizeMemberNull(void);
