...
static NSString* const show_keyValue = @"\u2026";
...
- (NSAttributedString *)attributedEnable:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
...
- (NSAttributedString *)snapLine:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
+ (NSRegularExpression *)languageSum {
...
- (NSMutableAttributedString *)scaleValue:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[InsertView languageSum] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
...
- (NSMutableAttributedString*)bubble:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self attributedEnable:text];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self snapLine:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self bubble:[self scaleValue:attString]];
...
- (void)gift:(NSString *)text
{
    //: NSAttributedString *attributedText = [self attributedString:text];
    NSAttributedString *attributedText = [self attributedEnable:text];
    //: [self appendAttributedText:attributedText];
    [self disable:attributedText];
}

//: - (void)appendAttributedText:(NSAttributedString *)attributedText
- (void)disable:(NSAttributedString *)attributedText
{
