...
@interface RandomSharedView () <SessionResponder>
...
@property (nonatomic, strong) NSMutableArray<ItemPushView*> *items;
...
@property (nonatomic, strong) PositionView *touchReceiver;
...
@implementation RandomSharedView

//: static NSInteger const DEFAULT_VISIBLE_ITEM_COUNT = 2;
static NSInteger const showFormatMessage = 2;
//: static NSTimeInterval const DEFAULT_TIME_INTERVAL = 4.0;
static NSTimeInterval const dream_timeFlushUrl = 4.0;
//: static NSTimeInterval const DEFAULT_TIME_DURATION = 1.0;
static NSTimeInterval const mValueData = 1.0;
//: static float const DEFAULT_SCROLL_SPEED = 40.0f;
static float const mainSourceMsg = 40.0f;
//: static float const DEFAULT_ITEM_SPACING = 20.0f;
static float const user_dataInsideContent = 20.0f;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _timeIntervalPerScroll = dream_timeFlushUrl;
        //: _timeDurationPerScroll = DEFAULT_TIME_DURATION;
        _timeDurationPerScroll = mValueData;
        //: _scrollSpeed = DEFAULT_SCROLL_SPEED;
        _scrollSpeed = mainSourceMsg;
        //: _itemSpacing = DEFAULT_ITEM_SPACING;
        _itemSpacing = user_dataInsideContent;
        //: _touchEnabled = NO;
        _touchEnabled = NO;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 ...
                                                 selector:@selector(bearAlongForTopClear:)
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 ...
                                                 selector:@selector(becomeWithShould:)
...
- (void)bearAlongForTopClear:(NSNotification*)notification {
...
- (void)becomeWithShould:(NSNotification*)notification {
    //: if (!_isPausingBeforeResignActive) {
    if (!_isPausingBeforeResignActive) {
        //: [self start];
        [self linguisticTitle];
    }
}
...
- (void)model {
    ...
    //: } else {
        ...
        if ([_delegate respondsToSelector:@selector(viewwed:)]) {
            //: self.visibleItemCount = [_delegate numberOfVisibleItemsForMarqueeView:self];
            self.visibleItemCount = [_delegate viewwed:self];
        ...
        //: } else {
            ...
            self.visibleItemCount = showFormatMessage;
        }
    }
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        ...
        ItemPushView *itemView = [[ItemPushView alloc] init];
    ...
    if (_direction == UUMarqueeViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            ...
            //: } else {
                ...
                [self end:_items[index] nameIndex:_items[index].tag];
            }
        }
    //: } else {
    } else {
        ...
        if (_useDynamicHeight) {
            ...
            for (int i = 0; i < _items.count; i++) {
                ...
                if (i == _items.count - 1) {
                    ...
                    _items[index].height = [self valueTouch:_items[index].tag];
        ...
        //: } else {
            ...
            if ([_delegate respondsToSelector:@selector(pathing:)]) {
                //: dataCount = [_delegate numberOfDataForMarqueeView:self];
                dataCount = [_delegate pathing:self];
            }
...
- (int)artifact:(int)offsetFromTop {
...
- (void)end:(ItemPushView*)itemView nameIndex:(NSInteger)index {
    //: if (index < 0) {
    if (index < 0) {
        //: [itemView clear];
        [itemView messageClear];
    }
    ...
    if (index >= 0) {
        ...
        if ([_delegate respondsToSelector:@selector(componentPartQueryed:valueTing:index:)]) {
            //: [_delegate updateItemView:itemView atIndex:index forMarqueeView:self];
            [_delegate componentPartQueryed:itemView valueTing:index index:self];
        }
    }
}
...
- (void)willing:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        ...
        if (_direction == UUMarqueeViewDirectionLeftward) {
            ...
            _items[_firstItemIndex].width = [self version:_items[_firstItemIndex].tag];
            ...
            if (firstItemWidth != nextItemWidth) {
                ...
                [_items[_firstItemIndex] messageClear];
            }
            //: [self updateItemView:_items[_firstItemIndex] atIndex:_items[_firstItemIndex].tag];
            [self end:_items[_firstItemIndex] nameIndex:_items[_firstItemIndex].tag];
        ...
        //: } else {
            ...
            if (_useDynamicHeight) {
                ...
                _items[_firstItemIndex].height = [self valueTouch:_items[_firstItemIndex].tag];
...
- (void)find {
    ...
    if (_touchEnabled) {
        ...
        if (!_touchReceiver) {
            ...
            self.touchReceiver = [[PositionView alloc] init];
...
- (void)itemFor:(CGPoint)point {
    ...
    for (ItemPushView *itemView in _items) {
        ...
        if ([itemView.layer.presentationLayer hitTest:point]) {
            ...
            if (dataCount > 0 && itemView.tag >= 0 && itemView.tag < dataCount) {
                ...
                if ([self.delegate respondsToSelector:@selector(color:voice:)]) {
                    //: [self.delegate didTouchItemViewAtIndex:itemView.tag forMarqueeView:self];
                    [self.delegate color:itemView.tag voice:self];
                }
            }
            //: break;
            break;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchDelegate) {
        ...
        [_touchDelegate itemFor:touchLocation];
    }
}
