...
typedef struct {
    Byte nonetheless;
    Byte *legacyButton;
    unsigned int successNumberry;
} StructTheologyData;

@interface TheologyData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation TheologyData

+ (instancetype)sharedInstance {
    static TheologyData *instance = nil;
...
- (Byte *)TheologyDataToByte:(StructTheologyData *)data {
    for (int i = 0; i < data->successNumberry; i++) {
        data->legacyButton[i] ^= data->nonetheless;
    }
    data->legacyButton[data->successNumberry] = 0;
    return data->legacyButton;
}

- (NSString *)StringFromTheologyData:(StructTheologyData *)data {
    return [NSString stringWithUTF8String:(char *)[self TheologyDataToByte:data]];
}

//: friend_circle_adapter_like
- (NSString *)show_drugTitle {
    /* static */ NSString *show_drugTitle = nil;
    if (!show_drugTitle) {
        StructTheologyData value = (StructTheologyData){222, (Byte []){184, 172, 183, 187, 176, 186, 129, 189, 183, 172, 189, 178, 187, 129, 191, 186, 191, 174, 170, 187, 172, 129, 178, 183, 181, 187, 200}, 26};
        show_drugTitle = [self StringFromTheologyData:&value];
    }
    return show_drugTitle;
}

//: ic_praise%d
- (NSString *)k_annuallyName {
    /* static */ NSString *k_annuallyName = nil;
    if (!k_annuallyName) {
        StructTheologyData value = (StructTheologyData){99, (Byte []){10, 0, 60, 19, 17, 2, 10, 16, 6, 70, 7, 175}, 11};
        k_annuallyName = [self StringFromTheologyData:&value];
    }
    return k_annuallyName;
}

//: #333333
- (NSString *)dream_endContent {
    /* static */ NSString *dream_endContent = nil;
    if (!dream_endContent) {
        StructTheologyData value = (StructTheologyData){156, (Byte []){191, 175, 175, 175, 175, 175, 175, 155}, 7};
        dream_endContent = [self StringFromTheologyData:&value];
    }
    return dream_endContent;
}

//: WorkCF7
- (NSString *)dreamRamValue {
    /* static */ NSString *dreamRamValue = nil;
    if (!dreamRamValue) {
        StructTheologyData value = (StructTheologyData){61, (Byte []){123, 123, 123, 126, 123, 10, 119}, 6};
        dreamRamValue = [self StringFromTheologyData:&value];
    }
    return dreamRamValue;
}

//: arrowUp
- (NSString *)main_gooData {
    /* static */ NSString *main_gooData = nil;
    if (!main_gooData) {
        StructTheologyData value = (StructTheologyData){163, (Byte []){194, 209, 209, 204, 212, 246, 211, 217}, 7};
        main_gooData = [self StringFromTheologyData:&value];
    }
    return main_gooData;
}

//: ic_fold
- (NSString *)kMainlyTitle {
    /* static */ NSString *kMainlyTitle = nil;
    if (!kMainlyTitle) {
        StructTheologyData value = (StructTheologyData){69, (Byte []){44, 38, 26, 35, 42, 41, 33, 71}, 7};
        kMainlyTitle = [self StringFromTheologyData:&value];
    }
    return kMainlyTitle;
}

//: arrowDown
- (NSString *)kPopularityValue {
    /* static */ NSString *kPopularityValue = nil;
    if (!kPopularityValue) {
        StructTheologyData value = (StructTheologyData){60, (Byte []){93, 78, 78, 83, 75, 120, 83, 75, 82, 185}, 9};
        kPopularityValue = [self StringFromTheologyData:&value];
    }
    return kPopularityValue;
}
...
//: typedef NS_OPTIONS(NSUInteger, WorkBubbleMenuViewDirectionPriorityType) {
typedef NS_OPTIONS(NSUInteger, WorkBubbleMenuViewDirectionPriorityType) {
    //: WorkBubbleMenuViewDirectionPriorityType_Up,
    WorkBubbleMenuViewDirectionPriorityType_Up,
    //: WorkBubbleMenuViewDirectionPriorityType_Down
    WorkBubbleMenuViewDirectionPriorityType_Down
//: };
};

//: @interface WorkBubbleMenuView()
@interface ValueWithView()
...
//: @property (nonatomic, assign)WorkBubbleMenuViewDirectionPriorityType directionPriority;
@property (nonatomic, assign)WorkBubbleMenuViewDirectionPriorityType directionPriority;
...
//: @implementation WorkBubbleMenuView
@implementation ValueWithView

//: + (instancetype)shareMenuView {
+ (instancetype)success {
    //: static WorkBubbleMenuView *menu = nil;
    static ValueWithView *menu = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    ...
    _dispatch_once(&onceToken, ^{
        //: if (menu == nil) {
        if (menu == nil) {
            //: menu = [[WorkBubbleMenuView alloc] init];
            menu = [[ValueWithView alloc] init];
        }
    //: });
    });
  //: return menu;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _bubbleBottomMargin = 20;
        //: _directionPriority = WorkBubbleMenuViewDirectionPriorityType_Up;
        _directionPriority = WorkBubbleMenuViewDirectionPriorityType_Up;

        //: UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickView)];
        UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(blueTime)];
...
- (void)praise{
    ...
    for (int i = 0; i < _buttonCount; i ++) {
        ...
        if(_isfold && i==7){
            ...
            [_foldbtn addTarget:self action:@selector(offPull:) forControlEvents:UIControlEventTouchUpInside];
            //: [_foldbtn setImage:[UIImage imageNamed:@"ic_fold"] forState:UIControlStateNormal];
            [_foldbtn setImage:[UIImage imageNamed:[[TheologyData sharedInstance] kMainlyTitle]] forState:UIControlStateNormal];
        ...
        [button addTarget:self action:@selector(valueBy:) forControlEvents:UIControlEventTouchUpInside];
        //: [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"ic_praise%d",i+1]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:[[TheologyData sharedInstance] k_annuallyName],i+1]] forState:UIControlStateNormal];
...
- (void)sieve:(NSArray *)array
                 //: cursorStartRect:(CGRect)cursorStartRect selectionTextRectInWindow:(CGRect)rect selectBlock:(void(^)(WorkMediaItem *item))block praiseBlock:(void(^)(NSInteger tag))praiseblock {
                 from:(CGRect)cursorStartRect stride:(CGRect)rect message:(void(^)(AccountAction *item))block show:(void(^)(NSInteger tag))praiseblock {
    ...
    UIView *buttonsBgView = [self matrix:array];
    ...
    //默认的方向是优先方向
    //: WorkBubbleMenuViewDirectionPriorityType direction = _directionPriority;
    WorkBubbleMenuViewDirectionPriorityType direction = _directionPriority;
//
...
//        //优先级Up的话，先判断在上面的情况。
        ...
        if (rect.origin.y - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            selfRect.origin.y = rect.origin.y - _bubbleContentMargin - _bubbleHeight - praiseheight + 15;
            //: direction = WorkBubbleMenuViewDirectionPriorityType_Up;
            direction = WorkBubbleMenuViewDirectionPriorityType_Up;
        ...
        //: } else if (screenHeight - rect.origin.y - rect.size.height - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            selfRect.origin.y = rect.origin.y + rect.size.height + _bubbleContentMargin -15;
            //: direction = WorkBubbleMenuViewDirectionPriorityType_Down;
            direction = WorkBubbleMenuViewDirectionPriorityType_Down;
...
//    }
    ...
    CGRect bgViewRect = buttonsBgView.frame;
    //: bgViewRect.origin.y = direction == WorkBubbleMenuViewDirectionPriorityType_Down ? _arrowHeight : praiseheight;
    bgViewRect.origin.y = direction == WorkBubbleMenuViewDirectionPriorityType_Down ? _arrowHeight : praiseheight;
...
//    }
    ...
    //: if (direction == WorkBubbleMenuViewDirectionPriorityType_Up) {
    if (direction == WorkBubbleMenuViewDirectionPriorityType_Up) {
        ...
        _arrowView.image = [UIImage imageNamed:[[TheologyData sharedInstance] kPopularityValue]];
    ...
    //: } else {
        ...
        _arrowView.image = [UIImage imageNamed:[[TheologyData sharedInstance] main_gooData]];
...
- (UIView *)matrix:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        [button addTarget:self action:@selector(fulls:) forControlEvents:UIControlEventTouchUpInside];

        //: WorkBubbleButtonModel *model = array[i];
        GlobuleSession *model = array[i];
        ...
        label.textColor = [UIColor ground:[[TheologyData sharedInstance] dream_endContent]];
    ...
    //: _buttonsBgView.backgroundColor = [UIColor colorWithHexString:@"WorkCF7"];
    _buttonsBgView.backgroundColor = [UIColor ground:[[TheologyData sharedInstance] dreamRamValue]];
...
- (void)fulls:(UIButton *)button {
    //: if (_myNewContentArray.count > button.tag - 100) {
    if (_myNewContentArray.count > button.tag - 100) {
        //: WorkBubbleButtonModel *model = (WorkBubbleButtonModel *)_myNewContentArray[button.tag - 100];
        GlobuleSession *model = (GlobuleSession *)_myNewContentArray[button.tag - 100];
        //: NSLog(@"点击了%@",model.name);

        //: if([model.name isEqual:[WorkLanguageManager getTextWithKey:@"friend_circle_adapter_like"]]){
        if([model.name isEqual:[InputRed preserve:[[TheologyData sharedInstance] show_drugTitle]]]){
...
- (BOOL)roundRecentSystem:(NSArray *)newArray {
    ...
    for (int i = 0; i < newArray.count; i ++) {
        //: BOOL finded = false;
        BOOL finded = false;
        //: NSInteger newId = ((WorkBubbleButtonModel *)_oldContentArray[i]).buttonId;
        NSInteger newId = ((GlobuleSession *)_oldContentArray[i]).buttonId;
        //: for (int j = 0; j < _oldContentArray.count; j ++) {
        for (int j = 0; j < _oldContentArray.count; j ++) {
            //: NSInteger oldId = ((WorkBubbleButtonModel *)_oldContentArray[j]).buttonId;
            NSInteger oldId = ((GlobuleSession *)_oldContentArray[j]).buttonId;
            //: if (newId == oldId) {
            if (newId == oldId) {
                //: finded = true;
                finded = true;
