...
@implementation ClickPair

//: - (instancetype)initWithFirst:(id)first second:(id)second {
- (instancetype)initWithLanguage:(id)first queryed:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
- (void)setMembers:(NSArray *)members {
    ...
    for (id<MemberProtocol>member in members) {
        ...
        NSString *groupTitle = [member quick];
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[ClickPair alloc] initWithLanguage:groupTitle queryed:groupedMembers]];
        }
    //: }];
    }];
    //: [self sortGroup];
...
- (void)setSpecialMembers:(NSArray *)specialMembers {
    ...
    if (specialMembers.count > 0) {
        ...
        [_groups insertObject:[[ClickPair alloc] initWithLanguage:@"$" queryed:specialMembers] atIndex:0];
...
- (NSDictionary *)contentDic {
    ...
    for (int i = 0; i < _groups.count; ++i) {
        ...
        NSArray *tempArr = [self group:i];
        //: NSString *title = [self titleofGroup:i];
        NSString *title = [self exceptVideo:i];
        //: [tempArr enumerateObjectsUsingBlock:^(id<NIMGroupMemberProtocol>member, NSUInteger idx, BOOL * _Nonnull stop) {
        [tempArr enumerateObjectsUsingBlock:^(id<MemberProtocol>member, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSString *)exceptVideo:(NSInteger)groupIndex {
...
- (NSArray *)group:(NSInteger)groupIndex
{
    //: if(groupIndex >= 0 && groupIndex < _groups.count) {
    if(groupIndex >= 0 && groupIndex < _groups.count) {
        //: NIMKitPair *pair = [_groups objectAtIndex:groupIndex];
        ClickPair *pair = [_groups objectAtIndex:groupIndex];
        //: return pair.second;
        return pair.second;
    }
    //: return nil;
...
- (void)dismiss
{
    ...
    [_groups sortUsingComparator:^NSComparisonResult(ClickPair *pair1, ClickPair *pair2) {
...
- (void)collection
{
    ...
    [_groups enumerateObjectsUsingBlock:^(ClickPair *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<MemberProtocol> member1, id<MemberProtocol> member2) {
            //: return _groupMemberComparator([member1 sortKey], [member2 sortKey]);
            return _groupMemberComparator([member1 screenSortFormat], [member2 screenSortFormat]);
        //: }];
        }];
    //: }];
    }];
