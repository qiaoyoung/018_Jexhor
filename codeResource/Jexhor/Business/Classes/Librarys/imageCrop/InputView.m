...
- (Byte *)BlackDataToCache:(Byte *)data {
    int negativeDelicate = data[0];
    int discrimination = data[1];
    for (int i = 0; i < negativeDelicate / 2; i++) {
        int begin = discrimination + i;
        int end = discrimination + negativeDelicate - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[discrimination + negativeDelicate] = 0;
    return data + discrimination;
}
...
//: center
- (NSString *)user_adviserName {
    /* static */ NSString *user_adviserName = nil;
    if (!user_adviserName) {
        Byte value[] = {6, 10, 150, 38, 4, 87, 239, 7, 255, 114, 114, 101, 116, 110, 101, 99, 80};
        user_adviserName = [self StringFromBlackData:value];
    }
    return user_adviserName;
}

//: frame
- (NSString *)main_imageTitle {
    /* static */ NSString *main_imageTitle = nil;
    if (!main_imageTitle) {
        Byte value[] = {5, 2, 101, 109, 97, 114, 102, 168};
        main_imageTitle = [self StringFromBlackData:value];
    }
    return main_imageTitle;
}
...
- (UIImage *)flipBy:(CGRect)rect
{

    //: UIImage *fixedImage = [self fixOrientation];
    UIImage *fixedImage = [self tag];
...
@interface ModelSizeView: UIView
...
@implementation QualifyView
//: - (instancetype)initWithLineWidth: (CGFloat)lineWidth lineHeight: (CGFloat)lineHeight lineColor: (UIColor *)lineColor {
- (instancetype)initWithName: (CGFloat)lineWidth ting: (CGFloat)lineHeight frame: (UIColor *)lineColor {
...
@interface TempEnablelyView : UIView
...
@implementation TempEnablelyView
...
@property (strong, nonatomic) ModelSizeView *topLeftCorner;
//: @property (strong, nonatomic) CornerView *topRightCorner;
@property (strong, nonatomic) ModelSizeView *topRightCorner;
//: @property (strong, nonatomic) CornerView *bottomLeftCorner;
@property (strong, nonatomic) ModelSizeView *bottomLeftCorner;
//: @property (strong, nonatomic) CornerView *bottomRightCorner;
@property (strong, nonatomic) ModelSizeView *bottomRightCorner;
//: @property (strong, nonatomic) CropAreaView *cropAreaView;
@property (strong, nonatomic) TempEnablelyView *cropAreaView;
...
@property (strong, nonatomic) QualifyView *topMidLine;
//: @property (strong, nonatomic) MidLineView *leftMidLine;
@property (strong, nonatomic) QualifyView *leftMidLine;
//: @property (strong, nonatomic) MidLineView *bottomMidLine;
@property (strong, nonatomic) QualifyView *bottomMidLine;
//: @property (strong, nonatomic) MidLineView *rightMidLine;
@property (strong, nonatomic) QualifyView *rightMidLine;
...
- (void)dealloc {
    ...
    [_cropAreaView removeObserver: self forKeyPath: [[BlackData sharedInstance] main_imageTitle]];
    //: [_cropAreaView removeObserver: self forKeyPath: @"center"];
    [_cropAreaView removeObserver: self forKeyPath: [[BlackData sharedInstance] user_adviserName]];
    //: [_imageView removeObserver: self forKeyPath: @"frame"];
    [_imageView removeObserver: self forKeyPath: [[BlackData sharedInstance] main_imageTitle]];
...
- (void)length {
    ...
    _cropAreaView = [[TempEnablelyView alloc] init];
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"frame"
                    forKeyPath: [[BlackData sharedInstance] main_imageTitle]
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"center"
                    forKeyPath: [[BlackData sharedInstance] user_adviserName]
                       //: options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial
                       options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial
                       //: context: NULL];
                       context: NULL];
...
- (void)heritagePlayer {
    ...
    _topLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(messages:)];
    //: _topRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _topRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(messages:)];
    //: _bottomLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _bottomLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(messages:)];
    //: _bottomRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _bottomRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(messages:)];
    //: _cropAreaPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCropAreaPan:)];
    _cropAreaPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(skillets:)];
...
- (void)mediaCropCamera: (UIPinchGestureRecognizer *)pinchGesture {
    ...
    switch (pinchGesture.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            [self recordFactor: pinchGesture.scale];
            //: break;
            break;
        }
        //: default:
...
- (void)skillets: (UIPanGestureRecognizer *)panGesture {
...
- (void)middleBlue: (UIPanGestureRecognizer *)panGesture {

    //: MidLineView *midLineView = (MidLineView *)panGesture.view;
    QualifyView *midLineView = (QualifyView *)panGesture.view;
    //: switch (panGesture.state) {
    switch (panGesture.state) {
        //: case UIGestureRecognizerStateBegan: {
        case UIGestureRecognizerStateBegan: {
...
- (void)messages: (UIPanGestureRecognizer *)panGesture {

    //: CornerView *panView = (CornerView *)panGesture.view;
    ModelSizeView *panView = (ModelSizeView *)panGesture.view;
    //: CornerView *relativeViewX = panView.relativeViewX;
    ModelSizeView *relativeViewX = panView.relativeViewX;
    //: CornerView *relativeViewY = panView.relativeViewY;
    ModelSizeView *relativeViewY = panView.relativeViewY;
...
- (void)message {
    ...
    [_topLeftCorner user: _cropAreaCornerWidth contentComment: _cropAreaCornerHeight];
    //: [_topRightCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_topRightCorner user: _cropAreaCornerWidth contentComment: _cropAreaCornerHeight];
    //: [_bottomLeftCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_bottomLeftCorner user: _cropAreaCornerWidth contentComment: _cropAreaCornerHeight];
    //: [_bottomRightCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_bottomRightCorner user: _cropAreaCornerWidth contentComment: _cropAreaCornerHeight];
...
- (void)change {
    ...
    _topLeftCorner = [[ModelSizeView alloc]initWithFrame: CGRectMake(0, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) text:_cropAreaCornerLineColor find: _cropAreaCornerLineWidth];
    ...
    _topRightCorner = [[ModelSizeView alloc]initWithFrame: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) text: _cropAreaCornerLineColor find: _cropAreaCornerLineWidth];
    ...
    _bottomLeftCorner = [[ModelSizeView alloc]initWithFrame: CGRectMake(0, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) text: _cropAreaCornerLineColor find: _cropAreaCornerLineWidth];
    ...
    _bottomRightCorner = [[ModelSizeView alloc]initWithFrame: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) text: _cropAreaCornerLineColor find: _cropAreaCornerLineWidth];
...
- (void)maxImageLines {
    ...
    _topMidLine = [[QualifyView alloc]initWithName: _cropAreaMidLineWidth ting: _cropAreaMidLineHeight frame: _cropAreaMidLineColor];
    ...
    _bottomMidLine = [[QualifyView alloc]initWithName: _cropAreaMidLineWidth ting: _cropAreaMidLineHeight frame: _cropAreaMidLineColor];
    ...
    _leftMidLine = [[QualifyView alloc]initWithName: _cropAreaMidLineWidth ting: _cropAreaMidLineHeight frame: _cropAreaMidLineColor];
    ...
    _rightMidLine = [[QualifyView alloc]initWithName: _cropAreaMidLineWidth ting: _cropAreaMidLineHeight frame: _cropAreaMidLineColor];
    ...
    _topMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(middleBlue:)];
    ...
    _bottomMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(middleBlue:)];
    ...
    _leftMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(middleBlue:)];
    ...
    _rightMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(middleBlue:)];
...
- (void)recordFactor: (CGFloat)scaleFactor {
...
- (void)setNeedScaleCrop:(BOOL)needScaleCrop {
    ...
    if(!_needScaleCrop && needScaleCrop) {
        ...
        _cropAreaPinch = [[UIPinchGestureRecognizer alloc]initWithTarget: self action:@selector(mediaCropCamera:)];
...
- (UIImage *)titleMentalRepresentation {
    ...
    return [_toCropImage flipBy: CGRectMake((CGRectGetMinX(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetMinY(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetWidth(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetHeight(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor)];
