...
#import "MarginBarView.h"

//: static CGFloat const DEFAULT_MASK_ALPHA = 0.75;
static CGFloat const show_nameSpaceValue = 0.75;
//: static BOOL const square = NO;
static BOOL const noti_redTitle = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float mTouchId = 400;
//: float IMAGE_MIN_WIDTH = 400;
float show_viewStr = 400;

//: @interface ImageCropViewController(){
@interface ToolMessageViewController(){
    //: CGRect _cropArea;
    CGRect _cropArea;
}
...
#pragma mark ToolMessageViewController implementation
//: @implementation ImageCropViewController
@implementation ToolMessageViewController
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[MarginBarView alloc] initWithItem:view domain:self.blurredBackground];
...
- (void)querying:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(language:titleNeed:)])
    {
        //: UIImage *cropped;
        UIImage *cropped;
        //: if (self.image != nil){
        ...
        [self.delegate language:self titleNeed:cropped];
    }
...
#pragma mark GraduatedTableView implementation

//: @implementation ControlPointView
@implementation GraduatedTableView
...
static CGFloat const main_textKey = 5;
...
- (void)initInfo {
    ...
    self.shadeView = [[View alloc] initWithFrame:subviewFrame];
    ...
    topLeftPoint = [self anyQueryAt:resourceDataCenter(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self anyQueryAt:resourceDataCenter(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self anyQueryAt:resourceDataCenter(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self anyQueryAt:resourceDataCenter(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self sessionPoints];
    ...
    UIPanGestureRecognizer* dragRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(trains:)];
    ...
    self.maskAlpha = show_nameSpaceValue;
...
- (GraduatedTableView*)anyQueryAt:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    GraduatedTableView* point = [[GraduatedTableView alloc] initWithFrame:frame];
    //: return point;
    return point;
}

//: - (CGRect)cropAreaFromControlPoints {
- (CGRect)sessionPoints {
...
- (void)worker:(CGPoint)topLeft eventRight:(CGPoint)bottomLeft image:(CGPoint)bottomRight queryed:(CGPoint)topRight {
...
- (UIView*)state:(CGPoint)point {
    //: UIView* view = cropAreaView;
    UIView* view = cropAreaView;
    //: for (int i = 0; i < PointsArray.count; i++) {
    for (int i = 0; i < PointsArray.count; i++) {
        //: CGFloat x = [(ControlPointView *)PointsArray[i] center].x;
        CGFloat x = [(GraduatedTableView *)PointsArray[i] center].x;
        //: CGFloat y = [(ControlPointView *)PointsArray[i] center].y;
        CGFloat y = [(GraduatedTableView *)PointsArray[i] center].y;
...
- (void)trains:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self colorBottom:recognizer textStart:count];
        //: else
        else
            //: [self prepSingleTouchPan:recognizer];
            [self direction:recognizer];
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self showView:[recognizer locationOfTouch:i inView:self] of:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self showView:[recognizer locationInView:self] of:dragViewOne];
    }
...
- (void)colorBottom:(UIPanGestureRecognizer*)recognizer textStart:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self state:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self state:[recognizer locationOfTouch:i inView:self]];
...
- (void)direction:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self state:[recognizer locationInView:self]];
...
- (void)showView:(CGPoint)location of:(UIView*)view
{
...
- (CGSize)marginView:(CGPoint)dragLocation sumPoint:(CGPoint)draggedPoint sharedOn:(CGPoint)oppositePoint {
...
- (void)when:(CGPoint)dragLocation {
    ...
    CGSize disp = [self marginView:dragLocation sumPoint:dragPoint.topLeftCenter sharedOn:dragPoint.bottomRightCenter];
    ...
    if (width >= show_viewStr && height < mTouchId) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - mTouchId) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < show_viewStr && height >= mTouchId) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - show_viewStr) / self.imageScale);
    ...
    [self worker:topLeft eventRight:bottomLeft image:dragPoint.bottomRightCenter queryed:topRight];
}
//: - (void)handleDragBottomLeft:(CGPoint)dragLocation {
- (void)distanceCenter:(CGPoint)dragLocation {
    ...
    CGSize disp = [self marginView:dragLocation sumPoint:dragPoint.bottomLeftCenter sharedOn:dragPoint.topRightCenter];
    ...
    if (width >= show_viewStr && height < mTouchId) {
        ...
        bottomLeft.y = cropArea.origin.y + (mTouchId / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < show_viewStr && height >= mTouchId) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - show_viewStr) / self.imageScale);
    ...
    [self worker:topLeft eventRight:bottomLeft image:bottomRight queryed:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)misconductLikeAdmittance:(CGPoint)dragLocation {
    ...
    CGSize disp = [self marginView:dragLocation sumPoint:dragPoint.bottomRightCenter sharedOn:dragPoint.topLeftCenter];
    ...
    if (width >= show_viewStr && height < mTouchId) {
        ...
        bottomRight.y = cropArea.origin.y + (mTouchId / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < show_viewStr && height >= mTouchId) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (show_viewStr / self.imageScale);
    ...
    [self worker:dragPoint.topLeftCenter eventRight:bottomLeft image:bottomRight queryed:topRight];
}

//: - (void)handleDragTopRight:(CGPoint)dragLocation {
- (void)titleUser:(CGPoint)dragLocation {
    ...
    CGSize disp = [self marginView:dragLocation sumPoint:dragPoint.topRightCenter sharedOn:dragPoint.bottomLeftCenter];
    ...
    if (width >= show_viewStr && height < mTouchId) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - mTouchId) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < show_viewStr && height >= mTouchId) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (show_viewStr / self.imageScale);
    ...
    [self worker:topLeft eventRight:dragPoint.bottomLeftCenter image:bottomRight queryed:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)app:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image thoughtImageScaleMessageWithIterationsGreenVisualProperty:30 contentView:1 showPraise:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image thoughtImageScaleMessageWithIterationsGreenVisualProperty:0 contentView:1 showPraise:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self worker:topLeftPoint.center eventRight:bottomLeftPoint.center image:bottomRightPoint.center queryed:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self sessionPoints];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
