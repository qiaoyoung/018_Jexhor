...
- (void)team
{
    ...
    self.dotViewClass = [ChangeTempDotView class];
    //: self.spacingBetweenDots = kDefaultSpacingBetweenDots;
    self.spacingBetweenDots = noti_originControlText;
    //: self.numberOfPages = kDefaultNumberOfPages;
    self.numberOfPages = dreamIndexMessage;
    //: self.currentPage = kDefaultCurrentPage;
    self.currentPage = show_rowUrl;
    //: self.hidesForSinglePage = kDefaultHideForSinglePage;
    self.hidesForSinglePage = m_modelScaleTitleKey;
    //: self.shouldResizeFromCenter = kDefaultShouldResizeFromCenter;
    self.shouldResizeFromCenter = mCurrentId;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(visible:array:)]) {
            //: [self.delegate TAPageControl:self didSelectPageAtIndex:index];
            [self.delegate visible:self array:index];
        }
    }
}
...
- (CGSize)scoff:(NSInteger)pageCount
{
...
- (void)action
{
    ...
    for (NSInteger i = 0; i < self.numberOfPages; i++) {
        ...
        [self noneValueHolder:dot downwards:i];
    }

    //: [self changeActivity:YES atIndex:self.currentPage];
    [self worshipDoing:YES will:self.currentPage];
...
- (void)flushWith:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self scoff:self.numberOfPages];
...
- (void)noneValueHolder:(UIView *)dot downwards:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self scoff:self.numberOfPages].width) / 2);
...
- (UIView *)present
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[ChangeTempDotView class]] && self.dotColor) {
            //: ((TAAnimatedDotView *)dotView).dotColor = self.dotColor;
            ((ChangeTempDotView *)dotView).dotColor = self.dotColor;
        }
    //: } else {
    } else {
...
- (void)worshipDoing:(BOOL)active will:(NSInteger)index
{
    //: if (self.dotViewClass) {
    if (self.dotViewClass) {
        //: TAAbstractDotView *abstractDotView = (TAAbstractDotView *)[self.dots objectAtIndex:index];
        TitleClickView *abstractDotView = (TitleClickView *)[self.dots objectAtIndex:index];
        //: if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
        if ([abstractDotView respondsToSelector:@selector(heatherMixture:)]) {
            //: [abstractDotView changeActivityState:active];
            [abstractDotView heatherMixture:active];
        //: } else {
        } else {
...
- (void)setCurrentPage:(NSInteger)currentPage
{
    ...
    [self worshipDoing:NO will:_currentPage];
    //: _currentPage = currentPage;
    _currentPage = currentPage;
    // Post set
    //: [self changeActivity:YES atIndex:_currentPage];
    [self worshipDoing:YES will:_currentPage];
}
