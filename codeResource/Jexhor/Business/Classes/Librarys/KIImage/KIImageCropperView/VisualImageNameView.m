...
@implementation VisualImageNameView
...
- (CaseView *)maskView {
    ...
    if (_maskView == nil) {
        ...
        _maskView = [[CaseView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)viewMax {
    ...
    if (_imageView == nil) {
        ...
        [[self selectedView] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)sessionPin:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self viewMax] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self viewMax] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)blue {
    ...
    [[self selectedView] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self selectedView] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self selectedView] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self selectedView] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self selectedView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(CaseView*)[self maskView] setRecent:[self maskBuildLargeMessage]];
...
- (UIImage *)lowlinessView {
    //: CGFloat zoomScale = [self scrollView].zoomScale;
    CGFloat zoomScale = [self selectedView].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self selectedView].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self selectedView].contentOffset.y;
    ...
    UIImage *image = [_image start:aX title:aY cut:aWidth smart:aHeight];
    ...
    image = [self modelRead:image coverTing:imgSize outerSpace:YES];
...
- (UIImage*)modelRead:(UIImage*)image coverTing:(CGSize)newSize outerSpace:(BOOL)opaque
{
