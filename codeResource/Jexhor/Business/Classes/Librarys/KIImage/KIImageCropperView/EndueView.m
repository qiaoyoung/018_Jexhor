...
- (void)setHighlightSnuggle:(CGSize)size;
//: - (CGSize)maskSize;
- (CGSize)highlightSnuggle;
//: @end
@end
...
- (EquatorView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KIImageCropperMaskView alloc] initWithFrame:self.bounds];
        _maskView = [[EquatorView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)dataView {
    ...
    if (_imageView == nil) {
        ...
        [[self tap] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)disable:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self dataView] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self dataView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)beginTo {
    ...
    [[self tap] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self tap] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self tap] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self tap] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self tap] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(EquatorView*)[self maskView] setHighlightSnuggle:[self array]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self tap] setFrame:self.bounds];
...
- (UIImage *)shirtButton {
    ...
    CGFloat zoomScale = [self tap].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self tap].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self tap].contentOffset.y;
    ...
    image = [self object:image atDisable:imgSize withMobile:YES];
...
- (UIImage*)object:(UIImage*)image atDisable:(CGSize)newSize withMobile:(BOOL)opaque
{
