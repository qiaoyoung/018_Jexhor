...
@property (nonatomic, strong) MustConfigure *configure;
...
- (void)dealloc {
    ...
    if ([TreatNurseLogMake queryed].log) {
        //: NSLog(@"SGScanView - - dealloc");
    }
}

//: - (instancetype)initWithFrame:(CGRect)frame configure:(SGScanViewConfigure *)configure {
- (instancetype)initWithRangeConfigure:(CGRect)frame message:(MustConfigure *)configure {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(domainFrame)];
...
+ (instancetype)replyBy:(CGRect)frame sub:(MustConfigure *)configure {
    //: return [[SGScanView alloc] initWithFrame:frame configure:configure];
    return [[SessionView alloc] initWithRangeConfigure:frame message:configure];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    [self to:borderX stop:borderY view:cornerLength should:insideExcess provider:outsideExcess];
    ...
    [self send:borderX trap:borderY addLocation:borderH label:cornerLength notSearchionValue:insideExcess managerExcess:outsideExcess];
    ...
    [self green:borderX sessionFloat:borderY field:borderW showExcess:cornerLength letter:insideExcess filter:outsideExcess];
    ...
    [self sizeOf:borderX bottomMessage:borderY physicalPropertyContainer:borderW message:borderH commissionerCell:cornerLength excessTouchPosition:insideExcess tingFloat:outsideExcess];
}
...
- (void)to:(CGFloat)borderX stop:(CGFloat)borderY view:(CGFloat)cornerLength should:(CGFloat) insideExcess provider:(CGFloat)outsideExcess {
...
- (void)green:(CGFloat)borderX sessionFloat:(CGFloat)borderY field:(CGFloat)borderW showExcess:(CGFloat)cornerLength letter:(CGFloat) insideExcess filter:(CGFloat)outsideExcess {
...
- (void)send:(CGFloat)borderX trap:(CGFloat)borderY addLocation:(CGFloat)borderH label:(CGFloat)cornerLength notSearchionValue:(CGFloat) insideExcess managerExcess:(CGFloat)outsideExcess {
...
- (void)sizeOf:(CGFloat)borderX bottomMessage:(CGFloat)borderY physicalPropertyContainer:(CGFloat)borderW message:(CGFloat)borderH commissionerCell:(CGFloat)cornerLength excessTouchPosition:(CGFloat) insideExcess tingFloat:(CGFloat)outsideExcess {
