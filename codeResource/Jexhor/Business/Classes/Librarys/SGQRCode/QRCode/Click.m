...
@interface Click () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: SGSoundEffect *soundEffect;
    SeemColor *soundEffect;
}
...
- (void)setDelegate:(id<ItemDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (void)setSampleBufferDelegate:(id<TotalTing>)sampleBufferDelegate {
...
- (void)full:(UIImage *)image skimOver:(void (^)(NSString *result))completion {
...
- (void)setOffPath:(CGFloat)factor {
...
- (void)field:(NSString *)name {
    ...
    NSString *voicePath = [[[ButtonManager examineedManager] quick] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [SeemColor contentFor:voicePath];
    //: [soundEffect play];
    [soundEffect with];
}
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self content];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(harvestObject:date:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate harvestObject:self date:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(ting:pictureFloat:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate ting:self pictureFloat:brightnessValue];
        }
    //: });
    });
}
