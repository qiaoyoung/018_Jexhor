...
@interface CenterRead () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
...
- (void)setSampleBufferDelegate:(id<NameInsert>)sampleBufferDelegate {
...
- (void)image:(UIImage *)image adpressed:(void (^)(NSString *result))completion {
...
- (void)setValueFactor:(CGFloat)factor {
...
- (void)firstTeam:(NSString *)name {
    ...
    NSString *voicePath = [[[SharedContentView manager] itemOf] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [Effect send:voicePath];
    //: [soundEffect play];
    [soundEffect alterPlay];
}
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self dataLast];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(lawfulBlackAndWhite:comment:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate lawfulBlackAndWhite:self comment:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(stepReport:max:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate stepReport:self max:brightnessValue];
        }
    //: });
    });
}
