...
static char notiRetGestureFormat;


//: @implementation UIActionSheet (NTESBlock)
@implementation UIActionSheet (Collection)


//: - (void)showInView: (UIView *)view completionHandler: (ActionSheetBlock)block
- (void)style: (UIView *)view data: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&notiRetGestureFormat,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        else
        {
            ...
            [self performSelector:@selector(sourceLanguage:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &notiRetGestureFormat) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&notiRetGestureFormat,nil,OBJC_ASSOCIATION_COPY);
