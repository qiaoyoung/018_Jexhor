...
@interface TagYaData : NSObject

+ (instancetype)sharedInstance;

//: jpeg
@property (nonatomic, copy) NSString *showOldItsData;

//: yyyy-MM-dd HH:mm:ss
@property (nonatomic, copy) NSString *userBeastData;

//: png
@property (nonatomic, copy) NSString *app_ambitionValue;

//: tiff
@property (nonatomic, copy) NSString *userInfoTitle;

//: gif
@property (nonatomic, copy) NSString *app_emergingValue;

@end

@implementation TagYaData

+ (instancetype)sharedInstance {
    static TagYaData *instance = nil;
...
- (Byte *)TagYaDataToCache:(Byte *)data {
    int department = data[0];
    Byte uniteTossMilk = data[1];
    int animal = data[2];
    for (int i = animal; i < animal + department; i++) {
        int value = data[i] + uniteTossMilk;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[animal + department] = 0;
    return data + animal;
}

- (NSString *)StringFromTagYaData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self TagYaDataToCache:data]];
}

//: tiff
- (NSString *)userInfoTitle {
    if (!_userInfoTitle) {
        Byte value[] = {4, 78, 10, 152, 166, 50, 183, 138, 60, 107, 38, 27, 24, 24, 44};
        _userInfoTitle = [self StringFromTagYaData:value];
    }
    return _userInfoTitle;
}

//: yyyy-MM-dd HH:mm:ss
- (NSString *)userBeastData {
    if (!_userBeastData) {
        Byte value[] = {19, 49, 4, 51, 72, 72, 72, 72, 252, 28, 28, 252, 51, 51, 239, 23, 23, 9, 60, 60, 9, 66, 66, 44};
        _userBeastData = [self StringFromTagYaData:value];
    }
    return _userBeastData;
}

//: gif
- (NSString *)app_emergingValue {
    if (!_app_emergingValue) {
        Byte value[] = {3, 43, 3, 60, 62, 59, 46};
        _app_emergingValue = [self StringFromTagYaData:value];
    }
    return _app_emergingValue;
}

//: jpeg
- (NSString *)showOldItsData {
    if (!_showOldItsData) {
        Byte value[] = {4, 4, 8, 141, 110, 52, 59, 253, 102, 108, 97, 99, 240};
        _showOldItsData = [self StringFromTagYaData:value];
    }
    return _showOldItsData;
}

//: png
- (NSString *)app_ambitionValue {
    if (!_app_ambitionValue) {
        Byte value[] = {3, 28, 10, 94, 176, 158, 192, 66, 157, 163, 84, 82, 75, 202};
        _app_ambitionValue = [self StringFromTagYaData:value];
    }
    return _app_ambitionValue;
}
...
@implementation UIImage (Eyeglasses)

//: - (UIImage *)flip:(BOOL)isHorizontal {
- (UIImage *)imageAt:(BOOL)isHorizontal {
...
- (UIImage *)incapacitate:(CGFloat)width {
    ...
    return [self message:width blowScale:height];
}
...
- (UIImage *)heightEnable:(CGFloat)heigh {
    ...
    return [self message:width blowScale:heigh];
}
...
- (UIImage *)window {
    ...
    [df setDateFormat:[TagYaData sharedInstance].userBeastData];
    ...
    return [self can:dateString
               //: textColor:[UIColor blackColor]
               showPoint:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    littleLeagueTeam:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   by:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
+ (NSString *) nameWithInformation:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return [TagYaData sharedInstance].showOldItsData;
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return [TagYaData sharedInstance].app_ambitionValue;
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return [TagYaData sharedInstance].app_emergingValue;
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return [TagYaData sharedInstance].userInfoTitle;
    }
    //: return @"";
    return @"";
}
...
- (UIImage*)barColorMessage:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
- (UIImage *)passage:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self name:maxLength*1024];
