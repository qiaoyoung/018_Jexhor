...
@implementation UIImage (Recent)
//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)markImage:(NSString *)imageNameOrPath{
...
- (UIImage *)item
{
    ...
    CGFloat pixels = [[FrameForwardSend user] limitMinPixels];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self content:pixels];
...
- (UIImage *)content: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self streetwise:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self streetwise:suggestPixels];
    }
}
...
- (UIImage *)streetwise: (CGFloat)maxPixels
{
    ...
    return [self value:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)value:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self pickUp:size];
}

//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)pickUp: (CGSize)size
{
