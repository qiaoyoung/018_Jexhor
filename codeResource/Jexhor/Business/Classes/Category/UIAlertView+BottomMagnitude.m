...
static char showObjectContent;

//: @implementation UIAlertView (NTESBlock)
@implementation UIAlertView (BottomMagnitude)
//: - (void)showAlertWithCompletionHandler: (void (^)(NSInteger))block
- (void)menuMedia: (void (^)(NSInteger))block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIAlertViewBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&showObjectContent,block,OBJC_ASSOCIATION_COPY);
    //: [self show];
    [self show];
}
...
- (void)snoop:(UIAlertView *)alertView discreditImage:(NSInteger)buttonIndex
{
    //: AlertBlock block = objc_getAssociatedObject(self, &kUIAlertViewBlockAddress);
    AlertBlock block = objc_getAssociatedObject(self, &showObjectContent);
    //: if (block)
    if (block)
    {
        //: block(buttonIndex);
        ...
        objc_setAssociatedObject(self, &showObjectContent, nil, OBJC_ASSOCIATION_COPY);
    }
}
...
@implementation UIAlertController (BottomMagnitude)
