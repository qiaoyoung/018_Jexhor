...
typedef struct {
    Byte underDate;
    Byte *rageCostly;
    unsigned int withTitle;
    Byte may1;
	int pinLiar;
	int threadDevice;
} MistakeData;

NSString *StringFromMistakeData(MistakeData *data);


//: frame
MistakeData noti_whiteKey = (MistakeData){154, (Byte []){252, 232, 251, 247, 255, 14}, 5, 193, 50, 180};

//: contentOffset
MistakeData m_termsContent = (MistakeData){223, (Byte []){188, 176, 177, 171, 186, 177, 171, 144, 185, 185, 172, 186, 171, 30}, 13, 188, 231, 69};

//: Pull to refresh...
MistakeData kTingId = (MistakeData){180, (Byte []){228, 193, 216, 216, 148, 192, 219, 148, 198, 209, 210, 198, 209, 199, 220, 154, 154, 154, 91}, 18, 181, 49, 48};

//: contentSize
MistakeData dreamBelowMsg = (MistakeData){143, (Byte []){236, 224, 225, 251, 234, 225, 251, 220, 230, 245, 234, 231}, 11, 201, 22, 113};

//: 松开刷新...
MistakeData appUpName = (MistakeData){62, (Byte []){216, 163, 128, 219, 130, 190, 219, 182, 137, 216, 168, 142, 16, 16, 16, 42}, 15, 224, 60, 215};

//: 下拉加载...
MistakeData user_pinId = (MistakeData){19, (Byte []){247, 171, 152, 245, 152, 154, 246, 153, 179, 251, 174, 174, 61, 61, 61, 12}, 15, 229, 235, 35};

//: 加载中...
MistakeData appChartMessage = (MistakeData){177, (Byte []){84, 59, 17, 89, 12, 12, 85, 9, 28, 159, 159, 159, 119}, 12, 187, 30, 92};

//: SVPullToRefreshView
MistakeData mEqualCoverPath = (MistakeData){113, (Byte []){34, 39, 33, 4, 29, 29, 37, 30, 35, 20, 23, 3, 20, 2, 25, 39, 24, 20, 6, 183}, 19, 226, 155, 135};

// __DEBUG__
// __CLOSE_PRINT__
//
// UIScrollView+Label.m
//
...
#import "UIScrollView+Label.h"
//: #import <objc/runtime.h>
#import <objc/runtime.h>

//: static CGFloat const NTESPullToRefreshViewHeight = 60;
static CGFloat const app_tapUrl = 60;
...
@property (nonatomic, strong) ContainerView *arrow;
...
- (void)setManager:(UIEdgeInsets)insets;
//: - (void)rotateArrow:(float)degrees hide:(BOOL)hide;
- (void)messageRotateHide:(float)degrees telecomWithinHide:(BOOL)hide;
...
static char mSessionIdent;

//: @implementation UIScrollView (NTESPullToRefresh)
@implementation UIScrollView (Label)
...
- (void)show:(void (^)(void))actionHandler addBy:(NTESPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        KissView *view = [[KissView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, app_tapUrl)];
...
- (void)appellative:(void (^)(void))actionHandler {
    ...
    [self show:actionHandler addBy:NTESPullToRefreshPositionTop];
}
...
- (void)dismiss {
    ...
    [self.pullToRefreshView past];
}

//: - (void)setPullToRefreshView:(NTESPullToRefreshView *)pullToRefreshView {
- (void)setPullToRefreshView:(KissView *)pullToRefreshView {
    //: [self willChangeValueForKey:@"SVPullToRefreshView"];
    [self willChangeValueForKey:StringFromMistakeData(&mEqualCoverPath)];
    //: objc_setAssociatedObject(self, &UIScrollViewPullToRefreshView,
    objc_setAssociatedObject(self, &mSessionIdent,
                             //: pullToRefreshView,
                             pullToRefreshView,
                             //: OBJC_ASSOCIATION_ASSIGN);
                             OBJC_ASSOCIATION_ASSIGN);
    //: [self didChangeValueForKey:@"SVPullToRefreshView"];
    [self didChangeValueForKey:StringFromMistakeData(&mEqualCoverPath)];
}

//: - (NTESPullToRefreshView *)pullToRefreshView {
- (KissView *)pullToRefreshView {
    //: return objc_getAssociatedObject(self, &UIScrollViewPullToRefreshView);
    return objc_getAssociatedObject(self, &mSessionIdent);
}
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (self.pullToRefreshView.isObserving) {
            ...
            [self removeObserver:self.pullToRefreshView forKeyPath:StringFromMistakeData(&m_termsContent)];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"contentSize"];
            [self removeObserver:self.pullToRefreshView forKeyPath:StringFromMistakeData(&dreamBelowMsg)];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"frame"];
            [self removeObserver:self.pullToRefreshView forKeyPath:StringFromMistakeData(&noti_whiteKey)];
            //: [self.pullToRefreshView resetScrollViewContentInset];
            [self.pullToRefreshView expiry];
    ...
    //: else {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            [self addObserver:self.pullToRefreshView forKeyPath:StringFromMistakeData(&m_termsContent) options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"contentSize" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:StringFromMistakeData(&dreamBelowMsg) options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"frame" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:StringFromMistakeData(&noti_whiteKey) options:NSKeyValueObservingOptionNew context:nil];
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, app_tapUrl);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(StringFromMistakeData(&user_pinId),),
                       //: NSLocalizedString(@"松开刷新...",),
                       NSLocalizedString(StringFromMistakeData(&appUpName),),
                       //: NSLocalizedString(@"加载中...",),
                       NSLocalizedString(StringFromMistakeData(&appChartMessage),),
                       //: nil];
                       nil];
    ...
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            ...
            if (self.isObserving) {
                ...
                [scrollView removeObserver:self forKeyPath:StringFromMistakeData(&m_termsContent)];
                //: [scrollView removeObserver:self forKeyPath:@"contentSize"];
                [scrollView removeObserver:self forKeyPath:StringFromMistakeData(&dreamBelowMsg)];
                //: [scrollView removeObserver:self forKeyPath:@"frame"];
                [scrollView removeObserver:self forKeyPath:StringFromMistakeData(&noti_whiteKey)];
                //: self.isObserving = NO;
                self.isObserving = NO;
            }
        }
            ...
            case NTESPullToRefreshStateStopped:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self messageRotateHide:0 telecomWithinHide:NO];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:NO];
                        [self messageRotateHide:(float)3.14159265358979323846264338327950288 telecomWithinHide:NO];
                        //: break;
                        break;
                }
                //: break;
            ...
            case NTESPullToRefreshStateLoading:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self messageRotateHide:0 telecomWithinHide:YES];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:YES];
                        [self messageRotateHide:(float)3.14159265358979323846264338327950288 telecomWithinHide:YES];
                        //: break;
                        break;
                }
                //: break;
...
- (void)setManager:(UIEdgeInsets)contentInset {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if([keyPath isEqualToString:StringFromMistakeData(&m_termsContent)])
        //: [self scrollViewDidScroll:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
        [self tip:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
    //: else if([keyPath isEqualToString:@"contentSize"]) {
    else if([keyPath isEqualToString:StringFromMistakeData(&dreamBelowMsg)]) {
        //: [self layoutSubviews];
        [self layoutSubviews];
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, app_tapUrl);
    }
    //: else if([keyPath isEqualToString:@"frame"])
    else if([keyPath isEqualToString:StringFromMistakeData(&noti_whiteKey)])
        //: [self layoutSubviews];
        [self layoutSubviews];
...
- (void)tip:(CGPoint)contentOffset {
...
- (ContainerView *)arrow {
    ...
    if(!_arrow) {
        ...
        _arrow = [[ContainerView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (UILabel *)titleLabel {
    ...
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(StringFromMistakeData(&kTingId),);
...
Byte *MistakeDataToByte(MistakeData *data) {
    if (data->may1 < 131) return data->rageCostly;
    for (int i = 0; i < data->withTitle; i++) {
        data->rageCostly[i] ^= data->underDate;
    }
    data->rageCostly[data->withTitle] = 0;
    data->may1 = 52;
	if (data->withTitle >= 2) {
		data->pinLiar = data->rageCostly[0];
		data->threadDevice = data->rageCostly[1];
	}
    return data->rageCostly;
}

NSString *StringFromMistakeData(MistakeData *data) {
    return [NSString stringWithUTF8String:(char *)MistakeDataToByte(data)];
}
