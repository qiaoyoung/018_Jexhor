...
@implementation UIImage (SearchedTeam)
//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)originImage:(NSString *)imageNameOrPath{
...
- (UIImage *)content
{
    ...
    CGFloat pixels = [[MemoryDevice color] supplement];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self cell:pixels];
...
- (UIImage *)cell: (CGFloat)suggestPixels
{
...
- (UIImage *)mean: (CGFloat)maxPixels
{
    ...
    return [self superfluous:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)superfluous:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
