...
// __CLOSE_PRINT__
//
//  NSDictionary+AttributeAdditions.m
//  Kitalker
//
...
#import "NSDictionary+AttributeAdditions.h"

//: @implementation NSDictionary (KIAdditions)
@implementation NSDictionary (AttributeAdditions)
...
- (BOOL)noneMaxDate:(id)key {
    //: return [[self objectForKey:key] boolValue];
    return [[self objectForKey:key] boolValue];
}

//: - (int)intValueForKey:(id)key {
- (int)valueLength:(id)key {
    //: return [[self objectForKey:key] intValue];
    return [[self objectForKey:key] intValue];
}

//: - (NSInteger)integerValueForKey:(id)key {
- (NSInteger)shouldItem:(id)key {
    //: return [[self objectForKey:key] integerValue];
    return [[self objectForKey:key] integerValue];
}

//: - (float)floatValueForKey:(id)key {
- (float)unwantedKey:(id)key {
...
- (NSArray *)appearUnwelcome:(id)aKey{
    //: if (!self || !aKey) {
    if (!self || !aKey) {
        //: return @[];
        return @[];
