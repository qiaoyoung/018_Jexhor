...
- (Byte *)SessionDataToCache:(Byte *)data {
    int legallyHostile = data[0];
    Byte nanCell = data[1];
    int collectionSession = data[2];
    for (int i = collectionSession; i < collectionSession + legallyHostile; i++) {
        int value = data[i] + nanCell;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[collectionSession + legallyHostile] = 0;
    return data + collectionSession;
}
...
//: gif
- (NSString *)show_laverData {
    /* static */ NSString *show_laverData = nil;
    if (!show_laverData) {
        Byte value[] = {3, 73, 4, 12, 30, 32, 29, 202};
        show_laverData = [self StringFromSessionData:value];
    }
    return show_laverData;
}

//: tiff
- (NSString *)main_lightId {
    /* static */ NSString *main_lightId = nil;
    if (!main_lightId) {
        Byte value[] = {4, 22, 8, 241, 227, 197, 122, 72, 94, 83, 80, 80, 226};
        main_lightId = [self StringFromSessionData:value];
    }
    return main_lightId;
}
...
- (NSString *)user_collectionIdent {
    /* static */ NSString *user_collectionIdent = nil;
    if (!user_collectionIdent) {
        Byte value[] = {4, 19, 10, 137, 238, 18, 30, 11, 88, 48, 87, 93, 82, 84, 76};
        user_collectionIdent = [self StringFromSessionData:value];
    }
    return user_collectionIdent;
}

//: png
- (NSString *)app_coldProfessionName {
    /* static */ NSString *app_coldProfessionName = nil;
    if (!app_coldProfessionName) {
        Byte value[] = {3, 91, 10, 250, 117, 245, 142, 92, 16, 215, 21, 19, 12, 119};
        app_coldProfessionName = [self StringFromSessionData:value];
    }
    return app_coldProfessionName;
}

//: yyyy-MM-dd HH:mm:ss
- (NSString *)main_lightTitle {
    /* static */ NSString *main_lightTitle = nil;
    if (!main_lightTitle) {
        Byte value[] = {19, 72, 12, 5, 156, 14, 27, 56, 135, 85, 82, 213, 49, 49, 49, 49, 229, 5, 5, 229, 28, 28, 216, 0, 0, 242, 37, 37, 242, 43, 43, 237};
        main_lightTitle = [self StringFromSessionData:value];
    }
    return main_lightTitle;
}
...
@implementation UIImage (AttributeAdditions)

//: - (UIImage *)flip:(BOOL)isHorizontal {
- (UIImage *)array:(BOOL)isHorizontal {
...
- (UIImage *)path:(CGFloat)width {
    ...
    return [self key:width dateHeight:height];
}
...
- (UIImage *)contentHeight:(CGFloat)heigh {
    ...
    return [self key:width dateHeight:heigh];
}
...
- (UIImage *)reply {
    ...
    [df setDateFormat:[[SessionData sharedInstance] main_lightTitle]];
    ...
    return [self snapPoint:dateString
               //: textColor:[UIColor blackColor]
               size:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    title:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   current:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
+ (NSString *) pullMessage:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return [[SessionData sharedInstance] user_collectionIdent];
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return [[SessionData sharedInstance] app_coldProfessionName];
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return [[SessionData sharedInstance] show_laverData];
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return [[SessionData sharedInstance] main_lightId];
    }
    //: return @"";
    return @"";
}
...
CGFloat messageRadians(CGFloat degrees) {return degrees * 3.14159265358979323846264338327950288 / 180;};
//: CGFloat KKRadiansToDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
CGFloat sessionVideo(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
...
- (UIImage *)must:(CGFloat)degrees{
    ...
    CGAffineTransform t = CGAffineTransformMakeRotation(messageRadians(degrees));
    ...
    CGContextRotateCTM(bitmap, messageRadians(degrees));
...
- (UIImage*)sizeSend:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
- (UIImage *)iconWithBubble:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self compress:maxLength*1024];
...
-(NSData *)compress:(double)maxLength{
    // Compress by quality
