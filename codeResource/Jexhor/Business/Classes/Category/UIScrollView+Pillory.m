...
@interface JammedData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation JammedData

+ (instancetype)sharedInstance {
    static JammedData *instance = nil;
...
- (Byte *)JammedDataToCache:(Byte *)data {
    int distribute = data[0];
    Byte kaAppear = data[1];
    int balancePlanet = data[2];
    for (int i = balancePlanet; i < balancePlanet + distribute; i++) {
        int value = data[i] + kaAppear;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[balancePlanet + distribute] = 0;
    return data + balancePlanet;
}

- (NSString *)StringFromJammedData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self JammedDataToCache:data]];
}

//: 下拉加载...
- (NSString *)dream_parentShowData {
    /* static */ NSString *dream_parentShowData = nil;
    if (!dream_parentShowData) {
        Byte value[] = {15, 22, 12, 40, 105, 227, 37, 181, 219, 120, 208, 238, 206, 162, 117, 208, 117, 115, 207, 116, 138, 210, 167, 167, 24, 24, 24, 148};
        dream_parentShowData = [self StringFromJammedData:value];
    }
    return dream_parentShowData;
}

//: contentOffset
- (NSString *)appToeName {
    /* static */ NSString *appToeName = nil;
    if (!appToeName) {
        Byte value[] = {13, 11, 7, 142, 47, 78, 151, 88, 100, 99, 105, 90, 99, 105, 68, 91, 91, 104, 90, 105, 163};
        appToeName = [self StringFromJammedData:value];
    }
    return appToeName;
}

//: Pull to refresh...
- (NSString *)dream_pollutionName {
    /* static */ NSString *dream_pollutionName = nil;
    if (!dream_pollutionName) {
        Byte value[] = {18, 54, 3, 26, 63, 54, 54, 234, 62, 57, 234, 60, 47, 48, 60, 47, 61, 50, 248, 248, 248, 128};
        dream_pollutionName = [self StringFromJammedData:value];
    }
    return dream_pollutionName;
}

//: contentSize
- (NSString *)app_pressureName {
    /* static */ NSString *app_pressureName = nil;
    if (!app_pressureName) {
        Byte value[] = {11, 68, 12, 96, 184, 217, 159, 212, 101, 115, 112, 225, 31, 43, 42, 48, 33, 42, 48, 15, 37, 54, 33, 35};
        app_pressureName = [self StringFromJammedData:value];
    }
    return app_pressureName;
}

//: 加载中...
- (NSString *)app_controlValue {
    /* static */ NSString *app_controlValue = nil;
    if (!app_controlValue) {
        Byte value[] = {12, 46, 13, 205, 17, 166, 161, 176, 22, 169, 199, 114, 156, 183, 92, 114, 186, 143, 143, 182, 138, 127, 0, 0, 0, 39};
        app_controlValue = [self StringFromJammedData:value];
    }
    return app_controlValue;
}

//: SVPullToRefreshView
- (NSString *)mDrugName {
    /* static */ NSString *mDrugName = nil;
    if (!mDrugName) {
        Byte value[] = {19, 80, 5, 93, 67, 3, 6, 0, 37, 28, 28, 4, 31, 2, 21, 22, 34, 21, 35, 24, 6, 25, 21, 39, 130};
        mDrugName = [self StringFromJammedData:value];
    }
    return mDrugName;
}
...
- (NSString *)appEndContent {
    /* static */ NSString *appEndContent = nil;
    if (!appEndContent) {
        Byte value[] = {5, 34, 13, 90, 228, 164, 175, 184, 145, 35, 225, 233, 247, 68, 80, 63, 75, 67, 179};
        appEndContent = [self StringFromJammedData:value];
    }
    return appEndContent;
}

//: 松开刷新...
- (NSString *)appMaxValue {
    /* static */ NSString *appMaxValue = nil;
    if (!appMaxValue) {
        Byte value[] = {15, 38, 9, 72, 36, 26, 99, 33, 18, 192, 119, 152, 191, 150, 90, 191, 98, 145, 192, 112, 138, 8, 8, 8, 124};
        appMaxValue = [self StringFromJammedData:value];
    }
    return appMaxValue;
}
...
#import "UIScrollView+Pillory.h"
//: #import <objc/runtime.h>
#import <objc/runtime.h>

//: static CGFloat const NTESPullToRefreshViewHeight = 60;
static CGFloat const m_valueTitle = 60;

//: @interface NTESPullToRefreshArrow : UIView
@interface RecordBarView : UIView
...
@property (nonatomic, strong) RecordBarView *arrow;
...
- (void)setSuccess:(UIEdgeInsets)insets;
//: - (void)rotateArrow:(float)degrees hide:(BOOL)hide;
- (void)shaft:(float)degrees standard:(BOOL)hide;
...
#pragma mark - UIScrollView (Pillory)


//: static char UIScrollViewPullToRefreshView;
static char user_unitData;

//: @implementation UIScrollView (NTESPullToRefresh)
@implementation UIScrollView (Pillory)
...
- (void)margin:(void (^)(void))actionHandler lengthName:(NTESPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        ActionView *view = [[ActionView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, m_valueTitle)];
...
- (void)userExtra:(void (^)(void))actionHandler {
    ...
    [self margin:actionHandler lengthName:NTESPullToRefreshPositionTop];
}
...
- (void)loadIn {
    ...
    [self.pullToRefreshView clear];
}

//: - (void)setPullToRefreshView:(NTESPullToRefreshView *)pullToRefreshView {
- (void)setPullToRefreshView:(ActionView *)pullToRefreshView {
    //: [self willChangeValueForKey:@"SVPullToRefreshView"];
    [self willChangeValueForKey:[[JammedData sharedInstance] mDrugName]];
    //: objc_setAssociatedObject(self, &UIScrollViewPullToRefreshView,
    objc_setAssociatedObject(self, &user_unitData,
                             //: pullToRefreshView,
                             pullToRefreshView,
                             //: OBJC_ASSOCIATION_ASSIGN);
                             OBJC_ASSOCIATION_ASSIGN);
    //: [self didChangeValueForKey:@"SVPullToRefreshView"];
    [self didChangeValueForKey:[[JammedData sharedInstance] mDrugName]];
}

//: - (NTESPullToRefreshView *)pullToRefreshView {
- (ActionView *)pullToRefreshView {
    //: return objc_getAssociatedObject(self, &UIScrollViewPullToRefreshView);
    return objc_getAssociatedObject(self, &user_unitData);
}
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (self.pullToRefreshView.isObserving) {
            ...
            [self removeObserver:self.pullToRefreshView forKeyPath:[[JammedData sharedInstance] appToeName]];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"contentSize"];
            [self removeObserver:self.pullToRefreshView forKeyPath:[[JammedData sharedInstance] app_pressureName]];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"frame"];
            [self removeObserver:self.pullToRefreshView forKeyPath:[[JammedData sharedInstance] appEndContent]];
            //: [self.pullToRefreshView resetScrollViewContentInset];
            [self.pullToRefreshView member];
    ...
    //: else {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            [self addObserver:self.pullToRefreshView forKeyPath:[[JammedData sharedInstance] appToeName] options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"contentSize" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:[[JammedData sharedInstance] app_pressureName] options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"frame" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:[[JammedData sharedInstance] appEndContent] options:NSKeyValueObservingOptionNew context:nil];
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, m_valueTitle);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString([[JammedData sharedInstance] dream_parentShowData],),
                       //: NSLocalizedString(@"松开刷新...",),
                       NSLocalizedString([[JammedData sharedInstance] appMaxValue],),
                       //: NSLocalizedString(@"加载中...",),
                       NSLocalizedString([[JammedData sharedInstance] app_controlValue],),
                       //: nil];
                       nil];
    ...
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            ...
            if (self.isObserving) {
                ...
                [scrollView removeObserver:self forKeyPath:[[JammedData sharedInstance] appToeName]];
                //: [scrollView removeObserver:self forKeyPath:@"contentSize"];
                [scrollView removeObserver:self forKeyPath:[[JammedData sharedInstance] app_pressureName]];
                //: [scrollView removeObserver:self forKeyPath:@"frame"];
                [scrollView removeObserver:self forKeyPath:[[JammedData sharedInstance] appEndContent]];
                //: self.isObserving = NO;
                self.isObserving = NO;
            }
        }
            ...
            case NTESPullToRefreshStateStopped:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self shaft:0 standard:NO];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:NO];
                        [self shaft:(float)3.14159265358979323846264338327950288 standard:NO];
                        //: break;
                        break;
                }
                //: break;
            ...
            case NTESPullToRefreshStateLoading:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self shaft:0 standard:YES];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:YES];
                        [self shaft:(float)3.14159265358979323846264338327950288 standard:YES];
                        //: break;
                        break;
                }
                //: break;
...
- (void)setSuccess:(UIEdgeInsets)contentInset {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if([keyPath isEqualToString:[[JammedData sharedInstance] appToeName]])
        //: [self scrollViewDidScroll:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
        [self array:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
    //: else if([keyPath isEqualToString:@"contentSize"]) {
    else if([keyPath isEqualToString:[[JammedData sharedInstance] app_pressureName]]) {
        //: [self layoutSubviews];
        [self layoutSubviews];
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, m_valueTitle);
    }
    //: else if([keyPath isEqualToString:@"frame"])
    else if([keyPath isEqualToString:[[JammedData sharedInstance] appEndContent]])
        //: [self layoutSubviews];
        [self layoutSubviews];
...
- (void)array:(CGPoint)contentOffset {
...
- (RecordBarView *)arrow {
    ...
    if(!_arrow) {
        ...
        _arrow = [[RecordBarView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (UILabel *)titleLabel {
    ...
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString([[JammedData sharedInstance] dream_pollutionName],);
...
#pragma mark - RecordBarView

//: @implementation NTESPullToRefreshArrow
@implementation RecordBarView
