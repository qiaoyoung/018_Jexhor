...
@interface GivingData : NSObject

@end

@implementation GivingData

+ (Byte *)GivingDataToCache:(Byte *)data {
    int changeSever = data[0];
    int callus = data[1];
    for (int i = 0; i < changeSever / 2; i++) {
        int begin = callus + i;
        int end = callus + changeSever - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[callus + changeSever] = 0;
    return data + callus;
}

+ (NSString *)StringFromGivingData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self GivingDataToCache:data]];
}  

//: content
+ (NSString *)user_distributeValue {
    /* static */ NSString *user_distributeValue = nil;
    if (!user_distributeValue) {
        Byte value[] = {7, 6, 201, 14, 177, 164, 116, 110, 101, 116, 110, 111, 99, 206};
        user_distributeValue = [self StringFromGivingData:value];
    }
    return user_distributeValue;
}

//: 你收到了一个白板请求
+ (NSString *)m_adultQuickTitle {
    /* static */ NSString *m_adultQuickTitle = nil;
    if (!m_adultQuickTitle) {
        Byte value[] = {30, 12, 232, 144, 58, 203, 172, 91, 143, 255, 191, 99, 130, 177, 230, 183, 175, 232, 191, 157, 230, 189, 153, 231, 170, 184, 228, 128, 184, 228, 134, 186, 228, 176, 136, 229, 182, 148, 230, 160, 189, 228, 66};
        m_adultQuickTitle = [self StringFromGivingData:value];
    }
    return m_adultQuickTitle;
}

//: message.wav
+ (NSString *)showComplainData {
    /* static */ NSString *showComplainData = nil;
    if (!showComplainData) {
        Byte value[] = {11, 7, 224, 181, 47, 81, 81, 118, 97, 119, 46, 101, 103, 97, 115, 115, 101, 109, 187};
        showComplainData = [self StringFromGivingData:value];
    }
    return showComplainData;
}

//: 向你发起了一个白板请求
+ (NSString *)dream_zineBotText {
    /* static */ NSString *dream_zineBotText = nil;
    if (!dream_zineBotText) {
        Byte value[] = {33, 10, 4, 87, 54, 14, 105, 230, 158, 49, 130, 177, 230, 183, 175, 232, 191, 157, 230, 189, 153, 231, 170, 184, 228, 128, 184, 228, 134, 186, 228, 183, 181, 232, 145, 143, 229, 160, 189, 228, 145, 144, 229, 184};
        dream_zineBotText = [self StringFromGivingData:value];
    }
    return dream_zineBotText;
}
...
@property (nonatomic,strong) CountNotifier *notifier;

//: @end
@end

//: @implementation NTESNotificationCenter
@implementation ClickCenter

//: + (instancetype)sharedCenter
+ (instancetype)at
{
    //: static NTESNotificationCenter *instance = nil;
    static ClickCenter *instance = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESNotificationCenter alloc] init];
        instance = [[ClickCenter alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (instancetype)init {
    ...
    if(self) {
        ...
        NSString *voicePath = [[[ButtonManager examineedManager] quick] stringByAppendingPathComponent:[NSString stringWithFormat:[GivingData showComplainData]]];
        ...
        _notifier = [[CountNotifier alloc] init];
...
- (void)onRecvMessages:(NSArray *)recvMessages
{
    ...
    NSArray *messages = [self to:recvMessages];
    //: if (messages.count)
    if (messages.count)
    {
...
- (void)optionTip
{
    ...
    UINavigationController *nav = [ManagerBarController can].selectedViewController;
    //: BOOL needPlay = YES;
    BOOL needPlay = YES;
...
- (void)messageShow:(NSArray<NIMMessage *> *)messages
{
    ...
    for (NIMMessage *message in messages) {
        ...
        if ([message.apnsMemberOption.userIds containsObject:me]) {
            ...
            [SessionUtil save:session viewType:NTESRecentSessionMarkTypeAt];
            //: return;
            return;
        }
    }
...
- (NSArray *)to:(NSArray *)messages
{
    ...
    for (NIMMessage *message in messages)
    {
        ...
        if ([self bar:message] && ![self counterfoil:message])
        {
            //: [[NIMSDK sharedSDK].conversationManager deleteMessage:message];
            [[NIMSDK sharedSDK].conversationManager deleteMessage:message];
            //: [self.currentSessionViewController uiDeleteMessage:message];
            [self.currentnessController image:message];
            //: continue;
            continue;
        }
...
- (BOOL)bar:(NIMMessage *)message
{
    ...
    if ([object isKindOfClass:[NIMCustomObject class]] && [object.attachment isKindOfClass:[NameButton class]])
    {
        //: return YES;
        return YES;
    }
...
- (BOOL)counterfoil:(NIMMessage *)message
{
    //: NIMCustomObject *object = message.messageObject;
    NIMCustomObject *object = message.messageObject;
...
- (void)onRecvRevokeMessageNotification:(NIMRevokeMessageNotification *)notification
{
    ...
    NIMMessage *tipMessage = [ViewLine menu:[SessionUtil groundBubble:notification]
                                                    //: revokeAttach:notification.attach
                                                    range:notification.attach
                                               //: revokeCallbackExt:notification.callbackExt];
                                               bringHome:notification.callbackExt];
    ...
    ManagerBarController *tabVC = [ManagerBarController can];
    //: UINavigationController *nav = tabVC.selectedViewController;
    UINavigationController *nav = tabVC.selectedViewController;

    //: for (NTESSessionViewController *vc in nav.viewControllers) {
    for (DisplayAccountViewController *vc in nav.viewControllers) {
        ...
        if ([dict isKindOfClass:[NSDictionary class]])
        {
            ...
            switch ([dict cell:@"id"]) {
                ...
                case (2):{
                    ...
                    NSString *content = [dict table:[GivingData user_distributeValue]];
                    //: [self makeToast:content];
                    [self exaggerateCustom:content];
                }
                    //: break;
                    break;
                //: case (3):{
...
- (void)recent:(UInt64)callID
              //: by:(NSString *)user
              join:(NSString *)user
{
    //: [_notifier stop];
    [_notifier randomStop];
}

//: - (void)onRTSRequest:(NSString *)sessionID
- (void)request:(NSString *)sessionID
                //: from:(NSString *)caller
                success:(NSString *)caller
            //: services:(NSUInteger)types
            name:(NSUInteger)types
             //: message:(NSString *)info
             location:(NSString *)info
{
...
- (void)onReceiveBroadcastMessage:(NIMBroadcastMessage *)broadcastMessage
{
    ...
    [self exaggerateCustom:broadcastMessage.content];
}
...
- (NSString *)womanTitle:(NSString *)caller
{
    ...
    if ([info.showName length])
    {
        ...
        text = [NSString stringWithFormat:@"%@%@",
                ...
                [GivingData dream_zineBotText].shouldLocalized];
    }
    //: return text;
    return text;
}
...
- (BOOL)lineOpen:(NSString *)callerId
{
    //退后台后 APP 存活，然后收到通知
    //: BOOL should = YES;
    BOOL should = YES;
...
- (void)exaggerateCustom:(NSString *)content
{
    //: [[NTESMainTabController instance].selectedViewController.view makeToast:content duration:2.0 position:CSToastPositionCenter];
    [[ManagerBarController can].selectedViewController.view makeToast:content duration:2.0 position:CSToastPositionCenter];
}
