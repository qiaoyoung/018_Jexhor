...
@implementation HeatherShow

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)button:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self ting:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self soapBubbleImage:filepath];
        //: }else {
        }else {
...
+ (void)trademark:(NSString *)filepath file:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self button:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)soapBubbleImage:(NSString *)filePath
{
...
+ (unsigned long long)ting:(NSString *)filepath
{
