...
@implementation BringHomeConverter
//: + (NTESPinyinConverter *)sharedInstance
+ (BringHomeConverter *)common
{
    //: static NTESPinyinConverter *instance = nil;
    static BringHomeConverter *instance = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESPinyinConverter alloc] init];
        instance = [[BringHomeConverter alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (NSString *)deviceClose: (NSString *)source
{
    //: if ([source length] == 0)
    if ([source length] == 0)
    {
