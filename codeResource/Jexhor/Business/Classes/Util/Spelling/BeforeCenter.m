...
#import "BringHomeConverter.h"

//: @implementation SpellingUnit
@implementation DeviceUnit
...
- (DeviceUnit *)unwelcome:(NSString *)source
{
    //: if ([source length] == 0)
    if ([source length] == 0)
    {
    ...
    DeviceUnit *spellingUnit = nil;
    //: @synchronized(self)
    @synchronized(self)
    {
        //: SpellingUnit *unit = [_spellingCache objectForKey:source];
        DeviceUnit *unit = [_spellingCache objectForKey:source];
        //: if (!unit)
        if (!unit)
        {
            //: unit = [self calcSpellingOfString:source];
            unit = [self field:source];
...
- (NSString *)phonation:(NSString *)input
{
    //: SpellingUnit *unit = [self spellingForString:input];
    DeviceUnit *unit = [self unwelcome:input];
...
- (DeviceUnit *)field:(NSString *)source
{
    ...
    for (NSInteger i = 0; i < [source length]; i++)
    {
        ...
        NSString *pinyin = [[BringHomeConverter common] deviceClose:word];
    ...
    DeviceUnit *unit = [[DeviceUnit alloc]init];
