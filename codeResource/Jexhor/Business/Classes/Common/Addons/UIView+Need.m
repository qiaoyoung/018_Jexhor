...
#import "UIView+Need.h"
//: #import "SwizzlingDefine.h"
#import "SwizzlingDefine.h"

//: @implementation UIView (Swizzling)
@implementation UIView (Need)
...
- (UIView *)titlePress:(CGPoint)point button:(UIEvent *)event
{
    //: UIView *view = [self swizzling_hitTest:point withEvent:event];
    UIView *view = [self titlePress:point button:event];
    //: if (view) {
    if (view) {
    }
    //: return view;
...
- (void)lengthFrame:(CGRect)frame
{
    //: [self swizzling_setFrame:frame];
    [self lengthFrame:frame];
