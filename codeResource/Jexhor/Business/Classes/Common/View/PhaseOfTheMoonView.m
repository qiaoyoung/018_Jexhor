...
@interface VoiceView : UIScrollView
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(qualifyChild)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate qualifyChild];
    }
...
- (void)readControls
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[VoiceView alloc]initWithFrame:self.bounds];
...
- (NSInteger)path:(NSInteger)value image:(NSInteger)min orientation:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)progress:(NSInteger)currentPage
{
    ...
    NSInteger first = [self path:currentPage - 1 image:0 orientation:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self path:currentPage + 1 image:0 orientation:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(teamValue:addWith:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource teamValue:self addWith:index];
...
- (void)plantOrgan
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(ranges:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource ranges:self];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(minorLeagueTeam:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate minorLeagueTeam:self];
    }
}
...
- (void)from
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(appear:red:isTitle:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate appear:self
                                //: currentIndex:_currentPage
                                red:_currentPage
                                  //: totolPages:[_pages count]];
                                  isTitle:[_pages count]];
    }
}
...
- (BOOL)recognizer:(UIGestureRecognizer *)gestureRecognizer domain:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
