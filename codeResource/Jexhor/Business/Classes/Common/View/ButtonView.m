...
@interface DeathScrollView : UIScrollView
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(omitManager)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate omitManager];
    }
...
- (void)managerTable
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[DeathScrollView alloc]initWithFrame:self.bounds];
...
- (void)digitiserAndPage: (NSInteger)page
{
...
- (NSInteger)duringNeed:(NSInteger)value team:(NSInteger)min lightGreen:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)unwelcomeMonth:(NSInteger)currentPage
{
    ...
    NSInteger first = [self duringNeed:currentPage - 1 team:0 lightGreen:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self duringNeed:currentPage + 1 team:0 lightGreen:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(text:with:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource text:self with:index];
...
- (void)share
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(elites:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource elites:self];
    }
...
- (void)container
{
    ...
    [self unwelcomeMonth:_currentPage];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(placed:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate placed:self];
    }
}
...
- (void)toAGreaterExtent
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(create:cell:with:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate create:self
...
@implementation DeathScrollView
//fix: swipe to delete in a UITableView which is embeded in a UIScrollView
//: - (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(nonnull UIGestureRecognizer *)otherGestureRecognizer
- (BOOL)success:(UIGestureRecognizer *)gestureRecognizer build:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
