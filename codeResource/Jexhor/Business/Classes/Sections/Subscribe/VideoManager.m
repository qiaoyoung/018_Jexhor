...
NSString *const noti_videoUnitMsg = @"net_state";

//: NSString *const NTESSubscribeOnlineState = @"online_state";
NSString *const kNamePath = @"online_state";
...
@interface VideoManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
- (void)manager:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self at];
...
- (void)enableline
{
    ...
    NSDictionary *ext = @{
                            ...
                            noti_videoUnitMsg : @([MemoryDevice color].purl),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            kNamePath : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext twine]];
...
- (void)doingLine
{
    ...
    [_subscribeIds addObjectsFromArray:self.asStandard.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.sumeraction.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self need:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)need:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.asStandard containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.sumeraction containsObject:recentSession.session.sessionId]) {
