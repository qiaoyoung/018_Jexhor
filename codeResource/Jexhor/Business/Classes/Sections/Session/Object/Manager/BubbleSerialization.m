...
@interface EmpireSkyData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation EmpireSkyData

+ (instancetype)sharedInstance {
    static EmpireSkyData *instance = nil;
...
- (Byte *)EmpireSkyDataToCache:(Byte *)data {
    int departTermImage = data[0];
    int stress = data[1];
    for (int i = 0; i < departTermImage / 2; i++) {
        int begin = stress + i;
        int end = stress + departTermImage - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[stress + departTermImage] = 0;
    return data + stress;
}

- (NSString *)StringFromEmpireSkyData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self EmpireSkyDataToCache:data]];
}  

//: YYYYMMddhhmmssSSS
- (NSString *)kAbortFormat {
    /* static */ NSString *kAbortFormat = nil;
    if (!kAbortFormat) {
        Byte value[] = {17, 12, 140, 226, 21, 65, 124, 18, 65, 146, 106, 221, 83, 83, 83, 115, 115, 109, 109, 104, 104, 100, 100, 77, 77, 89, 89, 89, 89, 201};
        kAbortFormat = [self StringFromEmpireSkyData:value];
    }
    return kAbortFormat;
}

//: ntes.message.serialization
- (NSString *)dream_choiceTitle {
    /* static */ NSString *dream_choiceTitle = nil;
    if (!dream_choiceTitle) {
        Byte value[] = {26, 4, 184, 172, 110, 111, 105, 116, 97, 122, 105, 108, 97, 105, 114, 101, 115, 46, 101, 103, 97, 115, 115, 101, 109, 46, 115, 101, 116, 110, 105};
        dream_choiceTitle = [self StringFromEmpireSkyData:value];
    }
    return dream_choiceTitle;
}

//: .txt
- (NSString *)dreamUnderPromotionContent {
    /* static */ NSString *dreamUnderPromotionContent = nil;
    if (!dreamUnderPromotionContent) {
        Byte value[] = {4, 2, 116, 120, 116, 46, 5};
        dreamUnderPromotionContent = [self StringFromEmpireSkyData:value];
    }
    return dreamUnderPromotionContent;
}
...
static dispatch_queue_t maxUser()
{
...
@implementation BubbleSerialization
    ...
    dispatch_async(maxUser(), ^{
        ...
        if (!headerRowData) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                error = [NSError errorWithDomain:[[EmpireSkyData sharedInstance] dream_choiceTitle] code:1000 userInfo:nil];
                //: if (completion) {
                if (completion) {
        ...
        NSArray *messageRowData = [self partView:messages];
        //: if (messageRowData.count == 0) {
        if (messageRowData.count == 0) {
            //: dispatch_async(dispatch_get_main_queue(), ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                //: error = [NSError errorWithDomain:@"ntes.message.serialization" code:1001 userInfo:nil];
                error = [NSError errorWithDomain:[[EmpireSkyData sharedInstance] dream_choiceTitle] code:1001 userInfo:nil];
                //: if (completion) {
                if (completion) {
                    //: completion(error, nil);
                    completion(error, nil);
        ...
        NSString *file = [self doMessage:datas bar:encrypt green:password];
        //: if (file == nil) {
        if (file == nil) {
            //: error = [NSError errorWithDomain:@"ntes.message.serialization" code:1002 userInfo:nil];
            error = [NSError errorWithDomain:[[EmpireSkyData sharedInstance] dream_choiceTitle] code:1002 userInfo:nil];
            //: dispatch_async(dispatch_get_main_queue(), ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                //: if (completion) {
                if (completion) {
        ...
        NSString *md5 = [HeatherShow button:file];
        //: if (!md5) {
        if (!md5) {
            //delete
            ...
            error = [NSError errorWithDomain:[[EmpireSkyData sharedInstance] dream_choiceTitle] code:1003 userInfo:nil];
            //: dispatch_async(dispatch_get_main_queue(), ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                //: if (completion) {
                if (completion) {
        ...
        BlockTitleInfo *info = [[BlockTitleInfo alloc] init];
        //: info.filePath = file;
        info.filePath = file;
        //: info.md5 = md5;
        info.md5 = md5;
...
- (void)sendFind:(NSString *)filePath
    ...
    dispatch_async(maxUser(), ^{
        ...
        NSMutableArray *ret = [self construeFromRead:filePath tableFrom:encrypt trademark:password];
        //: if (!ret) {
        if (!ret) {
            //: error = [NSError errorWithDomain:@"ntes.message.serialization" code:1003 userInfo:nil];
            error = [NSError errorWithDomain:[[EmpireSkyData sharedInstance] dream_choiceTitle] code:1003 userInfo:nil];
...
- (NSString *)targetFile {
    ...
    formatter.dateFormat = [[EmpireSkyData sharedInstance] kAbortFormat];
    //: NSString *fileName = [[formatter stringFromDate:date] stringByAppendingString:@".txt"];
    NSString *fileName = [[formatter stringFromDate:date] stringByAppendingString:[[EmpireSkyData sharedInstance] dreamUnderPromotionContent]];
    //: return [NTESFileLocationHelper filepathForMergeForwardFile:fileName];
    return [BarVideo container:fileName];
}
...
- (NSData *)to:(NSInteger)count {
    ...
    MemoryMain *header = [MemoryMain initWithMessage];
...
- (NSMutableArray <NSData *>*)partView:(NSArray <NIMMessage *>*)messages {
...
- (NSString *)doMessage:(NSMutableArray *)datas
                          //: encrypt:(BOOL)encrypt
                          bar:(BOOL)encrypt
                         //: password:(NSString *)password {
                         green:(NSString *)password {
    ...
    if (temp.length != 0) {
        ...
        if (encrypt) {
            ...
            NSData *encryptData = [temp wire:password];
...
- (NSString *)more:(NSString *)file
                     //: encrypt:(BOOL)encrypt
                     session:(BOOL)encrypt
                    //: password:(NSString *)password{
    ...
    while (!feof(fp)) {
        ...
        if (encrypt) {
            ...
            NSData *decryptData = [fileData latchkey:password];
...
- (NSMutableArray <NIMMessage *>*)construeFromRead:(NSString *)file
                                               //: encrypt:(BOOL)encrypt
                                               tableFrom:(BOOL)encrypt
                                              //: password:(NSString *)password {
                                              trademark:(NSString *)password {

    //: NSMutableArray *ret = nil;
    NSMutableArray *ret = nil;
    //: NSString *fileString = [self readFileString:file encrypt:encrypt password:password];
    NSString *fileString = [self more:file session:encrypt by:password];
    //: NSArray *subStrings = [fileString componentsSeparatedByString:@"\n"];
    NSArray *subStrings = [fileString componentsSeparatedByString:@"\n"];
    //: NTESMigrateHeader *header = nil;
    MemoryMain *header = nil;
    //: NSInteger count = 0;
    NSInteger count = 0;
    ...
    for (int i = 0; i < subStrings.count; i++) {
        ...
        if (i == 0) {
            ...
            header = [MemoryMain initWithLimit:headerData];
            //: if (header == nil) {
            if (header == nil) {
                //: break;
                break;
