...
- (void)behindDisable{
    ...
    label.font = [UIFont systemFontOfSize:showPanTitle];
    //: label.text = content;
    label.text = content;
    ...
    CGSize labelSize = [label sizeThatFits:CGSizeMake(showPointChangeValue * [UIScreen mainScreen].bounds.size.width / 320, 1.7976931348623157e+308)];
    //: CGFloat labelHeight = ((SearchCellContentMinHeight) > (labelSize.height) ? (SearchCellContentMinHeight) : (labelSize.height));
    CGFloat labelHeight = ((dreamItemLabelName) > (labelSize.height) ? (dreamItemLabelName) : (labelSize.height));
    //: CGFloat height = labelHeight + SearchCellContentTop + SearchCellContentBottom;
    CGFloat height = labelHeight + dreamMemberName + m_cellTitle;
    //: _uiHeight = height;
    _uiHeight = height;
}
