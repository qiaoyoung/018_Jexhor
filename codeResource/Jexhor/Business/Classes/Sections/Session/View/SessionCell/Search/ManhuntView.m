...
@property (nonatomic,strong) TouchShakedownObject *object;

//: @end
@end

//: @implementation NTESSearchMessageContentCell
@implementation ManhuntView
...
- (void)shadowRecord:(TouchShakedownObject *)object{
    ...
    [self.avatar everyBarImage:avatarURL messageColor:info.avatarImage];
    //: self.titleLabel.text = info.showName;
    self.titleLabel.text = info.showName;
    //: self.contentLabel.text = message.text;
    self.contentLabel.text = message.text;
    //: self.timeLabel.text = [NTESSessionUtil showTime:message.timestamp showDetail:YES];
    self.timeLabel.text = [ReadUtil betweenNet:message.timestamp content:YES];
    ...
    self.contentLabel.size = [self.contentLabel sizeThatFits:CGSizeMake(app_userMsg * [UIScreen mainScreen].bounds.size.width / 320, 1.7976931348623157e+308)];
    ...
    self.contentLabel.height = ((mainVersionTitle) > (self.contentLabel.height) ? (mainVersionTitle) : (self.contentLabel.height));
