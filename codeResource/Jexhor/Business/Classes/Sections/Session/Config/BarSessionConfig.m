...
@interface BarSessionConfig ()

//: @property (nonatomic,strong) NTESThreadDataSourceProvider *provider;
@property (nonatomic,strong) PushProvider *provider;
...
@implementation BarSessionConfig
...
- (instancetype)initWithTable:(NIMMessage *)message
{
    ...
    if (self)
    {
        ...
        _provider = [[PushProvider alloc] init];
...
- (void)imaginationImage:(NIMMessage *)firstMessage drawPath:(NIMKitDataProvideHandler)handler
{
    ...
    BOOL enablePullCloudMessages = [[SettingImage name] sub];
