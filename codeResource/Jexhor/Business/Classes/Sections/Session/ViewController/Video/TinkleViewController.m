...
- (Byte *)WhichDataToCache:(Byte *)data {
    int ethnicTeam = data[0];
    int adviserAlly = data[1];
    for (int i = 0; i < ethnicTeam / 2; i++) {
        int begin = adviserAlly + i;
        int end = adviserAlly + ethnicTeam - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[adviserAlly + ethnicTeam] = 0;
    return data + adviserAlly;
}

- (NSString *)StringFromWhichData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self WhichDataToCache:data]];
}  

//: ic_pic_share
- (NSString *)show_storagePath {
    /* static */ NSString *show_storagePath = nil;
    if (!show_storagePath) {
        Byte value[] = {12, 5, 63, 12, 102, 101, 114, 97, 104, 115, 95, 99, 105, 112, 95, 99, 105, 78};
        show_storagePath = [self StringFromWhichData:value];
    }
    return show_storagePath;
}

//: player_play
- (NSString *)userUpId {
    /* static */ NSString *userUpId = nil;
    if (!userUpId) {
        Byte value[] = {11, 4, 250, 136, 121, 97, 108, 112, 95, 114, 101, 121, 97, 108, 112, 217};
        userUpId = [self StringFromWhichData:value];
    }
    return userUpId;
}

//: player_push
- (NSString *)mainCancelTitle {
    /* static */ NSString *mainCancelTitle = nil;
    if (!mainCancelTitle) {
        Byte value[] = {11, 4, 207, 87, 104, 115, 117, 112, 95, 114, 101, 121, 97, 108, 112, 114};
        mainCancelTitle = [self StringFromWhichData:value];
    }
...
- (NSString *)showAllyTitle {
    ...
    if (!showAllyTitle) {
        Byte value[] = {7, 6, 206, 161, 148, 150, 102, 102, 102, 102, 102, 102, 35, 94};
        showAllyTitle = [self StringFromWhichData:value];
    }
    return showAllyTitle;
}

//: 1.5x
- (NSString *)notiEconomicMessage {
    /* static */ NSString *notiEconomicMessage = nil;
    if (!notiEconomicMessage) {
        Byte value[] = {4, 5, 114, 95, 244, 120, 53, 46, 49, 120};
        notiEconomicMessage = [self StringFromWhichData:value];
    }
    return notiEconomicMessage;
}

//: error
- (NSString *)app_aspectId {
    /* static */ NSString *app_aspectId = nil;
    if (!app_aspectId) {
        Byte value[] = {5, 6, 138, 242, 105, 170, 114, 111, 114, 114, 101, 91};
        app_aspectId = [self StringFromWhichData:value];
    }
    return app_aspectId;
}
...
//: ic_close_w
- (NSString *)notiToKey {
    /* static */ NSString *notiToKey = nil;
    if (!notiToKey) {
        Byte value[] = {10, 4, 215, 23, 119, 95, 101, 115, 111, 108, 99, 95, 99, 105, 34};
        notiToKey = [self StringFromWhichData:value];
    }
    return notiToKey;
}

//: ic_pic_save
- (NSString *)app_abortDepartBornIdent {
    /* static */ NSString *app_abortDepartBornIdent = nil;
    if (!app_abortDepartBornIdent) {
        Byte value[] = {11, 10, 55, 248, 177, 200, 132, 155, 57, 236, 101, 118, 97, 115, 95, 99, 105, 112, 95, 99, 105, 187};
        app_abortDepartBornIdent = [self StringFromWhichData:value];
    }
...
- (NSString *)noti_infoKey {
    ...
    if (!noti_infoKey) {
        Byte value[] = {9, 2, 70, 70, 70, 70, 70, 70, 54, 54, 35, 155};
        noti_infoKey = [self StringFromWhichData:value];
    }
    return noti_infoKey;
}

//: 0.5x
- (NSString *)mainRoughKey {
    /* static */ NSString *mainRoughKey = nil;
    if (!mainRoughKey) {
        Byte value[] = {4, 11, 78, 5, 97, 31, 24, 151, 180, 133, 39, 120, 53, 46, 48, 32};
        mainRoughKey = [self StringFromWhichData:value];
    }
    return mainRoughKey;
}

//: group_info_activity_update_failed
- (NSString *)appPenaltyValue {
    /* static */ NSString *appPenaltyValue = nil;
    if (!appPenaltyValue) {
        Byte value[] = {33, 8, 105, 169, 120, 244, 117, 61, 100, 101, 108, 105, 97, 102, 95, 101, 116, 97, 100, 112, 117, 95, 121, 116, 105, 118, 105, 116, 99, 97, 95, 111, 102, 110, 105, 95, 112, 117, 111, 114, 103, 16};
        appPenaltyValue = [self StringFromWhichData:value];
    }
    return appPenaltyValue;
}

//: group_info_activity_update_success
- (NSString *)main_treatyId {
    /* static */ NSString *main_treatyId = nil;
    if (!main_treatyId) {
        Byte value[] = {34, 10, 108, 61, 175, 70, 84, 7, 207, 45, 115, 115, 101, 99, 99, 117, 115, 95, 101, 116, 97, 100, 112, 117, 95, 121, 116, 105, 118, 105, 116, 99, 97, 95, 111, 102, 110, 105, 95, 112, 117, 111, 114, 103, 188};
        main_treatyId = [self StringFromWhichData:value];
    }
    return main_treatyId;
}

//: friend_verify_avtivity_net_error
- (NSString *)showHorrorBornFormat {
    /* static */ NSString *showHorrorBornFormat = nil;
    if (!showHorrorBornFormat) {
        Byte value[] = {32, 3, 117, 114, 111, 114, 114, 101, 95, 116, 101, 110, 95, 121, 116, 105, 118, 105, 116, 118, 97, 95, 121, 102, 105, 114, 101, 118, 95, 100, 110, 101, 105, 114, 102, 185};
        showHorrorBornFormat = [self StringFromWhichData:value];
    }
    return showHorrorBornFormat;
}

//: 00:00
- (NSString *)m_standingName {
    /* static */ NSString *m_standingName = nil;
    if (!m_standingName) {
        Byte value[] = {5, 7, 165, 154, 185, 94, 140, 48, 48, 58, 48, 48, 222};
        m_standingName = [self StringFromWhichData:value];
    }
    return m_standingName;
}

//: 播放失败
- (NSString *)mainWhichDefineDeadKey {
    /* static */ NSString *mainWhichDefineDeadKey = nil;
    if (!mainWhichDefineDeadKey) {
        Byte value[] = {12, 7, 194, 240, 115, 22, 153, 165, 180, 232, 177, 164, 229, 190, 148, 230, 173, 146, 230, 213};
        mainWhichDefineDeadKey = [self StringFromWhichData:value];
    }
    return mainWhichDefineDeadKey;
}

//: /  00:00
- (NSString *)userPickCancelMessage {
    /* static */ NSString *userPickCancelMessage = nil;
    if (!userPickCancelMessage) {
        Byte value[] = {8, 4, 243, 85, 48, 48, 58, 48, 48, 32, 32, 47, 208};
        userPickCancelMessage = [self StringFromWhichData:value];
    }
    return userPickCancelMessage;
}
...
@interface TinkleViewController ()<SessionDelegate>

//: @property (nonatomic,strong) NTESVideoViewItem *item;
@property (nonatomic,strong) SearchedName *item;
...
@property (nonatomic, strong) TeamView *videoSlider; //滑动条
...
@implementation TinkleViewController
{
    //: CGPoint _startPoint; 
...
- (void)viewDidLoad {
    ...
    [backButton setImage:[UIImage imageNamed:[[WhichData sharedInstance] notiToKey]] forState:(UIControlStateNormal)];
    ...
    _btnShare.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-15-24, [UIDevice title]+10, 24, 24);
    //: [_btnShare setImage:[UIImage imageNamed:@"ic_pic_share"] forState:UIControlStateNormal];
    [_btnShare setImage:[UIImage imageNamed:[[WhichData sharedInstance] show_storagePath]] forState:UIControlStateNormal];
    //: [_btnShare addTarget:self action:@selector(onTouchshare) forControlEvents:UIControlEventTouchUpInside];
    [_btnShare addTarget:self action:@selector(cutBar) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_btnSave setImage:[UIImage imageNamed:[[WhichData sharedInstance] app_abortDepartBornIdent]] forState:UIControlStateNormal];
    ...
    [_btnPlay setImage:[UIImage imageNamed:[[WhichData sharedInstance] userUpId]] forState:UIControlStateNormal];
    //: [_btnPlay setImage:[UIImage imageNamed:@"player_push"] forState:UIControlStateSelected];
    [_btnPlay setImage:[UIImage imageNamed:[[WhichData sharedInstance] mainCancelTitle]] forState:UIControlStateSelected];
    //: [_btnPlay addTarget:self action:@selector(onTap) forControlEvents:UIControlEventTouchUpInside];
    [_btnPlay addTarget:self action:@selector(valueTitle) forControlEvents:UIControlEventTouchUpInside];
...
//    self.progressSlider.thumbTintColor = ThemeColor;
    ...
    [self.progressSlider addTarget:self action:@selector(titleSub:) forControlEvents:UIControlEventValueChanged];
...
- (void)actionRate:(UIButton *)sender
{
    @
    ...
    .wDataSet(@[[[WhichData sharedInstance] mainRoughKey],@"1x",[[WhichData sharedInstance] notiEconomicMessage],@"2x"])
    //: .wStart();
    .wStart();
}
...
- (void)roundBox:(void(^)(NSError *error))handler{
//    [SVProgressHUD show];
...
- (void)pathed:(NSInteger)totalTime
{
    //: self.totalTime = totalTime;
    self.totalTime = totalTime;
    //: self.totalLabel.text = [NSString stringWithFormat:@"/  %@",[self timeFormatted:totalTime]];
    self.totalLabel.text = [NSString stringWithFormat:@"/  %@",[self openInput:totalTime]];
}

//: - (void)videoPlayercurrentTime:(NSInteger)currentTime
- (void)subObject:(NSInteger)currentTime
{
    //: self.currentTime = currentTime;
    self.currentTime = currentTime;
    //: self.currentLabel.text = [self timeFormatted:currentTime];
    self.currentLabel.text = [self openInput:currentTime];
...
- (NSString *)openInput:(NSInteger)totalSeconds
{
...
- (void)scale{
    ...
    self.avPlayer.view.frame = CGRectMake(0, (44.0f + [UIDevice title]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice title])-(({ BOOL isPhoneX = NO; if (@available(iOS 11.0, *)) { isPhoneX = [[UIApplication sharedApplication] delegate].window.safeAreaInsets.bottom > 0.0; } isPhoneX;}) ? 49 + 35 : 49));
//    self.avPlayer.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             ...
                                             selector:@selector(irresistiblenessed:)
                                                 //: name:NTESAVMoviePlayerPlaybackDidFinishNotification
                                                 name:main_toApplicationKey
...
- (void)irresistiblenessed: (NSNotification *)aNotification
{
    ...
    if (self.avPlayer == aNotification.object)
    {
        ...
        NSNumber *resultValue = [notificationUserInfo objectForKey:notiValueName];
        ...
        if (reason == NTESAVMovieFinishReasonPlaybackError)
        {
            ...
            NSError *mediaPlayerError = [notificationUserInfo objectForKey:[[WhichData sharedInstance] app_aspectId]];
            //: NSString *errorTip = [NSString stringWithFormat:@"%@: %@", @"播放失败".ntes_localized, [mediaPlayerError localizedDescription]];
            NSString *errorTip = [NSString stringWithFormat:@"%@: %@", [[WhichData sharedInstance] mainWhichDefineDeadKey].ting, [mediaPlayerError localizedDescription]];
...
- (void)viewwed: (NSNotification *)aNotification
{
    //: if (self.avPlayer == aNotification.object)
    if (self.avPlayer == aNotification.object)
    {
...
- (PushController *)avPlayer {
    ...
    if (!_avPlayer) {
        ...
        _avPlayer = [[PushController alloc] initWithSmart:[NSURL fileURLWithPath:self.item.path]];
...
//        _imgPlay.center = self.view.center;
        ...
        _imgPlay.image = [UIImage imageNamed:[[WhichData sharedInstance] mainCancelTitle]];
        //: _imgPlay.hidden = YES;
        _imgPlay.hidden = YES;
    }
    //: return _imgPlay;
...
- (UILabel *)currentLabel{
    ...
    if (!_currentLabel) {
        ...
        _currentLabel.text = [[WhichData sharedInstance] m_standingName];
...
- (UILabel *)totalLabel{
    ...
    if (!_totalLabel) {
        ...
        _totalLabel.text = [[WhichData sharedInstance] userPickCancelMessage];
...
- (TeamView *)videoSlider{
    //: if (!_videoSlider) {
    if (!_videoSlider) {
        //: _videoSlider = [[LYSlider alloc] initWithFrame:CGRectMake(0, 0, 3*([[UIScreen mainScreen] bounds].size.width/4), 20)];
        _videoSlider = [[TeamView alloc] initWithFrame:CGRectMake(0, 0, 3*([[UIScreen mainScreen] bounds].size.width/4), 20)];
...
//        UIImage *highlightImage = [UIImage creatImageWithView:highlightView];
        ...
        [_videoSlider greenTime:normalImage thumb:UIControlStateNormal];
...
//    [self startHideControlTimer];
    ...
    [self.avPlayer will: slider.value];
}
...
- (UIPanGestureRecognizer *)panGesture{
    ...
    if (!_panGesture) {
        ...
        _panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(consuetuding:)];
    }
    //: return _panGesture;
    return _panGesture;
}
//: - (void)panGestureTouch:(UIPanGestureRecognizer *)panGestureTouch{
- (void)consuetuding:(UIPanGestureRecognizer *)panGestureTouch{
...
//            }
            ...
            [self.avPlayer will:_fastCurrentTime];
        }
//        [self startHideControlTimer];
    }
}
