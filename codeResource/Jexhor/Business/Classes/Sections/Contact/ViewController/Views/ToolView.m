...
+ (Byte *)DemonstrateDataToCache:(Byte *)data {
    int assemblage = data[0];
    Byte eye = data[1];
    int onTing = data[2];
    for (int i = onTing; i < onTing + assemblage; i++) {
        int value = data[i] - eye;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[onTing + assemblage] = 0;
    return data + onTing;
}
...
//: #F7F8FB
+ (NSString *)app_maxData {
    /* static */ NSString *app_maxData = nil;
    if (!app_maxData) {
        Byte value[] = {7, 93, 7, 91, 78, 53, 191, 128, 163, 148, 163, 149, 163, 159, 63};
        app_maxData = [self StringFromDemonstrateData:value];
    }
    return app_maxData;
}
...
-(instancetype)init{
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor user:[DemonstrateData app_maxData]];
...
-(instancetype)init{
    ...
    if (self) {
        ...
        [button addTarget:self action:@selector(withed:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)withed:(UIButton *)sender{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(boxing:)]) {
        //: [self.delegate headerClickWith:self.section];
        [self.delegate boxing:self.section];
    }
}
