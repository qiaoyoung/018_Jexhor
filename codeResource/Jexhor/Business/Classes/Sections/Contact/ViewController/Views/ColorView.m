...
@interface SizeThreadData : NSObject

@end

@implementation SizeThreadData

+ (Byte *)SizeThreadDataToCache:(Byte *)data {
    int straddle = data[0];
    Byte candleHolder = data[1];
    int weltanschauung = data[2];
    for (int i = weltanschauung; i < weltanschauung + straddle; i++) {
        int value = data[i] + candleHolder;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[weltanschauung + straddle] = 0;
    return data + weltanschauung;
}

+ (NSString *)StringFromSizeThreadData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self SizeThreadDataToCache:data]];
}

//: wc_scan_torch
+ (NSString *)app_itemButtonMsg {
    /* static */ NSString *app_itemButtonMsg = nil;
    if (!app_itemButtonMsg) {
        Byte value[] = {13, 85, 3, 34, 14, 10, 30, 14, 12, 25, 10, 31, 26, 29, 14, 19, 50};
        app_itemButtonMsg = [self StringFromSizeThreadData:value];
    }
    return app_itemButtonMsg;
}

//: qrcode_activity_title
+ (NSString *)dream_showTitle {
    /* static */ NSString *dream_showTitle = nil;
    if (!dream_showTitle) {
        Byte value[] = {21, 8, 5, 79, 106, 105, 106, 91, 103, 92, 93, 87, 89, 91, 108, 97, 110, 97, 108, 113, 87, 108, 97, 108, 100, 93, 157};
        dream_showTitle = [self StringFromSizeThreadData:value];
    }
    return dream_showTitle;
}

//: wc_scan_album
+ (NSString *)dream_viewStr {
    /* static */ NSString *dream_viewStr = nil;
    if (!dream_viewStr) {
        Byte value[] = {13, 42, 5, 85, 239, 77, 57, 53, 73, 57, 55, 68, 53, 55, 66, 56, 75, 67, 196};
        dream_viewStr = [self StringFromSizeThreadData:value];
    }
    return dream_viewStr;
}

//: activity_qrcode_scan_me
+ (NSString *)noti_collectionUrl {
    /* static */ NSString *noti_collectionUrl = nil;
    if (!noti_collectionUrl) {
        Byte value[] = {23, 32, 10, 11, 77, 46, 132, 5, 156, 231, 65, 67, 84, 73, 86, 73, 84, 89, 63, 81, 82, 67, 79, 68, 69, 63, 83, 67, 65, 78, 63, 77, 69, 1};
        noti_collectionUrl = [self StringFromSizeThreadData:value];
    }
    return noti_collectionUrl;
}

//: wc_scan_torch_hl
+ (NSString *)noti_rangePath {
    /* static */ NSString *noti_rangePath = nil;
    if (!noti_rangePath) {
        Byte value[] = {16, 79, 13, 162, 230, 114, 194, 96, 45, 177, 27, 113, 16, 40, 20, 16, 36, 20, 18, 31, 16, 37, 32, 35, 20, 25, 16, 25, 29, 62};
        noti_rangePath = [self StringFromSizeThreadData:value];
    }
    return noti_rangePath;
}

//: wc_scan_mine_qrcode
+ (NSString *)notiSessionStatusText {
    /* static */ NSString *notiSessionStatusText = nil;
    if (!notiSessionStatusText) {
        Byte value[] = {19, 67, 10, 30, 143, 18, 82, 30, 64, 207, 52, 32, 28, 48, 32, 30, 43, 28, 42, 38, 43, 34, 28, 46, 47, 32, 44, 33, 34, 156};
        notiSessionStatusText = [self StringFromSizeThreadData:value];
    }
    return notiSessionStatusText;
}

//: message_send_album
+ (NSString *)appNameFormat {
    /* static */ NSString *appNameFormat = nil;
    if (!appNameFormat) {
        Byte value[] = {18, 73, 8, 222, 225, 208, 146, 170, 36, 28, 42, 42, 24, 30, 28, 22, 42, 28, 37, 27, 22, 24, 35, 25, 44, 36, 95};
        appNameFormat = [self StringFromSizeThreadData:value];
    }
    return appNameFormat;
}
...
- (UIButton *)torchBtn {
    ...
    if (!_torchBtn) {
        ...
        [_torchBtn setBackgroundImage:[UIImage imageNamed:[SizeThreadData app_itemButtonMsg]] forState:(UIControlStateNormal)];
        //: [_torchBtn addTarget:self action:@selector(torchBtn_action:) forControlEvents:(UIControlEventTouchUpInside)];
        [_torchBtn addTarget:self action:@selector(textContentAction:) forControlEvents:(UIControlEventTouchUpInside)];
...
- (void)textContentAction:(UIButton *)btn {
    //: if (btn.selected) {
    if (btn.selected) {
        //: btn.selected = NO;
        btn.selected = NO;
        //: [btn setBackgroundImage:[UIImage imageNamed:@"wc_scan_torch"] forState:(UIControlStateNormal)];
        [btn setBackgroundImage:[UIImage imageNamed:[SizeThreadData app_itemButtonMsg]] forState:(UIControlStateNormal)];
    ...
    //: } else {
        ...
        [btn setBackgroundImage:[UIImage imageNamed:[SizeThreadData noti_rangePath]] forState:(UIControlStateNormal)];
        //: [SGTorch turnOnTorch];
        [TingBarTorch value];
    }
}
...
- (UIImageView *)qrcodeImgView {
    ...
    if (!_qrcodeImgView) {
        ...
        _qrcodeImgView.image = [UIImage imageNamed:[SizeThreadData notiSessionStatusText]];
    }
...
- (UIImageView *)albumImgView {
    ...
    if (!_albumImgView) {
        ...
        _albumImgView.image = [UIImage imageNamed:[SizeThreadData dream_viewStr]];
    }
