...
- (instancetype)initWithMedia:(id)first usage:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
- (void)setMembers:(NSArray *)members
{
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[Pair alloc] initWithMedia:groupTitle usage:groupedMembers]];
        }
    //: }];
    }];
    //: [self sort];
...
- (void)time:(id<SessionCreate>)member
{
    ...
    if(!pair) {
        ...
        pair = [[Pair alloc] initWithMedia:groupTitle usage:members];
    }
...
- (void)with:(NSString *)title counternalRepresentation:(NSArray *)members {
    ...
    Pair *pair = [[Pair alloc] initWithMedia:title usage:members];
