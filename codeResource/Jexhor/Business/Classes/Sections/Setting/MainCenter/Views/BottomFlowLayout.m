...
static CGFloat const user_edgeRefTitle = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const noti_itemName = 0.6;
...
@interface BottomFlowLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<FigureSource> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<SessionLayout> delegate;

//: @end
@end

//: @implementation LxGridViewFlowLayout
@implementation BottomFlowLayout
{
...
- (void)gestureFor
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(tingPin:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = user_edgeRefTitle;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(gestureColorTriggerd:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tabled:) name:UIApplicationWillResignActiveNotification object:nil];
}

//: - (void)removeGestureRecognizers
- (void)viewRecognizers
{
...
- (void)tingPin:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(doing:)];
                ...
                _remainSecondsToBeginEditing = noti_itemName;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(color:recording:title:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate color:self.collectionView recording:self title:_movingItemIndexPath];
            }
            ...
            ButtonViewCell *sourceCell = (ButtonViewCell *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell read];
            ...
            UIView * snapshotView = [sourceCell read];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(nameShow:add:to:)]) {
                                         ...
                                         [strongSelf.delegate nameShow:strongSelf.collectionView add:strongSelf to:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(collectionUnit:colorOff:name:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate collectionUnit:self.collectionView colorOff:self name:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(restoreColor:bubbleImage:blue:)]) {
                                             ...
                                             [strongSelf.delegate restoreColor:strongSelf.collectionView bubbleImage:strongSelf blue:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)gestureColorTriggerd:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if ([self.dataSource respondsToSelector:@selector(atSize:button:bar:)] && [self.dataSource atSize:self.collectionView button:sourceIndexPath bar:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(enable:timePath:blue:)]) {
                ...
                [self.dataSource enable:self.collectionView timePath:sourceIndexPath blue:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(item:formatting:name:)]) {
                    ...
                    [strongSelf.dataSource item:strongSelf.collectionView formatting:sourceIndexPath name:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)doing:(CADisplayLink *)displayLink
{
...
- (void)tabled:(NSNotification *)notificaiton
{
