...
static CGFloat const mMinIdent = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const k_nameMessage = 0.6;

//: static inline CGPoint CGPointOffset(CGPoint point, CGFloat dx, CGFloat dy)
static inline CGPoint remoteSession(CGPoint point, CGFloat dx, CGFloat dy)
{
...
@interface SurgeViewLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<WorkerView> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<ChangeControl> delegate;

//: @end
@end

//: @implementation LxGridViewFlowLayout
@implementation SurgeViewLayout
{
...
- (void)aftermath
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(imageColor:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = mMinIdent;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(sessions:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(digitizerred:) name:UIApplicationWillResignActiveNotification object:nil];
}
...
- (void)imageColor:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(progressAt:)];
                ...
                _remainSecondsToBeginEditing = k_nameMessage;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(name:color:forefront:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate name:self.collectionView color:self forefront:_movingItemIndexPath];
            }
            ...
            BubblePushViewCell *sourceCell = (BubblePushViewCell *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell agreementImage];
            ...
            UIView * snapshotView = [sourceCell agreementImage];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(icon_strong:argument:message:)]) {
                                         ...
                                         [strongSelf.delegate icon_strong:strongSelf.collectionView argument:strongSelf message:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(image:module:session:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate image:self.collectionView module:self session:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(secret:color:table:)]) {
                                             ...
                                             [strongSelf.delegate secret:strongSelf.collectionView color:strongSelf table:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)sessions:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            _beingMovedPromptView.center = remoteSession(_sourceItemCollectionViewCellCenter, panTranslation.x, panTranslation.y);
            ...
            if ([self.dataSource respondsToSelector:@selector(advanced:blue:ptolemaicSystem:)] && [self.dataSource advanced:self.collectionView blue:sourceIndexPath ptolemaicSystem:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(mode:bubble:beyondClose:)]) {
                ...
                [self.dataSource mode:self.collectionView bubble:sourceIndexPath beyondClose:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(record_strong:view:mustButton:)]) {
                    ...
                    [strongSelf.dataSource record_strong:strongSelf.collectionView view:sourceIndexPath mustButton:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)progressAt:(CADisplayLink *)displayLink
{
...
- (void)digitizerred:(NSNotification *)notificaiton
{
