...
NSString *StringFromStraddleData(StraddleData *data);


//: AlbumAddBtn
StraddleData main_withJuryFormat = (StraddleData){132, (Byte []){197, 232, 230, 241, 233, 197, 224, 224, 198, 240, 234, 198}, 11, 232};

//: code
StraddleData showPractitionerName = (StraddleData){69, (Byte []){38, 42, 33, 32, 74}, 4, 247};

//: image%lu
StraddleData appTermsStr = (StraddleData){29, (Byte []){116, 112, 124, 122, 120, 56, 113, 104, 120}, 8, 166};

//: /other/feedback
StraddleData appBluePath = (StraddleData){9, (Byte []){38, 102, 125, 97, 108, 123, 38, 111, 108, 108, 109, 107, 104, 106, 98, 17}, 15, 184};

//: report_activity_title
StraddleData main_chartStr = (StraddleData){29, (Byte []){111, 120, 109, 114, 111, 105, 66, 124, 126, 105, 116, 107, 116, 105, 100, 66, 105, 116, 105, 113, 120, 88}, 21, 245};

//: report_info
StraddleData dream_threadIdent = (StraddleData){227, (Byte []){145, 134, 147, 140, 145, 151, 188, 138, 141, 133, 140, 159}, 11, 212};

//: icon_checkbox_selected
StraddleData dreamIconValue = (StraddleData){54, (Byte []){95, 85, 89, 88, 105, 85, 94, 83, 85, 93, 84, 89, 78, 105, 69, 83, 90, 83, 85, 66, 83, 82, 122}, 22, 173};

//: %lu/400
StraddleData mVeteranWipeMessage = (StraddleData){169, (Byte []){140, 197, 220, 134, 157, 153, 153, 51}, 7, 173};

//: msg
StraddleData show_buttonMessage = (StraddleData){24, (Byte []){117, 107, 127, 243}, 3, 243};

//: #A148FF
StraddleData app_tingMessage = (StraddleData){203, (Byte []){232, 138, 250, 255, 243, 141, 141, 141}, 7, 151};

//: group_info_activity_op_failed
StraddleData notiItselfIdent = (StraddleData){35, (Byte []){68, 81, 76, 86, 83, 124, 74, 77, 69, 76, 124, 66, 64, 87, 74, 85, 74, 87, 90, 124, 76, 83, 124, 69, 66, 74, 79, 70, 71, 237}, 29, 228};

//: public.image
StraddleData mContentUrl = (StraddleData){31, (Byte []){111, 106, 125, 115, 118, 124, 49, 118, 114, 126, 120, 122, 25}, 12, 180};

//: #612CF9
StraddleData appInputName = (StraddleData){126, (Byte []){93, 72, 79, 76, 61, 56, 71, 83}, 7, 176};

//: #999999
StraddleData appHolderKey = (StraddleData){183, (Byte []){148, 142, 142, 142, 142, 142, 142, 113}, 7, 210};

//: contact
StraddleData kWithRageName = (StraddleData){39, (Byte []){68, 72, 73, 83, 70, 68, 83, 212}, 7, 169};

//: #875FFA
StraddleData notiChartIdent = (StraddleData){28, (Byte []){63, 36, 43, 41, 90, 90, 93, 105}, 7, 171};

//: common_bg
StraddleData dream_amPath = (StraddleData){97, (Byte []){2, 14, 12, 12, 14, 15, 62, 3, 6, 19}, 9, 253};

//: back_arrow_b
StraddleData user_statusStr = (StraddleData){44, (Byte []){78, 77, 79, 71, 115, 77, 94, 94, 67, 91, 115, 78, 138}, 12, 165};
...
@interface MaxViewController () <UITextViewDelegate,TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
//    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"common_bg"]];
    ...
    bg.image = [UIImage imageNamed:StringFromStraddleData(&dream_amPath)];
...
//    [self.view addSubview:bg];
    ...
    [backButton setImage:[UIImage imageNamed:StringFromStraddleData(&user_statusStr)] forState:(UIControlStateNormal)];
    ...
    [submitButton setImage:[UIImage imageNamed:StringFromStraddleData(&dreamIconValue)] forState:(UIControlStateNormal)];
...
//    self.navigationItem.rightBarButtonItem.enabled = NO;
    ...
    self.numLabel.text = [NSString stringWithFormat:StringFromStraddleData(&mVeteranWipeMessage),(unsigned long)_textView.text.length];
    //: self.numLabel.frame = CGRectMake(15, (44.0f + [UIDevice vg_statusBarHeight])+200+30, [[UIScreen mainScreen] bounds].size.width-30, 20);
    self.numLabel.frame = CGRectMake(15, (44.0f + [UIDevice title])+200+30, [[UIScreen mainScreen] bounds].size.width-30, 20);
...
//    }
    ...
    self.numLabel.text = [NSString stringWithFormat:StringFromStraddleData(&mVeteranWipeMessage),textView.text.length];
}
...
-(void)allBlue{
    ...
    dict[StringFromStraddleData(&kWithRageName)] = _textView.text;
    ...
    if (_selectedPhotos.count > 0) {
        ...
        [_selectedPhotos enumerateObjectsUsingBlock:^(UIImage *image , NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [nameArray addObject:[NSString stringWithFormat:StringFromStraddleData(&appTermsStr),(unsigned long)(idx+1)]];
        //: }];
        }];
    }
    //: [SVProgressHUD show];
    ...
    [TouchShowMessage imageAdd:[NSString stringWithFormat:StringFromStraddleData(&appBluePath)] ting:dict stateFailed:array userRecent:nameArray behindAddFailed:^(NSProgress *progress) {
    ...
    //: } success:^(id responseObject) {
        ...
        NSString *code = [resultDict index:StringFromStraddleData(&showPractitionerName)];
        //: NSString *msg = [resultDict newStringValueForKey:@"msg"];
        NSString *msg = [resultDict index:StringFromStraddleData(&show_buttonMessage)];
...
- (UILabel *)numLabel{
    ...
    if (!_numLabel) {
        ...
        _numLabel.textColor = [UIColor user:StringFromStraddleData(&appHolderKey)];
    }
    //: return _numLabel;
    return _numLabel;
}
...
- (UIImagePickerController *)imagePickerVc {
    ...
    if (_imagePickerVc == nil) {
        ...
        _imagePickerVc.navigationBar.barTintColor = [UIColor colorWithPatternImage:[BubbleHelper infoCurrent:[UIColor user:StringFromStraddleData(&notiChartIdent)] tap:[UIColor user:StringFromStraddleData(&appInputName)] caterer:SNLinearGradientDirectionLevel]];
        //: _imagePickerVc.navigationBar.tintColor = [UIColor colorWithHexString:@"#A148FF"];
        _imagePickerVc.navigationBar.tintColor = [UIColor user:StringFromStraddleData(&app_tingMessage)];
    }
...
- (void)viewDidLayoutSubviews {
    ...
    self.collectionView.frame = CGRectMake(0, self.textView.titleCell + self.textView.height, self.view.tz_width, self.view.tz_height - self.textView.height);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == _selectedPhotos.count) {
        ...
        cell.imageView.image = [UIImage imageNamed:StringFromStraddleData(&main_withJuryFormat)];
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:StringFromStraddleData(&mContentUrl)]) {
        ...
        [[TZImageManager manager] savePhotoWithImage:image meta:meta location:nil completion:^(PHAsset *asset, NSError *error){
            ...
            //: } else {
                ...
                [self value:assetModel.asset radius:image];
            }
        //: }];
        }];
    }
...
- (void)value:(PHAsset *)asset radius:(UIImage *)image {
...
Byte *StraddleDataToByte(StraddleData *data) {
    if (data->wipeInput < 142) return data->sod;
    for (int i = 0; i < data->alter; i++) {
        data->sod[i] ^= data->fearfulness;
    }
    data->sod[data->alter] = 0;
    data->wipeInput = 35;
    return data->sod;
}

NSString *StringFromStraddleData(StraddleData *data) {
    return [NSString stringWithUTF8String:(char *)StraddleDataToByte(data)];
}
