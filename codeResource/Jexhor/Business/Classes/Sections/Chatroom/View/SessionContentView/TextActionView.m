...
@implementation TextActionView

//: - (instancetype)initSessionMessageContentView
- (instancetype)initPageRandom
{
...
- (void)tool:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
...
- (UIImage *)path:(UIControlState)state with:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)imageShouldUp:(UIButton *)button
{
...
- (void)record:(NIMMessageModel *)data
{
...
- (void)dateRobot:(NIMMessageModel *)data
{
...
- (void)incidental:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           model:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            VideoTextView *label = [self genLabel];
...
- (CGSize)count:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [TextActionView itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    ...
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [TextActionView continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [TextActionView itemSpacing];
        }
    }
...
- (void)paged:(ChangeButton *)button
{
    //: NIMKitEvent *event = [[NIMKitEvent alloc] init];
    NIMKitEvent *event = [[NIMKitEvent alloc] init];
    ...
    else
    {
        ...
        for (VideoTextView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[VideoTextView class]])
            {
...
- (void)sessionColor
{
    ...
    [self exhibit:self lengthId:self.model.contentViewInsets];

    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
    //: CGFloat top = [NTESSessionRobotContentView itemSpacing] + contentInsets.top;
    CGFloat top = [TextActionView itemSpacing] + contentInsets.top;
    //: for (UIView *view in self.subviews)
    for (UIView *view in self.subviews)
    {
        //: if (view == self.bubbleImageView)
        ...
        top = view.bottom + [TextActionView itemSpacing];
    }
}


//: - (void)resizeAllSubView:(UIView *)superView insets:(UIEdgeInsets)insets
- (void)exhibit:(UIView *)superView lengthId:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[VideoTextView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                VideoTextView *label = (VideoTextView *)subView;
            ...
            else if ([subView isKindOfClass:[ChangeButton class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                ChangeButton *button = (ChangeButton *)subView;
                //: button.width = width;
                button.width = width;
...
- (void)needView:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[ChangeButton class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            ChangeButton *btn = (ChangeButton *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (VideoTextView *)noneGreen
{
    //: M80AttributedLabel *label = nil;
    VideoTextView *label = nil;
    ...
    else
    {
        ...
        label = [[VideoTextView alloc] initWithFrame:CGRectZero];
...
- (ChangeButton *)mark
{
    ...
    else
    {
        ...
        button = [[ChangeButton alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
