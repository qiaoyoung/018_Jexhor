...
- (void)providerCell:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
...
- (UIImage *)show:(UIControlState)state line:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)itemContinue:(UIButton *)button
{
...
- (void)frame:(NIMMessageModel *)data
{
...
- (void)label:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           futurism:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            InsertView *label = [self genLabel];
        ...
        case NIMKitRobotTemplateItemTypeLinkBlock:
        {
            ...
            ViewButton *button = [self coatButtonBy];
...
- (CGSize)withName:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [ContentView itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    ...
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [ContentView continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [ContentView itemSpacing];
        }
    }
...
- (void)doing:(ViewButton *)button
{
    ...
    else
    {
        ...
        for (InsertView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[InsertView class]])
            {
...
- (void)textTo
{
    ...
    [self tap:self red:self.model.contentViewInsets];

    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
    //: CGFloat top = [NTESSessionRobotContentView itemSpacing] + contentInsets.top;
    CGFloat top = [ContentView itemSpacing] + contentInsets.top;
    //: for (UIView *view in self.subviews)
    for (UIView *view in self.subviews)
    {
        //: if (view == self.bubbleImageView)
        ...
        top = view.bottom + [ContentView itemSpacing];
    }
}


//: - (void)resizeAllSubView:(UIView *)superView insets:(UIEdgeInsets)insets
- (void)tap:(UIView *)superView red:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[InsertView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                InsertView *label = (InsertView *)subView;
            ...
            else if ([subView isKindOfClass:[ViewButton class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                ViewButton *button = (ViewButton *)subView;
                //: button.width = width;
                button.width = width;
...
- (void)with:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[ViewButton class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            ViewButton *btn = (ViewButton *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (InsertView *)drawShould
{
    //: M80AttributedLabel *label = nil;
    InsertView *label = nil;
    ...
    else
    {
        ...
        label = [[InsertView alloc] initWithFrame:CGRectZero];
...
- (ViewButton *)coatButtonBy
{
    //: NTESSessionRobotButton *button = nil;
    ViewButton *button = nil;
    ...
    else
    {
        ...
        button = [[ViewButton alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
