...
- (void)data
{
    ...
    self.dotViewClass = [VisualizationView class];
    //: self.spacingBetweenDots = kDefaultSpacingBetweenDots;
    self.spacingBetweenDots = user_versionData;
    //: self.numberOfPages = kDefaultNumberOfPages;
    self.numberOfPages = user_transformName;
    //: self.currentPage = kDefaultCurrentPage;
    self.currentPage = dream_marginData;
    //: self.hidesForSinglePage = kDefaultHideForSinglePage;
    self.hidesForSinglePage = user_layerContentValue;
    //: self.shouldResizeFromCenter = kDefaultShouldResizeFromCenter;
    self.shouldResizeFromCenter = show_groundValue;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(receive:facingPages:)]) {
            //: [self.delegate TAPageControl:self didSelectPageAtIndex:index];
            [self.delegate receive:self facingPages:index];
        }
    }
}
...
- (CGSize)video:(NSInteger)pageCount
{
...
- (void)by
{
    ...
    for (NSInteger i = 0; i < self.numberOfPages; i++) {
        ...
        [self tableIndex:dot topIndex:i];
    }

    //: [self changeActivity:YES atIndex:self.currentPage];
    [self parent:YES top:self.currentPage];
...
- (void)show:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self video:self.numberOfPages];
...
- (void)tableIndex:(UIView *)dot topIndex:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self video:self.numberOfPages].width) / 2);
...
- (UIView *)voiceView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[VisualizationView class]] && self.dotColor) {
            //: ((TAAnimatedDotView *)dotView).dotColor = self.dotColor;
            ((VisualizationView *)dotView).dotColor = self.dotColor;
        }
    //: } else {
    } else {
...
- (void)parent:(BOOL)active top:(NSInteger)index
{
    //: if (self.dotViewClass) {
    if (self.dotViewClass) {
        //: TAAbstractDotView *abstractDotView = (TAAbstractDotView *)[self.dots objectAtIndex:index];
        TidyView *abstractDotView = (TidyView *)[self.dots objectAtIndex:index];
        //: if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
        if ([abstractDotView respondsToSelector:@selector(changeState:)]) {
            //: [abstractDotView changeActivityState:active];
            [abstractDotView changeState:active];
        //: } else {
        } else {
...
- (void)setCurrentPage:(NSInteger)currentPage
{
    ...
    [self parent:NO top:_currentPage];
    //: _currentPage = currentPage;
    _currentPage = currentPage;
    // Post set
    //: [self changeActivity:YES atIndex:_currentPage];
    [self parent:YES top:_currentPage];
}
