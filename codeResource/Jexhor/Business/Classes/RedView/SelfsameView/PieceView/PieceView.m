...
- (void)black:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self world:self.numberOfPages];
...
- (CGSize)world:(NSInteger)pageCount
{
...
- (void)modifyView
{
    ...
    for (NSInteger i = 0; i <= self.numberOfPages; i++) {
        ...
        [self referInterval:dot label:i];
    }

    //: [self changeIndicatorViewLayout:self.currentPage];
    [self revolutionScaleLayout:self.currentPage];
...
- (void)referInterval:(UIView *)dot label:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self world:self.numberOfPages].width) / 2);
...
- (void)revolutionScaleLayout:(CGFloat)page
{
...
- (void)setCurrentPage:(CGFloat)currentPage
{
    ...
    [self revolutionScaleLayout:currentPage];
}
