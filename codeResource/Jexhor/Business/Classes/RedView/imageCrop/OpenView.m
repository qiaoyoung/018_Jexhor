...
typedef struct {
    Byte alphaBronze;
    Byte *payerQuest;
    unsigned int canPlanetDrug;
	int chairman;
} StructBroData;

@interface BroData : NSObject

@end

@implementation BroData

+ (Byte *)BroDataToByte:(StructBroData *)data {
    for (int i = 0; i < data->canPlanetDrug; i++) {
        data->payerQuest[i] ^= data->alphaBronze;
    }
    data->payerQuest[data->canPlanetDrug] = 0;
	if (data->canPlanetDrug >= 1) {
		data->chairman = data->payerQuest[0];
	}
    return data->payerQuest;
}

+ (NSString *)StringFromBroData:(StructBroData *)data {
    return [NSString stringWithUTF8String:(char *)[self BroDataToByte:data]];
}

//: frame
+ (NSString *)mainKaWithValue {
    /* static */ NSString *mainKaWithValue = nil;
    if (!mainKaWithValue) {
        StructBroData value = (StructBroData){130, (Byte []){228, 240, 227, 239, 231, 87}, 5, 68};
        mainKaWithValue = [self StringFromBroData:&value];
    }
    return mainKaWithValue;
}

//: center
+ (NSString *)app_resourceValue {
    /* static */ NSString *app_resourceValue = nil;
    if (!app_resourceValue) {
        StructBroData value = (StructBroData){4, (Byte []){103, 97, 106, 112, 97, 118, 66}, 6, 122};
        app_resourceValue = [self StringFromBroData:&value];
    }
    return app_resourceValue;
}
...
- (UIImage *)bottomRect:(CGRect)rect
{

    //: UIImage *fixedImage = [self fixOrientation];
    UIImage *fixedImage = [self view];
...
@interface CornerColorView: UIView
...
@implementation SameOpenView
//: - (instancetype)initWithLineWidth: (CGFloat)lineWidth lineHeight: (CGFloat)lineHeight lineColor: (UIColor *)lineColor {
- (instancetype)initWithShould: (CGFloat)lineWidth value: (CGFloat)lineHeight textFormation: (UIColor *)lineColor {
...
@interface UnknownQuantityView : UIView
...
@implementation UnknownQuantityView
...
@property (strong, nonatomic) CornerColorView *topLeftCorner;
//: @property (strong, nonatomic) CornerView *topRightCorner;
@property (strong, nonatomic) CornerColorView *topRightCorner;
//: @property (strong, nonatomic) CornerView *bottomLeftCorner;
@property (strong, nonatomic) CornerColorView *bottomLeftCorner;
//: @property (strong, nonatomic) CornerView *bottomRightCorner;
@property (strong, nonatomic) CornerColorView *bottomRightCorner;
//: @property (strong, nonatomic) CropAreaView *cropAreaView;
@property (strong, nonatomic) UnknownQuantityView *cropAreaView;
...
@property (strong, nonatomic) SameOpenView *topMidLine;
//: @property (strong, nonatomic) MidLineView *leftMidLine;
@property (strong, nonatomic) SameOpenView *leftMidLine;
//: @property (strong, nonatomic) MidLineView *bottomMidLine;
@property (strong, nonatomic) SameOpenView *bottomMidLine;
//: @property (strong, nonatomic) MidLineView *rightMidLine;
@property (strong, nonatomic) SameOpenView *rightMidLine;
...
- (void)dealloc {
    ...
    [_cropAreaView removeObserver: self forKeyPath: [BroData mainKaWithValue]];
    //: [_cropAreaView removeObserver: self forKeyPath: @"center"];
    [_cropAreaView removeObserver: self forKeyPath: [BroData app_resourceValue]];
    //: [_imageView removeObserver: self forKeyPath: @"frame"];
    [_imageView removeObserver: self forKeyPath: [BroData mainKaWithValue]];
...
- (void)sonogram {
    ...
    _cropAreaView = [[UnknownQuantityView alloc] init];
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"frame"
                    forKeyPath: [BroData mainKaWithValue]
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"center"
                    forKeyPath: [BroData app_resourceValue]
                       //: options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial
                       options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial
                       //: context: NULL];
                       context: NULL];
...
- (void)dateRestore {
    ...
    _cropAreaPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(changeReply:)];
...
- (void)showFromPinch: (UIPinchGestureRecognizer *)pinchGesture {
    ...
    switch (pinchGesture.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            [self device: pinchGesture.scale];
            //: break;
            break;
        }
        //: default:
...
- (void)behindHouse: (UIPanGestureRecognizer *)panGesture {

    //: MidLineView *midLineView = (MidLineView *)panGesture.view;
    SameOpenView *midLineView = (SameOpenView *)panGesture.view;
    //: switch (panGesture.state) {
    switch (panGesture.state) {
        //: case UIGestureRecognizerStateBegan: {
        case UIGestureRecognizerStateBegan: {
...
- (void)bringHome: (UIPanGestureRecognizer *)panGesture {
    ...
    CornerColorView *panView = (CornerColorView *)panGesture.view;
    //: CornerView *relativeViewX = panView.relativeViewX;
    CornerColorView *relativeViewX = panView.relativeViewX;
    //: CornerView *relativeViewY = panView.relativeViewY;
    CornerColorView *relativeViewY = panView.relativeViewY;
...
- (void)natalDay {
    ...
    _topLeftCorner = [[CornerColorView alloc]initWithVersion: CGRectMake(0, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) be:_cropAreaCornerLineColor widthExtra: _cropAreaCornerLineWidth];
    ...
    _topRightCorner = [[CornerColorView alloc]initWithVersion: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) be: _cropAreaCornerLineColor widthExtra: _cropAreaCornerLineWidth];
    ...
    _bottomLeftCorner = [[CornerColorView alloc]initWithVersion: CGRectMake(0, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) be: _cropAreaCornerLineColor widthExtra: _cropAreaCornerLineWidth];
    ...
    _bottomRightCorner = [[CornerColorView alloc]initWithVersion: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) be: _cropAreaCornerLineColor widthExtra: _cropAreaCornerLineWidth];
...
- (void)nameLines {
    ...
    _topMidLine = [[SameOpenView alloc]initWithShould: _cropAreaMidLineWidth value: _cropAreaMidLineHeight textFormation: _cropAreaMidLineColor];
    ...
    _bottomMidLine = [[SameOpenView alloc]initWithShould: _cropAreaMidLineWidth value: _cropAreaMidLineHeight textFormation: _cropAreaMidLineColor];
    ...
    _leftMidLine = [[SameOpenView alloc]initWithShould: _cropAreaMidLineWidth value: _cropAreaMidLineHeight textFormation: _cropAreaMidLineColor];
    ...
    _rightMidLine = [[SameOpenView alloc]initWithShould: _cropAreaMidLineWidth value: _cropAreaMidLineHeight textFormation: _cropAreaMidLineColor];
    ...
    _topMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(behindHouse:)];
    ...
    _bottomMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(behindHouse:)];
    ...
    _leftMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(behindHouse:)];
    ...
    _rightMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(behindHouse:)];
...
- (void)device: (CGFloat)scaleFactor {
...
- (void)setNeedScaleCrop:(BOOL)needScaleCrop {
    ...
    if(!_needScaleCrop && needScaleCrop) {
        ...
        _cropAreaPinch = [[UIPinchGestureRecognizer alloc]initWithTarget: self action:@selector(showFromPinch:)];
...
- (UIImage *)change {
    ...
    return [_toCropImage bottomRect: CGRectMake((CGRectGetMinX(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetMinY(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetWidth(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetHeight(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor)];
