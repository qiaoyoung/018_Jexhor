...
static CGFloat const userCountValue = 0.75;
//: static BOOL const square = NO;
static BOOL const show_beginActionValue = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float dream_errorName = 400;
//: float IMAGE_MIN_WIDTH = 400;
float userSwitchceData = 400;

//: @interface ImageCropViewController(){
@interface HoldfastViewController(){
    //: CGRect _cropArea;
    CGRect _cropArea;
}
...
#pragma mark HoldfastViewController implementation
//: @implementation ImageCropViewController
@implementation HoldfastViewController
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[ModeView alloc] initWithIndependentDoing:view stop:self.blurredBackground];
...
- (void)grays:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(textTheory:)])
    {
        //: [self.delegate ImageCropViewControllerDidCancel:self];
        [self.delegate textTheory:self];
    }

}

//: - (void)done:(id)sender
- (void)downing:(id)sender
{

    //: if ([self.delegate respondsToSelector:@selector(ImageCropViewControllerSuccess:didFinishCroppingImage:)])
    if ([self.delegate respondsToSelector:@selector(file:net:)])
    {
        //: UIImage *cropped;
        UIImage *cropped;
        //: if (self.image != nil){
        ...
        [self.delegate file:self net:cropped];
    }
...
#pragma mark QuantityIngredientView implementation

//: @implementation ControlPointView
@implementation QuantityIngredientView
...
static CGFloat const user_inputRetValue = 5;
...
- (void)initManager {
    ...
    self.shadeView = [[StyleView alloc] initWithFrame:subviewFrame];
    ...
    controlPointSize = user_inputRetValue;
    ...
    topLeftPoint = [self keyCorner:SquareCGRectAtCenter(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self keyCorner:SquareCGRectAtCenter(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self keyCorner:SquareCGRectAtCenter(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self keyCorner:SquareCGRectAtCenter(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self windowSharedPoints];
    ...
    UIPanGestureRecognizer* dragRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(blueDown:)];
...
- (QuantityIngredientView*)keyCorner:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    QuantityIngredientView* point = [[QuantityIngredientView alloc] initWithFrame:frame];
    //: return point;
    return point;
}

//: - (CGRect)cropAreaFromControlPoints {
- (CGRect)windowSharedPoints {
...
- (void)crown:(CGPoint)topLeft show:(CGPoint)bottomLeft regionAlready:(CGPoint)bottomRight declaration:(CGPoint)topRight {
...
- (UIView*)counteract:(CGPoint)point {
    //: UIView* view = cropAreaView;
    UIView* view = cropAreaView;
    //: for (int i = 0; i < PointsArray.count; i++) {
    for (int i = 0; i < PointsArray.count; i++) {
        //: CGFloat x = [(ControlPointView *)PointsArray[i] center].x;
        CGFloat x = [(QuantityIngredientView *)PointsArray[i] center].x;
        //: CGFloat y = [(ControlPointView *)PointsArray[i] center].y;
        CGFloat y = [(QuantityIngredientView *)PointsArray[i] center].y;
...
- (void)blueDown:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self of:recognizer arrayView:count];
        //: else
        else
            //: [self prepSingleTouchPan:recognizer];
            [self memberText:recognizer];
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self redWith:[recognizer locationOfTouch:i inView:self] output:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self redWith:[recognizer locationInView:self] output:dragViewOne];
    }
...
- (void)of:(UIPanGestureRecognizer*)recognizer arrayView:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self counteract:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self counteract:[recognizer locationOfTouch:i inView:self]];
...
- (void)memberText:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self counteract:[recognizer locationInView:self]];
...
- (void)setEmotion:(CGRect)cropArea
{
...
- (void)redWith:(CGPoint)location output:(UIView*)view
{
    ...
    //: } else if (view == cropAreaView) {
        ...
        [self garner:location];
    }

    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self windowSharedPoints];
    //: [self setCropAreaForViews:cropArea];
    [self setEmotion:cropArea];
}
...
- (CGSize)color:(CGPoint)dragLocation change:(CGPoint)draggedPoint reply:(CGPoint)oppositePoint {
...
- (void)custom:(CGPoint)dragLocation {
    ...
    CGSize disp = [self color:dragLocation change:dragPoint.topLeftCenter reply:dragPoint.bottomRightCenter];
    ...
    if (width >= userSwitchceData && height < dream_errorName) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - dream_errorName) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userSwitchceData && height >= dream_errorName) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - userSwitchceData) / self.imageScale);
    ...
    [self crown:topLeft show:bottomLeft regionAlready:dragPoint.bottomRightCenter declaration:topRight];
}
//: - (void)handleDragBottomLeft:(CGPoint)dragLocation {
- (void)location:(CGPoint)dragLocation {
    ...
    CGSize disp = [self color:dragLocation change:dragPoint.bottomLeftCenter reply:dragPoint.topRightCenter];
    ...
    if (width >= userSwitchceData && height < dream_errorName) {
        ...
        bottomLeft.y = cropArea.origin.y + (dream_errorName / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userSwitchceData && height >= dream_errorName) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - userSwitchceData) / self.imageScale);
    ...
    [self crown:topLeft show:bottomLeft regionAlready:bottomRight declaration:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)path:(CGPoint)dragLocation {
    ...
    CGSize disp = [self color:dragLocation change:dragPoint.bottomRightCenter reply:dragPoint.topLeftCenter];
    ...
    if (width >= userSwitchceData && height < dream_errorName) {
        ...
        bottomRight.y = cropArea.origin.y + (dream_errorName / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userSwitchceData && height >= dream_errorName) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (userSwitchceData / self.imageScale);
    ...
    [self crown:dragPoint.topLeftCenter show:bottomLeft regionAlready:bottomRight declaration:topRight];
}
...
- (void)color:(CGPoint)dragLocation {
    ...
    CGSize disp = [self color:dragLocation change:dragPoint.topRightCenter reply:dragPoint.bottomLeftCenter];
    ...
    if (width >= userSwitchceData && height < dream_errorName) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - dream_errorName) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < userSwitchceData && height >= dream_errorName) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (userSwitchceData / self.imageScale);
    ...
    [self crown:topLeft show:dragPoint.bottomLeftCenter regionAlready:bottomRight declaration:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)garner:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image will:30 by:1 style:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image will:0 by:1 style:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self crown:topLeftPoint.center show:bottomLeftPoint.center regionAlready:bottomRightPoint.center declaration:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self windowSharedPoints];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
...
@implementation UIImage (PositionTingClick)
