...
typedef struct {
    Byte apartmentBarrel;
    Byte *conferenceComfort;
    unsigned int cozyComplete;
    Byte taiAnyway;
	int showIllegal;
	int lucifer;
	int cliffRip;
...
NSString *StringFromPressurePaintData(PressurePaintData *data);


//: backgroundColor
PressurePaintData showImageName = (PressurePaintData){245, (Byte []){151, 148, 150, 158, 146, 135, 154, 128, 155, 145, 182, 154, 153, 154, 135, 79}, 15, 243, 185, 190, 137};

//: linear
PressurePaintData mPicValue = (PressurePaintData){233, (Byte []){133, 128, 135, 140, 136, 155, 121}, 6, 187, 12, 69, 99};

//: bounds
PressurePaintData kAssaultValue = (PressurePaintData){29, (Byte []){127, 114, 104, 115, 121, 110, 53}, 6, 225, 49, 52, 8};

//: blurRadius
PressurePaintData appUndertakeValue = (PressurePaintData){140, (Byte []){238, 224, 249, 254, 222, 237, 232, 229, 249, 255, 37}, 10, 178, 240, 233, 219};

//: position
PressurePaintData dream_rowData = (PressurePaintData){169, (Byte []){217, 198, 218, 192, 221, 192, 198, 199, 7}, 8, 239, 244, 87, 159};
...
//
//  https://github.com/nicklockwood/Clean
//
...
- (UIImage *)will:(CGFloat)radius by:(NSUInteger)iterations style:(UIColor *)tintColor
{
...
@interface MiddleClassLayer: CALayer
...
@implementation MiddleClassLayer
...
+ (BOOL)needsDisplayForKey:(NSString *)key
{
    ...
    if ([@[StringFromPressurePaintData(&appUndertakeValue), StringFromPressurePaintData(&kAssaultValue), StringFromPressurePaintData(&dream_rowData)] containsObject:key])
    {
        //: return YES;
        return YES;
    }
...
@implementation ShowClick

//: + (instancetype)sharedInstance
+ (instancetype)event
{
    //: static FXBlurScheduler *sharedInstance = nil;
    static ShowClick *sharedInstance = nil;
    //: if (!sharedInstance)
    if (!sharedInstance)
    {
        //: sharedInstance = [[FXBlurScheduler alloc] init];
        sharedInstance = [[ShowClick alloc] init];
    }
...
- (void)colorSave
{
    ...
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        for (NSUInteger i = self.viewIndex; i < [self.views count]; i++)
        {
            ...
            Clean *view = self.views[i];
            //: if (view.dynamic && !view.hidden && view.window && [view shouldUpdate])
            if (view.dynamic && !view.hidden && view.window && [view along])
            {
        ...
        [self performSelector:@selector(colorSave)
                   //: withObject:nil
                   withObject:nil
...
+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    [ShowClick event].blurEnabled = blurEnabled;
}
...
- (void)input
{
    ...
    if (!_blurRadiusSet) [self random].blurRadius = 40;
    ...
    self.layer.magnificationFilter = StringFromPressurePaintData(&mPicValue); // kCAFilterLinear
...
- (void)setBlurRadius:(CGFloat)blurRadius
{
    ...
    [self random].blurRadius = blurRadius;
}
...
- (MiddleClassLayer *)random
{
    //: return (FXBlurLayer *)self.layer;
    return (MiddleClassLayer *)self.layer;
}

//: - (FXBlurLayer *)blurPresentationLayer
- (MiddleClassLayer *)message
{
    //: FXBlurLayer *blurLayer = [self blurLayer];
    MiddleClassLayer *blurLayer = [self random];
    //: return (FXBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
    return (MiddleClassLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)show
{
    ...
    if (self.window && self.dynamic && self.blurEnabled)
    {
        ...
        [[ShowClick event] separate:self];
    }
    //: else
    else
    {
        //: [[FXBlurScheduler sharedInstance] removeView:self];
        [[ShowClick event] view:self];
    }
}
...
- (BOOL)along
{
    ...
    __strong CALayer *underlyingLayer = [self title];
    ...
    self.blurEnabled && [ShowClick event].blurEnabled &&
...
- (id<CAAction>)actionForLayer:(CALayer *)layer forKey:(NSString *)key
{
    ...
    if ([key isEqualToString:StringFromPressurePaintData(&appUndertakeValue)])
    {
        //animations are enabled
        //: CAAnimation *action = (CAAnimation *)[super actionForLayer:layer forKey:@"backgroundColor"];
        CAAnimation *action = (CAAnimation *)[super actionForLayer:layer forKey:StringFromPressurePaintData(&showImageName)];
...
- (UIImage *)compass
{
    ...
    __strong MiddleClassLayer *blurLayer = [self message];
    //: __strong CALayer *underlyingLayer = [self underlyingLayer];
    __strong CALayer *underlyingLayer = [self title];
    ...
    NSArray *hiddenViews = [self withColor];
    //: [underlyingLayer renderInContext:context];
    [underlyingLayer renderInContext:context];
    //: [self restoreSuperviewAfterSnapshot:hiddenViews];
    [self range:hiddenViews];
...
- (NSArray *)withColor
{
    ...
    __strong CALayer *blurlayer = [self random];
    //: __strong CALayer *underlyingLayer = [self underlyingLayer];
    __strong CALayer *underlyingLayer = [self title];
...
- (void)range:(NSArray *)hiddenLayers
{
...
- (UIImage *)ting:(UIImage *)snapshot backgroundByRadius:(CGFloat)blurRadius
{
    //: return [snapshot blurredImageWithRadius:blurRadius
    return [snapshot will:blurRadius
                                 //: iterations:self.iterations
                                 by:self.iterations
                                  //: tintColor:self.tintColor];
                                  style:self.tintColor];
}

//: - (void)setLayerContents:(UIImage *)image
- (void)setCustomTexts:(UIImage *)image
{
...
- (void)flush:(BOOL)async bar:(void (^)())completion
{
    //: if ([self shouldUpdate])
    if ([self along])
    {
        //: UIImage *snapshot = [self snapshotOfUnderlyingView];
        UIImage *snapshot = [self compass];
        //: if (async)
        if (async)
        {
            ...
            dispatch_async(dispatch_get_global_queue((-2), 0), ^{
                ...
                UIImage *blurredImage = [self ting:snapshot backgroundByRadius:self.blurRadius];
                //: dispatch_sync(dispatch_get_main_queue(), ^{
                dispatch_sync(dispatch_get_main_queue(), ^{

                    //: [self setLayerContents:blurredImage];
                    [self setCustomTexts:blurredImage];
        ...
        else
        {
            ...
            [self setCustomTexts:[self ting:snapshot backgroundByRadius:[self message].blurRadius]];
            //: if (completion) completion();
            if (completion) completion();
        }
    }
...
Byte *PressurePaintDataToByte(PressurePaintData *data) {
    if (data->taiAnyway < 114) return data->conferenceComfort;
    for (int i = 0; i < data->cozyComplete; i++) {
        data->conferenceComfort[i] ^= data->apartmentBarrel;
    }
    data->conferenceComfort[data->cozyComplete] = 0;
    data->taiAnyway = 83;
	if (data->cozyComplete >= 3) {
		data->showIllegal = data->conferenceComfort[0];
		data->lucifer = data->conferenceComfort[1];
		data->cliffRip = data->conferenceComfort[2];
	}
    return data->conferenceComfort;
}

NSString *StringFromPressurePaintData(PressurePaintData *data) {
    return [NSString stringWithUTF8String:(char *)PressurePaintDataToByte(data)];
}
