 ...
 *************************简要说明************************
 ...
 [SendAlert configMainWindow:self.window];
 
 
 Project 使用方法
 
 [SendAlert alert].cofing.XXXXX.XXXXX.LeeShow();
 
 ViewItem 使用方法
 
 [SendAlert actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface SendAlert: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull CleanValueBaseConfig *)hide;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull TinkleBaseConfig *)replacing;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull TeamWindow *)session;
...
+ (void)on:(NSString *)identifier green:(BOOL)force force:(void (^ _Nullable)(void))completionBlock;
...
+ (void)touchBlock:(void (^ _Nullable)(void))completionBlock;

//: @end
@end

//: @interface LEEBaseConfigModel: NSObject
@interface LabBottom: NSObject
...
@interface LabBottom(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([AccountPresentation windowLevel:1000] / [AccountPresentation viewController:xxxx]) */
...
@interface AccountPresentation: NSObject

//: + (LEEPresentation *)windowLevel: (UIWindowLevel)level;
+ (AccountPresentation *)disappear: (UIWindowLevel)level;

//: + (LEEPresentation *)viewController: (UIViewController *)controller;
+ (AccountPresentation *)view: (UIViewController *)controller;

//: @end
@end

//: @interface LEEBaseConfig: NSObject
@interface WireCircle: NSObject

//: @property (nonatomic, strong, nonnull) LEEBaseConfigModel *config;
@property (nonatomic, strong, nonnull) LabBottom *config;

//: @end
@end

//: @interface LEEAlertConfig: LEEBaseConfig
@interface CleanValueBaseConfig: WireCircle

//: @end
@end

//: @interface LEEActionSheetConfig: LEEBaseConfig
@interface TinkleBaseConfig: WireCircle

//: @end
@end

//: @interface LEEAlertWindow: UIWindow @end
@interface TeamWindow: UIWindow @end

//: @interface LEEBaseViewController: UIViewController @end
@interface PanoramicViewController: UIViewController @end

//: @interface LEEAlertViewController: LEEBaseViewController @end
@interface ClickImageViewController: PanoramicViewController @end

//: @interface LEEActionSheetViewController: LEEBaseViewController @end
@interface QuantityViewController: PanoramicViewController @end


//: @interface UIView (CornerRadii)
@interface UIView (Radii)
...
Radii CornerRadiiZero(void);

//: CornerRadii CornerRadiiNull(void);
Radii CornerRadiiNull(void);
