...
static NSString* const dreamStateValue = @"\u2026";

//: @interface M80AttributedLabel ()
@interface VideoTextView ()
...
- (NSAttributedString *)carmineWindow:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
...
- (NSAttributedString *)tap:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
- (NSMutableAttributedString *)colorationTitle:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[VideoTextView search] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
...
- (NSMutableAttributedString*)lineRequireHight:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self carmineWindow:text];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self tap:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self lineRequireHight:[self colorationTitle:attString]];
...
- (void)bubbleTitle:(NSString *)text
{
    //: NSAttributedString *attributedText = [self attributedString:text];
    NSAttributedString *attributedText = [self carmineWindow:text];
    //: [self appendAttributedText:attributedText];
    [self itemName:attributedText];
}

//: - (void)appendAttributedText:(NSAttributedString *)attributedText
- (void)itemName:(NSAttributedString *)attributedText
{
