...
NSString *StringFromConditionData(Byte *data);


//: bounds
Byte mainImpressValue[] = {18, 6, 3, 115, 100, 110, 117, 111, 98, 4};

//: lee_alert_view_
Byte mainCrushName[] = {43, 15, 13, 76, 148, 14, 86, 106, 51, 187, 27, 150, 247, 95, 119, 101, 105, 118, 95, 116, 114, 101, 108, 97, 95, 101, 101, 108, 104};

//: keyboardWillChangeFrame
Byte user_maxLopingTitle[] = {34, 23, 4, 226, 101, 109, 97, 114, 70, 101, 103, 110, 97, 104, 67, 108, 108, 105, 87, 100, 114, 97, 111, 98, 121, 101, 107, 147};

//: layoutSubviews
Byte appOccupyTitle[] = {63, 14, 9, 188, 188, 118, 113, 221, 1, 115, 119, 101, 105, 118, 98, 117, 83, 116, 117, 111, 121, 97, 108, 186};

//: 请在添加的某一项后面设置间距
Byte userMessageName[] = {94, 42, 11, 20, 207, 114, 181, 218, 111, 98, 156, 157, 183, 232, 180, 151, 233, 174, 189, 231, 190, 174, 232, 162, 157, 233, 142, 144, 229, 185, 161, 233, 128, 184, 228, 144, 159, 230, 132, 154, 231, 160, 138, 229, 187, 183, 230, 168, 156, 229, 183, 175, 232, 178};

//: bounds.size
Byte user_viewData[] = {5, 11, 9, 68, 226, 168, 161, 179, 241, 101, 122, 105, 115, 46, 115, 100, 110, 117, 111, 98, 117};

//: path
Byte mainRealistAnticipateValue[] = {38, 4, 9, 27, 248, 27, 183, 57, 176, 104, 116, 97, 112, 158};

//: frame
Byte k_scarValue[] = {78, 5, 8, 82, 250, 213, 81, 177, 101, 109, 97, 114, 102, 74};

//: lee_alert_button_
Byte showDogValue[] = {63, 17, 9, 86, 192, 152, 102, 52, 168, 95, 110, 111, 116, 116, 117, 98, 95, 116, 114, 101, 108, 97, 95, 101, 101, 108, 133};

//: windowLevel == %ld AND hidden == 0 
Byte app_expectGrossData[] = {10, 35, 9, 41, 228, 51, 176, 57, 57, 32, 48, 32, 61, 61, 32, 110, 101, 100, 100, 105, 104, 32, 68, 78, 65, 32, 100, 108, 37, 32, 61, 61, 32, 108, 101, 118, 101, 76, 119, 111, 100, 110, 105, 119, 9};
...
@property (nonatomic, assign) Radii modelCornerRadii;
//: @property (nonatomic, assign) CornerRadii modelActionSheetHeaderCornerRadii;
@property (nonatomic, assign) Radii modelActionSheetHeaderCornerRadii;
//: @property (nonatomic, assign) CornerRadii modelActionSheetCancelActionCornerRadii;
@property (nonatomic, assign) Radii modelActionSheetCancelActionCornerRadii;
...
@property (nonatomic, strong) AccountPresentation* modelPresentation;

//: @end
@end

//: @implementation LEEBaseConfigModel
@implementation LabBottom
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _modelPresentation = [AccountPresentation disappear:UIWindowLevelAlert];
    }
    //: return self;
    return self;
}
...
- (LEEConfigToView)LeeCustomView{
    ...
    return ^(UIView *view){
        ...
        return self.LeeAddCustomView(^(AssignView *custom) {
...
- (LEEConfigToStringAndBlock)LeeAction{
    ...
    return ^(NSString *title, void(^block)(void)){
        ...
        return self.LeeAddAction(^(NameVideo *action) {
...
- (LEEConfigToConfigLabel)LeeAddTitle{
    ...
    return ^(void(^block)(UILabel *)){
        ...
        return self.LeeAddItem(^(LineItem *item) {
...
- (LEEConfigToCustomView)LeeAddCustomView{
    ...
    return ^(void(^block)(AssignView *custom)){
...
- (LEEConfigToEdgeInsets)LeeItemInsets{
    ...
    return ^(UIEdgeInsets insets){
        ...
        //: } else {
            ...
            NSAssert(YES, StringFromConditionData(userMessageName));
        }
...
- (LEEConfigToPresentation)LeePresentation{
    ...
    return ^(AccountPresentation *presentation){
...
@implementation LabBottom (Project)
...
@implementation LabBottom (ViewItem)
...
@property (nonatomic, strong) TeamWindow *leeWindow;

//: @property (nonatomic, strong) NSMutableArray <LEEBaseConfig *>*queueArray;
@property (nonatomic, strong) NSMutableArray <WireCircle *>*queueArray;

//: @property (nonatomic, strong) LEEBaseViewController *viewController;
@property (nonatomic, strong) PanoramicViewController *viewController;

//: @end
@end

//: @protocol LEEAlertProtocol <NSObject>
@protocol ModeViewProtocol <NSObject>

//: - (void)closeWithCompletionBlock:(void (^)(void))completionBlock;
- (void)touchBlock:(void (^)(void))completionBlock;

//: @end
@end

//: @implementation LEEAlert
@implementation SendAlert

//: + (LEEAlert *)shareManager{
+ (SendAlert *)doingSave{

    //: static LEEAlert *alertManager;
    static SendAlert *alertManager;
    ...
    _dispatch_once(&onceToken, ^{
        ...
        alertManager = [[SendAlert alloc] init];
    //: });
    });
...
+ (CleanValueBaseConfig *)hide{

    //: return [[LEEAlertConfig alloc] init];
    return [[CleanValueBaseConfig alloc] init];
}

//: + (LEEActionSheetConfig *)actionsheet{
+ (TinkleBaseConfig *)replacing{

    //: return [[LEEActionSheetConfig alloc] init];
    return [[TinkleBaseConfig alloc] init];
}
...
+ (TeamWindow *)session{
    ...
    return [SendAlert doingSave].leeWindow;
}

//: + (void)configMainWindow:(UIWindow *)window{
+ (void)tap:(UIWindow *)window{

    //: if (window) [LEEAlert shareManager].mainWindow = window;
    if (window) [SendAlert doingSave].mainWindow = window;
}
...
+ (void)toDisplay{
    ...
    if ([SendAlert doingSave].queueArray.count) [SendAlert doingSave].queueArray.lastObject.config.modelFinishConfig();
}
...
+ (void)complete{
    ...
    WireCircle * _Nullable last = [SendAlert doingSave].queueArray.lastObject;

    //: [[LEEAlert shareManager].queueArray removeAllObjects];
    [[SendAlert doingSave].queueArray removeAllObjects];

    //: if ([last respondsToSelector:@selector(close)]) [last performSelector:@selector(close)];
    if ([last respondsToSelector:@selector(notMyOff)]) [last performSelector:@selector(notMyOff)];
}
...
+ (BOOL)view{
    ...
    return [SendAlert doingSave].queueArray.count == 0;
}

//: + (BOOL)containsQueueWithIdentifier:(NSString *)identifier {
+ (BOOL)overProgress:(NSString *)identifier {

    //: for (LEEBaseConfig *config in [LEEAlert shareManager].queueArray) {
    for (WireCircle *config in [SendAlert doingSave].queueArray) {
...
+ (void)withAtHandIdentifierBlockContainer:(NSString *)identifier current:(void (^ _Nullable)(void))completionBlock{

    //: [self closeWithIdentifier:identifier force:NO completionBlock:completionBlock];
    [self on:identifier green:NO force:completionBlock];
}

//: + (void)closeWithIdentifier:(NSString *)identifier force:(BOOL)force completionBlock:(void (^)(void))completionBlock{
+ (void)on:(NSString *)identifier green:(BOOL)force force:(void (^)(void))completionBlock{

    //: if ([LEEAlert shareManager].queueArray.count) {
    if ([SendAlert doingSave].queueArray.count) {

        //: BOOL isLast = false;
        BOOL isLast = false;

        //: NSUInteger count = [LEEAlert shareManager].queueArray.count;
        NSUInteger count = [SendAlert doingSave].queueArray.count;
        ...
        for (NSUInteger i = 0; i < count; i++) {
            ...
            WireCircle *config = [SendAlert doingSave].queueArray[i];

            //: LEEBaseConfigModel *model = config.config;
            LabBottom *model = config.config;
            ...
            if (model.modelIdentifier != nil && [identifier isEqualToString: model.modelIdentifier]) {
                ...
                if (i == count - 1 && [[SendAlert doingSave] viewController]) {
                    //: if (force) {
                    if (force) {
        ...
        [[SendAlert doingSave].queueArray removeObjectsAtIndexes:indexs];

        //: if (isLast) {
        if (isLast) {

            //: [LEEAlert closeWithCompletionBlock:completionBlock];
            [SendAlert touchBlock:completionBlock];
...
+ (void)touchBlock:(void (^)(void))completionBlock{
    ...
    if ([SendAlert doingSave].queueArray.count) {
        ...
        WireCircle *item = [SendAlert doingSave].queueArray.lastObject;
        ...
        if ([item respondsToSelector:@selector(touchBlock:)]) [item performSelector:@selector(touchBlock:) withObject:completionBlock];
...
- (NSMutableArray <WireCircle *>*)queueArray{
    ...
    if (!_leeWindow) {
        ...
        if (@available(iOS 13.0, *)) {
            ...
            if (_mainWindow.windowScene) {
                ...
                _leeWindow = [[TeamWindow alloc] initWithWindowScene: _mainWindow.windowScene];

            //: } else {
            } else {
                //: _leeWindow = [[LEEAlertWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
                _leeWindow = [[TeamWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
            }
...
@interface UIView (SameChange)

//: @property (nonatomic, assign) CornerRadii lee_alert_cornerRadii;
@property (nonatomic, assign) Radii lee_alert_cornerRadii;

//: @end
@end

//: @implementation UIView (LEEAlertExtension)
@implementation UIView (SameChange)

//: CornerRadii CornerRadiiMake(CGFloat topLeft, CGFloat topRight, CGFloat bottomLeft, CGFloat bottomRight) {
Radii CornerRadiiMake(CGFloat topLeft, CGFloat topRight, CGFloat bottomLeft, CGFloat bottomRight) {
    //: return (CornerRadii){
    return (Radii){
        //: topLeft,
        topLeft,
...
CGPathRef _Nullable LEECGPathCreateWithRoundedRect(CGRect bounds, Radii cornerRadii) {
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        NSArray *selStringsArray = @[StringFromConditionData(appOccupyTitle)];

        //: [selStringsArray enumerateObjectsUsingBlock:^(NSString *selString, NSUInteger idx, BOOL *stop) {
        [selStringsArray enumerateObjectsUsingBlock:^(NSString *selString, NSUInteger idx, BOOL *stop) {

            //: NSString *leeSelString = [@"lee_alert_view_" stringByAppendingString:selString];
            NSString *leeSelString = [StringFromConditionData(mainCrushName) stringByAppendingString:selString];
...
- (void)click{
    ...
    if (!CornerRadiiEqualTo([self lee_alert_cornerRadii], CornerRadiiNull())) {
        ...
        if (!CGPathEqualToPath(lastPath, path)) {
            ...
            [lastLayer removeAnimationForKey:StringFromConditionData(mainRealistAnticipateValue)];
            // 重置新路径mask
            ...
            CAAnimation *temp = [self.layer animationForKey:StringFromConditionData(user_viewData)];
            //: if (temp) {
            if (temp) {
                //: CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"path"];
                CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:StringFromConditionData(mainRealistAnticipateValue)];
                ...
                [maskLayer addAnimation:animation forKey:StringFromConditionData(mainRealistAnticipateValue)];
            }
...
- (void)administratorTitle{

    //: [self lee_alert_view_layoutSubviews];
    [self administratorTitle];
...
- (void)setLee_alert_cornerRadii:(Radii)cornerRadii{

    //: NSValue *value = [NSValue valueWithBytes:&cornerRadii objCType:@encode(CornerRadii)];
    NSValue *value = [NSValue valueWithBytes:&cornerRadii objCType:@encode(Radii)];
...
@interface UIButton (SameChange)

//: @end
@end

//: @implementation UIButton (LEEAlertExtension)
@implementation UIButton (SameChange)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        [selStringsArray enumerateObjectsUsingBlock:^(NSString *selString, NSUInteger idx, BOOL *stop) {
            ...
            NSString *leeSelString = [StringFromConditionData(showDogValue) stringByAppendingString:selString];
...
@property (nonatomic, copy) void (^updateBlock)(LineItem *);
...
@property (nonatomic, copy) void (^updateBlock)(NameVideo *);

//: @end
@end

//: @implementation LEEAction
@implementation NameVideo
...
@property (nonatomic, strong) LineItem *item;
...
+ (InputItemView *)pin{
    ...
    return [[InputItemView alloc] init];
}
...
+ (MessageLabel *)totalIn{
    ...
    return [[MessageLabel alloc] init];
}
...
@interface TextDisplayView : UITextField

//: @property (nonatomic, strong) LEEItem *item;
@property (nonatomic, strong) LineItem *item;

//: + (LEEItemTextField *)textField;
+ (TextDisplayView *)sharedField;
...
+ (TextDisplayView *)sharedField{
    ...
    return [[TextDisplayView alloc] init];
}
...
+ (ClickButton *)quantityBy{
    ...
    return [ClickButton buttonWithType:UIButtonTypeCustom];;
}

//: - (void)setAction:(LEEAction *)action{
- (void)setAction:(NameVideo *)action{
    ...
    if (action.backgroundColor) [self setBackgroundImage:[self elite:action.backgroundColor] forState:UIControlStateNormal];
    ...
    if (action.backgroundHighlightColor) [self setBackgroundImage:[self elite:action.backgroundHighlightColor] forState:UIControlStateHighlighted];
    ...
    if (action.height) [self setColor:action.height];
    ...
    //: } else {
        ...
        if (action.borderPosition & LEEActionBorderPositionTop) [self end]; else [self reload];

        //: if (action.borderPosition & LEEActionBorderPositionBottom) [self addBottomBorder]; else [self removeBottomBorder];
        if (action.borderPosition & LEEActionBorderPositionBottom) [self represent]; else [self cell];

        //: if (action.borderPosition & LEEActionBorderPositionLeft) [self addLeftBorder]; else [self removeLeftBorder];
        if (action.borderPosition & LEEActionBorderPositionLeft) [self date]; else [self circumference];

        //: if (action.borderPosition & LEEActionBorderPositionRight) [self addRightBorder]; else [self removeRightBorder];
        if (action.borderPosition & LEEActionBorderPositionRight) [self hide]; else [self table];
    }
    ...
    action.updateBlock = ^(NameVideo *act) {
...
- (CALayer *)topLayer{
    ...
    if (!_topLayer) _topLayer = [self content];
...
- (CALayer *)bottomLayer{
    ...
    if (!_bottomLayer) _bottomLayer = [self content];
...
- (CALayer *)leftLayer{
    ...
    if (!_leftLayer) _leftLayer = [self content];
...
- (CALayer *)rightLayer{
    ...
    if (!_rightLayer) _rightLayer = [self content];
...
- (void)dealloc{
    ...
    if (_container) {
        ...
        [_container removeObserver:self forKeyPath:StringFromConditionData(k_scarValue)];
        //: [_container removeObserver:self forKeyPath:@"bounds"];
        [_container removeObserver:self forKeyPath:StringFromConditionData(mainImpressValue)];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if ([view isEqual:self.container] && self.isAutoWidth) {
        ...
        if ([keyPath isEqualToString:StringFromConditionData(k_scarValue)] || [keyPath isEqualToString:StringFromConditionData(mainImpressValue)]) {
    ...
    if ([view isEqual:self.view]) {
        ...
        if ([keyPath isEqualToString:StringFromConditionData(k_scarValue)]) {
        ...
        if ([keyPath isEqualToString:StringFromConditionData(mainImpressValue)]) {
...
- (UIView *)container{
    ...
    if (!_container) {
        ...
        [_container addObserver: self forKeyPath: StringFromConditionData(k_scarValue) options: NSKeyValueObservingOptionNew context: nil];
        //: [_container addObserver: self forKeyPath: @"bounds" options: NSKeyValueObservingOptionNew context: nil];
        [_container addObserver: self forKeyPath: StringFromConditionData(mainImpressValue) options: NSKeyValueObservingOptionNew context: nil];
    }
...
- (void)setView:(UIView *)view{
    ...
    if (_view) {
        ...
        [_view removeObserver:self forKeyPath:StringFromConditionData(k_scarValue)];
        //: [_view removeObserver:self forKeyPath:@"bounds"];
        [_view removeObserver:self forKeyPath:StringFromConditionData(mainImpressValue)];
    }
    ...
    if (_view) {
        ...
        [view addObserver: self forKeyPath: StringFromConditionData(k_scarValue) options: NSKeyValueObservingOptionNew context: nil];
        //: [view addObserver: self forKeyPath: @"bounds" options: NSKeyValueObservingOptionNew context: nil];
        [view addObserver: self forKeyPath: StringFromConditionData(mainImpressValue) options: NSKeyValueObservingOptionNew context: nil];
...
@interface TeamPresentation: AccountPresentation
...
@interface InfoControllerPresentation: AccountPresentation
...
@implementation AccountPresentation

//: + (LEEPresentationWindow *)windowLevel:(UIWindowLevel)level {
+ (TeamPresentation *)disappear:(UIWindowLevel)level {
    //: LEEPresentationWindow *presentation = [[LEEPresentationWindow alloc] init];
    TeamPresentation *presentation = [[TeamPresentation alloc] init];
...
+ (InfoControllerPresentation *)view:(UIViewController *)controller {
    //: LEEPresentationViewController *presentation = [[LEEPresentationViewController alloc] init];
    InfoControllerPresentation *presentation = [[InfoControllerPresentation alloc] init];
...
@implementation TeamPresentation

//: @end
@end

//: @implementation LEEPresentationViewController
@implementation InfoControllerPresentation
...
@interface PanoramicViewController ()<UIGestureRecognizerDelegate>

//: @property (nonatomic, strong) LEEBaseConfigModel *config;
@property (nonatomic, strong) LabBottom *config;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (self.config.modelIsClickBackgroundClose) [self smart:nil];
}
...
- (void)exist:(void (^)(void))completionBlock{
...
- (void)smart:(void (^)(void))completionBlock{

    //: [[LEEAlert shareManager].leeWindow endEditing:YES];
    [[SendAlert doingSave].leeWindow endEditing:YES];
}
...
- (UIWindow *)currentKeyWindow{
    ...
    if (!_currentKeyWindow) _currentKeyWindow = [SendAlert doingSave].mainWindow;
    ...
    if (_currentKeyWindow.windowLevel != UIWindowLevelNormal) {
        ...
        NSPredicate *predicate = [NSPredicate predicateWithFormat:StringFromConditionData(app_expectGrossData), UIWindowLevelNormal];
    ...
    if (_currentKeyWindow) if (![SendAlert doingSave].mainWindow) [SendAlert doingSave].mainWindow = _currentKeyWindow;
...
@interface ClickImageViewController ()
...
@property (nonatomic, strong) NSMutableArray <ClickButton *>*alertActionArray;
...
- (void)recented:(NSNotification *)notify{
    ...
    if (self.config.modelIsAvoidKeyboard && local) {
        ...
        [UIView beginAnimations:StringFromConditionData(user_maxLopingTitle) context:NULL];
...
- (void)fruitage{
    ...
    [self present:CGRectGetWidth(self.view.frame) bubble:CGRectGetHeight(self.view.frame)];
}

//: - (void)updateAlertLayoutWithViewWidth:(CGFloat)viewWidth ViewHeight:(CGFloat)viewHeight{
- (void)present:(CGFloat)viewWidth bubble:(CGFloat)viewHeight{
    ...
    if (isShowingKeyboard) {
        ...
        if (keyboardFrame.size.height) {
            ...
            if (self.config.modelIsActionFollowScrollEnabled) {
                ...
                CGFloat itemsHeight = [self state:alertViewMaxWidth];
            ...
            //: } else {
                ...
                CGFloat actionsHeight = [self name:0 system:alertViewMaxWidth];
            ...
            [self.itemsScrollView scrollRectToVisible:[self infoResponder:self.itemsScrollView].frame animated:YES];
        }
...
- (CGFloat)state:(CGFloat)maxWidth{
    ...
    [self.alertItemArray enumerateObjectsUsingBlock:^(id _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([item isKindOfClass:[UIView class]]) {
            ...
            InputItemView *view = (InputItemView *)item;
        ...
        } else if ([item isKindOfClass:[AssignView class]]) {

            //: LEECustomView *custom = (LEECustomView *)item;
            AssignView *custom = (AssignView *)item;
    ...
    if (self.config.modelIsActionFollowScrollEnabled) {
        ...
        finalHeight += [self name:finalHeight system:maxWidth];
    }
...
- (CGFloat)name:(CGFloat)initialPosition system:(CGFloat)maxWidth{
    ...
    for (ClickButton *button in self.alertActionArray) {
    ...
    if (self.alertActionArray.count == 2) {
        ...
        ClickButton *buttonA = self.alertActionArray.count == self.config.modelActionArray.count ? self.alertActionArray.firstObject : self.alertActionArray.lastObject;
        ...
        ClickButton *buttonB = self.alertActionArray.count == self.config.modelActionArray.count ? self.alertActionArray.lastObject : self.alertActionArray.firstObject;
...
- (void)with{
    ...
    [self.config.modelItemArray enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (item.type) {
            ...
            case LEEItemTypeTitle:
            {
                ...
                MessageLabel *label = [MessageLabel totalIn];
            ...
            case LEEItemTypeCustomView:
            {
                ...
                void(^block)(AssignView *) = item.block;

                //: LEECustomView *custom = [[LEECustomView alloc] init];
                AssignView *custom = [[AssignView alloc] init];
            ...
            case LEEItemTypeTextField:
            {
                ...
                TextDisplayView *textField = [TextDisplayView sharedField];
    ...
    [self.config.modelActionArray enumerateObjectsUsingBlock:^(id item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        void (^block)(NameVideo *action) = item;

        //: LEEAction *action = [[LEEAction alloc] init];
        NameVideo *action = [[NameVideo alloc] init];
        ...
        ClickButton *button = [ClickButton quantityBy];

        //: button.action = action;
        button.action = action;

        //: [button addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
        [button addTarget:self action:@selector(quicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)quicked:(ClickButton *)sender{
...
- (void)beginses:(UITapGestureRecognizer *)tap{

    //: if (self.config.modelIsClickHeaderClose) [self closeAnimationsWithCompletionBlock:nil];
    if (self.config.modelIsClickHeaderClose) [self smart:nil];
}
...
- (UIView *)infoResponder:(UIView *)view{
    ...
    for (UIView *subView in view.subviews) {
        ...
        UIView *firstResponder = [self infoResponder:subView];
...
- (UIScrollView *)itemsScrollView{
    ...
    if (!_itemsScrollView) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(beginses:)];
...
- (NSMutableArray <ClickButton *>*)alertActionArray{
...
@property (nonatomic, strong) NSMutableArray <ClickButton *>*actionSheetActionArray;
...
@property (nonatomic, strong) ClickButton *actionSheetCancelAction;
...
- (void)date{
    ...
    [self along:CGRectGetWidth(self.view.frame) item:CGRectGetHeight(self.view.frame)];
}

//: - (void)updateActionSheetLayoutWithViewWidth:(CGFloat)viewWidth ViewHeight:(CGFloat)viewHeight{
- (void)along:(CGFloat)viewWidth item:(CGFloat)viewHeight{
    ...
    if (self.config.modelIsActionFollowScrollEnabled) {
        ...
        CGFloat itemsHeight = [self vocalization:actionSheetViewMaxWidth];
    ...
    //: } else {
        ...
        CGFloat actionsHeight = [self sendPress:0 darkGreen:actionSheetViewMaxWidth];
...
//: - (CGFloat)updateItemsLayoutWithMaxWidth:(CGFloat)maxWidth{
- (CGFloat)vocalization:(CGFloat)maxWidth{
    ...
    if (self.config.modelIsActionFollowScrollEnabled) {
        ...
        //: finalHeight += [self updateActionsLayoutWithInitialPosition:finalHeight MaxWidth:maxWidth];
        finalHeight += [self sendPress:finalHeight darkGreen:maxWidth];
    }
...
//: - (CGFloat)updateActionsLayoutWithInitialPosition:(CGFloat)initialPosition MaxWidth:(CGFloat)maxWidth{
- (CGFloat)sendPress:(CGFloat)initialPosition darkGreen:(CGFloat)maxWidth{
    ...
    for (ClickButton *button in self.actionSheetActionArray) {
...
- (NSMutableArray <ClickButton *>*)actionSheetActionArray{
...
@interface WireCircle ()<ModeViewProtocol>
    ...
    if (self) {
        ...
        self.config.modelFinishConfig = ^{
            ...
            if ([SendAlert doingSave].queueArray.count) {
                ...
                WireCircle *last = [SendAlert doingSave].queueArray.lastObject;
                ...
                if (!last.config.modelIsQueue && last.config.modelQueuePriority <= strongSelf.config.modelQueuePriority) {
                    ...
                    [[SendAlert doingSave].queueArray removeObject:last];
                }
                ...
                if (![[SendAlert doingSave].queueArray containsObject:strongSelf]) {

                    //: [[LEEAlert shareManager].queueArray addObject:strongSelf];
                    [[SendAlert doingSave].queueArray addObject:strongSelf];

                    //: [[LEEAlert shareManager].queueArray sortUsingComparator:^NSComparisonResult(LEEBaseConfig *configA, LEEBaseConfig *configB) {
                    [[SendAlert doingSave].queueArray sortUsingComparator:^NSComparisonResult(WireCircle *configA, WireCircle *configB) {
                ...
                if ([SendAlert doingSave].queueArray.lastObject == strongSelf) [strongSelf image];
...
- (void)image{
    ...
    if (![SendAlert doingSave].viewController) return;

    //: [LEEAlert shareManager].viewController.config = self.config;
    [SendAlert doingSave].viewController.config = self.config;

    //: if ([self.config.modelPresentation isKindOfClass:[LEEPresentationWindow class]]) {
    if ([self.config.modelPresentation isKindOfClass:[TeamPresentation class]]) {

        //: LEEPresentationWindow *presentation = (LEEPresentationWindow *)self.config.modelPresentation;
        TeamPresentation *presentation = (TeamPresentation *)self.config.modelPresentation;

        //: [LEEAlert shareManager].leeWindow.rootViewController = [LEEAlert shareManager].viewController;
        [SendAlert doingSave].leeWindow.rootViewController = [SendAlert doingSave].viewController;

        //: [LEEAlert shareManager].leeWindow.windowLevel = presentation.windowLevel;
        [SendAlert doingSave].leeWindow.windowLevel = presentation.windowLevel;

        //: [LEEAlert shareManager].leeWindow.hidden = NO;
        [SendAlert doingSave].leeWindow.hidden = NO;

        //: if (@available(iOS 13.0, *)) {
        if (@available(iOS 13.0, *)) {
            //: [LEEAlert shareManager].leeWindow.overrideUserInterfaceStyle = self.config.modelUserInterfaceStyle;
            [SendAlert doingSave].leeWindow.overrideUserInterfaceStyle = self.config.modelUserInterfaceStyle;

        //: } else {
        } else {
            //: [[LEEAlert shareManager].leeWindow makeKeyAndVisible];
            [[SendAlert doingSave].leeWindow makeKeyAndVisible];
        }
    ...
    if ([self.config.modelPresentation isKindOfClass:[InfoControllerPresentation class]]) {

        //: LEEPresentationViewController *presentation = (LEEPresentationViewController *)self.config.modelPresentation;
        InfoControllerPresentation *presentation = (InfoControllerPresentation *)self.config.modelPresentation;
        ...
        [presentation.viewController addChildViewController:[SendAlert doingSave].viewController];

        //: [presentation.viewController.view addSubview:[LEEAlert shareManager].viewController.view];
        [presentation.viewController.view addSubview:[SendAlert doingSave].viewController.view];

        //: if (@available(iOS 13.0, *)) {
        if (@available(iOS 13.0, *)) {
            //: [LEEAlert shareManager].viewController.view.overrideUserInterfaceStyle = self.config.modelUserInterfaceStyle;
            [SendAlert doingSave].viewController.view.overrideUserInterfaceStyle = self.config.modelUserInterfaceStyle;
        }

        //: [LEEAlert shareManager].viewController.view.frame = presentation.viewController.view.bounds;
        [SendAlert doingSave].viewController.view.frame = presentation.viewController.view.bounds;
        ...
        [SendAlert doingSave].viewController.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;

        //: [[LEEAlert shareManager].viewController didMoveToParentViewController:presentation.viewController];
        [[SendAlert doingSave].viewController didMoveToParentViewController:presentation.viewController];
    ...
    [SendAlert doingSave].viewController.openFinishBlock = ^{

    //: };
    };

    //: [LEEAlert shareManager].viewController.closeFinishBlock = ^{
    [SendAlert doingSave].viewController.closeFinishBlock = ^{
        ...
        if ([SendAlert doingSave].queueArray.lastObject == strongSelf) {

            //: [strongSelf close];
            [strongSelf notMyOff];

            //: [[LEEAlert shareManager].queueArray removeObject:strongSelf];
            [[SendAlert doingSave].queueArray removeObject:strongSelf];

            //: if (strongSelf.config.modelIsContinueQueueDisplay) [LEEAlert continueQueueDisplay];
            if (strongSelf.config.modelIsContinueQueueDisplay) [SendAlert toDisplay];
...
- (void)notMyOff{
    ...
    if ([self.config.modelPresentation isKindOfClass:[TeamPresentation class]]) {
        ...
        [SendAlert doingSave].leeWindow.hidden = YES;

        //: [[LEEAlert shareManager].leeWindow resignKeyWindow];
        [[SendAlert doingSave].leeWindow resignKeyWindow];

        //: [LEEAlert shareManager].leeWindow.rootViewController = nil;
        [SendAlert doingSave].leeWindow.rootViewController = nil;
    }
    ...
    if ([self.config.modelPresentation isKindOfClass:[InfoControllerPresentation class]]) {
        ...
        [[SendAlert doingSave].viewController willMoveToParentViewController:nil];

        //: [[LEEAlert shareManager].viewController.view removeFromSuperview];
        [[SendAlert doingSave].viewController.view removeFromSuperview];

        //: [[LEEAlert shareManager].viewController removeFromParentViewController];
        [[SendAlert doingSave].viewController removeFromParentViewController];
    }

    //: [LEEAlert shareManager].viewController = nil;
    [SendAlert doingSave].viewController = nil;
}
...
- (void)touchBlock:(void (^)(void))completionBlock{
    ...
    if ([SendAlert doingSave].viewController) [[SendAlert doingSave].viewController smart:completionBlock];
}
...
- (LabBottom *)config{
    ...
    if (!_config) _config = [[LabBottom alloc] init];
...
@implementation CleanValueBaseConfig
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        .LeeConfigMaxHeight(^CGFloat(LEEScreenOrientationType type, CGSize size) {
            ...
            return size.height - 40.0f - ({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [SendAlert session].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).top - ({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [SendAlert session].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).bottom;
        //: })
        })
...
- (void)image {
    ...
    [SendAlert doingSave].viewController = [[ClickImageViewController alloc] init];
...
@implementation TinkleBaseConfig
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        .LeeConfigMaxWidth(^CGFloat(LEEScreenOrientationType type, CGSize size) {
            ...
            return type == LEEScreenOrientationTypeHorizontal ? size.height - ({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [SendAlert session].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).top - ({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [SendAlert session].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).bottom - 20.0f : size.width - ({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [SendAlert session].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).left - ({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [SendAlert session].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).right - 20.0f;
        //: })
        })
        //: .LeeConfigMaxHeight(^CGFloat(LEEScreenOrientationType type, CGSize size) {
        .LeeConfigMaxHeight(^CGFloat(LEEScreenOrientationType type, CGSize size) {
...
- (void)image {
    ...
    [SendAlert doingSave].viewController = [[QuantityViewController alloc] init];
...
Byte * ConditionDataToCache(Byte *data) {
    int acceptanceBy = data[0];
    int joinNameGrand = data[1];
    int forwardSole = data[2];
    if (!acceptanceBy) return data + forwardSole;
    for (int i = 0; i < joinNameGrand / 2; i++) {
        int begin = forwardSole + i;
        int end = forwardSole + joinNameGrand - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[forwardSole + joinNameGrand] = 0;
    return data + forwardSole;
}

NSString *StringFromConditionData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ConditionDataToCache(data)];
}  
